...
BEE_API_VERSION ?= "$(shell grep '^  version:' openapi/Aisc.yaml | awk '{print $$2}')"
BEE_DEBUG_API_VERSION ?= "$(shell grep '^  version:' openapi/AiscDebug.yaml | awk '{print $$2}')"
...
LDFLAGS ?= -s -w \
-X github.com/Aisc/aisc.version="$(VERSION)" \
-X github.com/Aisc/aisc.commitHash="$(COMMIT_HASH)" \
-X github.com/Aisc/aisc.commitTime="$(COMMIT_TIME)" \
-X github.com/Aisc/aisc/pkg/api.Version="$(BEE_API_VERSION)" \
-X github.com/Aisc/aisc/pkg/api.DebugVersion="$(BEE_DEBUG_API_VERSION)" \
-X github.com/Aisc/aisc/pkg/p2p/libp2p.reachabilityOverridePublic="$(REACHABILITY_OVERRIDE_PUBLIC)" \
-X github.com/Aisc/aisc/pkg/postage/listener.batchFactorOverridePublic="$(BATCHFACTOR_OVERRIDE_PUBLIC)"
...
ifeq ($(BEEKEEPER_BRANCH), master)
	curl -sSfL https://raw.githubusercontent.com/Aisc/aisckeeper/master/scripts/install.sh | BEEKEEPER_INSTALL_DIR=$(BEEKEEPER_INSTALL_DIR) USE_SUDO=$(BEEKEEPER_USE_SUDO) bash
else
	git clone -b $(BEEKEEPER_BRANCH) https://github.com/Aisc/aisckeeper.git && mv beekeeper beekeeper_src && cd beekeeper_src && mkdir -p $(BEEKEEPER_INSTALL_DIR) && make binary
ifeq ($(BEEKEEPER_USE_SUDO), true)
	...
	test -f ~/.beekeeper.yaml || curl -sSfL https://raw.githubusercontent.com/Aisc/aisckeeper/$(BEEKEEPER_BRANCH)/config/beekeeper-local.yaml -o ~/.beekeeper.yaml
	mkdir -p ~/.beekeeper && curl -sSfL https://raw.githubusercontent.com/Aisc/aisckeeper/$(BEEKEEPER_BRANCH)/config/local.yaml -o ~/.beekeeper/local.yaml

.PHONY: beelocal
beelocal:
	curl -sSfL https://raw.githubusercontent.com/Aisc/aisclocal/$(BEELOCAL_BRANCH)/beelocal.sh | bash
