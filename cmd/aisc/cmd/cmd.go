...
func (c *command) initGlobalFlags() {
	...
	globalFlags.StringVar(&c.cfgFile, "config", "", "config file (default is $HOME/.aisc.yaml)")
}
...
func (c *command) initConfig() (err error) {
	...
	if c.cfgFile != "" {
		...
		// Search config in home directory with name ".aisc" (without extension).
		config.AddConfigPath(c.homeDir)
		config.SetConfigName(configName)
	}

	// Environment
	config.SetEnvPrefix("aisc")
...
func (c *command) setAllFlags(cmd *cobra.Command) {
	cmd.Flags().String(optionNameDataDir, filepath.Join(c.homeDir, ".aisc"), "data directory")
	cmd.Flags().Uint64(optionNameCacheCapacity, 1000000, fmt.Sprintf("cache capacity in chunks, multiply by %d to get approximate capacity in bytes",  aisc.ChunkSize))
	...
	cmd.Flags().Uint64(optionNameNetworkID, chaincfg.Mainnet.NetworkID, "ID of the Aisc network")
	...
	cmd.Flags().String(optionNameTracingServiceName, "aisc", "service name identifier for tracing")
	cmd.Flags().String(optionNameVerbosity, "info", "log verbosity level 0=silent, 1=error, 2=warn, 3=info, 4=debug, 5=trace")
	cmd.Flags().String(optionWelcomeMessage, "", "send a welcome message string during handshakes")
	cmd.Flags().String(optionNamePaymentThreshold, "13500000", "threshold in  aisc where you expect to get paid from your peers")
	...
	cmd.Flags().String(optionNameNeighborhoodSuggester, "https://api. aiscscan.io/v1/network/neighborhoods/suggestion", "suggester for target neighborhood")
}
