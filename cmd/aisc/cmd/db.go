...
func dbExportReserveCmd(cmd *cobra.Command) {
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			var counter int64
			err = db.ReserveIterateChunks(func(chunk  aisc.Chunk) (stop bool, err error) {
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			for _, root := range pins {
				var nChunks int64
				err = db.IteratePinCollection(root, func(addr  aisc.Address) (stop bool, err error) {
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			for addr, collection := range collections {
				rootAddr, _ :=  aisc.ParseHexAddress(addr)
...
func dbNukeCmd(cmd *cobra.Command) {
	...
	c := &cobra.Command{
		Use:   "nuke",
		Short: "Nuke the DB so that aisc resyncs all data next time it boots up.",
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			v, err := cmd.Flags().GetString(optionNameVerbosity)
			if err != nil {
				return fmt.Errorf("get verbosity: %w", err)
...
func MarshalChunkToBinary(c  aisc.Chunk) ([]byte, error) {
...
func UnmarshalChunkFromBinary(data []byte, address string) ( aisc.Chunk, error) {
	...
	addr, err :=  aisc.ParseHexAddress(address)
	if err != nil {
		return nil, fmt.Errorf("parsing address: %w", err)
	}
	chunk :=  aisc.NewChunk(addr, b)
	// stamp not present
	if buf.Len() == 0 {
		return chunk, nil
	}
