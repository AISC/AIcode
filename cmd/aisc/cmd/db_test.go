...
import (
	...
	storagetest "github.com/aisc/pkg/storage/testing"
...
func TestDBExportImport(t *testing.T) {
	...
	err = db2.ReserveIterateChunks(func(chunk  aisc.Chunk) (bool, error) {
...
func TestDBExportImportPinning(t *testing.T) {
	...
	for _, addr := range addresses {
		rootAddr, err :=  aisc.ParseHexAddress(addr.String())
		if err != nil {
			t.Fatal(err)
		}
		err = db2.IteratePinCollection(rootAddr, func(ch  aisc.Address) (bool, error) {
			chunks[ch.String()]++
			return false, nil
		})
		if err != nil {
