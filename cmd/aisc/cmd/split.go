...
type putter struct {
	cb func(chunk  aisc.Chunk) error
}

func (s *putter) Put(_ context.Context, chunk  aisc.Chunk) error {
	return s.cb(chunk)
}
func newPutter(cb func(ch  aisc.Chunk) error) *putter {
	return &putter{
		cb: cb,
	}
}
...
func splitRefs(cmd *cobra.Command) {
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var refs []string
			store := newPutter(func(ch  aisc.Chunk) error {
...
func splitChunks(cmd *cobra.Command) {
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			store := newPutter(func(chunk  aisc.Chunk) error {
