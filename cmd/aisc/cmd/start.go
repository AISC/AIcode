...
import (
	...
	"github.com/aisc"
	chaincfg "github.com/aisc/pkg/config"
	"github.com/aisc/pkg/crypto"
	"github.com/aisc/pkg/crypto/clef"
	"github.com/aisc/pkg/keystore"
	filekeystore "github.com/aisc/pkg/keystore/file"
	memkeystore "github.com/aisc/pkg/keystore/mem"
	"github.com/aisc/pkg/log"
	"github.com/aisc/pkg/node"
...
const (
	serviceName      = "AiscAiscSvc"
	libp2pPKFilename = "libp2p_v2"
)

//go:embed aisc-welcome-message.txt
var aiscWelcomeMessage string

func (c *command) initStartCmd() (err error) {
	cmd := &cobra.Command{
		Use:               "start",
		Short:             "Start a Aisc node",
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			fmt.Print(aiscWelcomeMessage)
			fmt.Printf("\n\nversion: %v - planned to be supported until %v, please follow https:// aisc.org/\n\n", aisc.Version, endSupportDate())
			logger.Info("aisc version", "version", aisc.Version)

			go startTimeBomb(logger)

			// ctx is global context of aisc node; which is canceled after interrupt signal is received.
			...
			// Building aisc node can take up some time (because node.NewAisc(...) is compute have function )
			// Because of this we need to do it in background so that program could be terminated when interrupt singal is received
			// while aisc node is being constructed.
			respC := buildAiscNodeAsync(ctx, c, cmd, logger)
			var aiscNode atomic.Value

			p := &program{
				start: func() {
					// Wait for aisc node to fully build and initialized
					select {
					case resp := <-respC:
						if resp.err != nil {
							logger.Error(resp.err, "failed to build aisc node")
							return
						}
						aiscNode.Store(resp.aisc)
					case <-ctx.Done():
						return
					}

					// Aisc has fully started at this point, from now on we
					// block main goroutine until it is interrupted or stopped
					select {
					case <-ctx.Done():
					case <-aiscNode.Load().(*node.Aisc).SyncingStopped():
				...
				stop: func() {
					...
					go func(aiscNode *node.Aisc) {
						defer close(done)

						if err := aiscNode.Shutdown(); err != nil {
...
type buildAiscNodeResp struct {
	aisc *node.Aisc
	err error
}

func buildAiscNodeAsync(ctx context.Context, c *command, cmd *cobra.Command, logger log.Logger) <-chan buildAiscNodeResp {
	respC := make(chan buildAiscNodeResp, 1)

	go func() {
		aisc, err := buildAiscNode(ctx, c, cmd, logger)
		respC <- buildAiscNodeResp{aisc, err}
	}()

	return respC
}

func buildAiscNode(ctx context.Context, c *command, cmd *cobra.Command, logger log.Logger) (*node.Aisc, error) {
	var err error
	...
	staticNodes := make([] aisc.Address, 0, len(staticNodesOpt))
	for _, p := range staticNodesOpt {
		addr, err :=  aisc.ParseHexAddress(p)
		if err != nil {
			return nil, fmt.Errorf("invalid  aisc address %q configured for static node", p)
		}
	...
	b, err := node.NewAisc(ctx, c.config.GetString(optionNameP2PAddr), signerConfig.publicKey, signerConfig.signer, networkID, logger, signerConfig.libp2pPrivateKey, signerConfig.pssPrivateKey, &node.Options{
...
func (c *command) configureSigner(cmd *cobra.Command, logger log.Logger) (config *signerConfig, err error) {
	...
	if c.config.GetBool(optionNameClefSignerEnable) {
		...
		if err != nil {
		 ...
		 aiscPrivateKey, _, err := keystore.Key(" aisc", password, crypto.EDGSecp256_K1)
		if err != nil {
			return nil, fmt.Errorf(" aisc key: %w", err)
		}
		signer = crypto.NewDefaultSigner( aiscPrivateKey)
		publicKey = & aiscPrivateKey.PublicKey
	}

	logger.Info(" aisc public key", "public_key", hex.EncodeToString(crypto.EncodeSecp256k1PublicKey(publicKey)))
...
func getConfigByNetworkID(networkID uint64, defaultBlockTimeInSeconds uint64) *networkConfig {
	...
	case chaincfg.Mainnet.NetworkID:
		config.bootNodes = []string{"/dnsaddr/mainnet. aisc.org"}
	...
	case chaincfg.Testnet.NetworkID:
		config.bootNodes = []string{"/dnsaddr/testnet. aisc.org"}
