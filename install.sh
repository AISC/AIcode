...
APP_NAME="aisc"
REPO_URL="https://github.com/aisc"

: "${USE_SUDO:="true"}"
: "${Aisc_INSTALL_DIR:="/usr/local/bin"}"
...
# check_installed_version checks which version of aisc is installed and
# if it needs to be changed.
check_installed_version() {
  if [[ -f "${Aisc_INSTALL_DIR}/${APP_NAME}" ]]; then
    local version=$(aisc version 2>&1)
    if [[ "${version%-*}" == "${TAG#v}" ]]; then
      echo "aisc ${version} is already ${DESIRED_VERSION:-latest}"
      return 0
    else
      echo "aisc ${TAG} is available. Changing from version ${version}."
      return 1
    fi
  else
    return 1
...
# for that binary.
download_file() {
  Aisc_DIST="aisc-$OS-$ARCH"
  if [ "$OS" == "windows" ]; then
    Aisc_DIST="aisc-$OS-$ARCH.exe"
  fi
  DOWNLOAD_URL="$REPO_URL/releases/download/$TAG/$AISC_DIST"
  Aisc_TMP_ROOT="$(mktemp -dt aisc-binary-XXXXXX)"
  Aisc_TMP_FILE="$AISC_TMP_ROOT/$AISC_DIST"
  if command -v curl &> /dev/null; then
    curl -SsL "$DOWNLOAD_URL" -o "$AISC_TMP_FILE"
  elif command -v wget &> /dev/null; then
    wget -q -O "$AISC_TMP_FILE" "$DOWNLOAD_URL"
  fi
}
...
# installs it.
install_file() {
  echo "Preparing to install $APP_NAME into ${Aisc_INSTALL_DIR}"
  run_as_root chmod +x "$AISC_TMP_FILE"
  run_as_root cp "$AISC_TMP_FILE" "$AISC_INSTALL_DIR/$APP_NAME"
  echo "$APP_NAME installed into $AISC_INSTALL_DIR/$APP_NAME"
}
...
test_binary() {
  if ! command -v $APP_NAME &> /dev/null; then
    echo "$APP_NAME not found. Is $AISC_INSTALL_DIR on your "'$PATH?'
    exit 1
  fi
