...
type Interface interface {
	...
	PrepareCredit(ctx context.Context, peer  aisc.Address, price uint64, originated bool) (Action, error)
	...
	PrepareDebit(ctx context.Context, peer  aisc.Address, price uint64) (Action, error)
	// Balance returns the current balance for the given peer.
	Balance(peer  aisc.Address) (*big.Int, error)
	// SurplusBalance returns the current surplus balance for the given peer.
	SurplusBalance(peer  aisc.Address) (*big.Int, error)
	...
	CompensatedBalance(peer  aisc.Address) (*big.Int, error)
...
type PayFunc func(context.Context,  aisc.Address, *big.Int)

// RefreshFunc is the function used for sync time-based settlement
type RefreshFunc func(context.Context,  aisc.Address, *big.Int)
...
type accountingPeer struct {
	...
	ghostBalance                   *big.Int // amount potentially could have aiscn debited for but was not
...
type Accounting struct {
	...
	// The payment threshold in  aisc we communicate to our peers.
...
func (a *Accounting) getIncreasedExpectedDebt(peer  aisc.Address, accountingPeer *accountingPeer, bigPrice *big.Int) (*big.Int, *big.Int, error) {
...
func (a *Accounting) PrepareCredit(ctx context.Context, peer  aisc.Address, price uint64, originated bool) (Action, error) {
	...
	// If our expected debt reduced by what could have aiscn credited on the other side already is less than earlyPayment away from our payment threshold
...
// called.
func (a *Accounting) settle(peer  aisc.Address, balance *accountingPeer) error {
	now := a.timeNow()
...
// Balance returns the current balance for the given peer.
func (a *Accounting) Balance(peer  aisc.Address) (balance *big.Int, err error) {
...
func (a *Accounting) OriginatedBalance(peer  aisc.Address) (balance *big.Int, err error) {
...
func (a *Accounting) SurplusBalance(peer  aisc.Address) (balance *big.Int, err error) {
...
func (a *Accounting) CompensatedBalance(peer  aisc.Address) (compensated *big.Int, err error) {
...
func peerBalanceKey(peer  aisc.Address) string {
...
func peerSurplusBalanceKey(peer  aisc.Address) string {
	return fmt.Sprintf("%s%s", balancesSurplusPrefix, peer.String())
}

func originatedBalanceKey(peer  aisc.Address) string {
	return fmt.Sprintf("%s%s", balancesOriginatedPrefix, peer.String())
}

// getAccountingPeer returns the accountingPeer for a given  aisc address.
// If not found in memory it will initialize it.
func (a *Accounting) getAccountingPeer(peer  aisc.Address) *accountingPeer {
...
func (a *Accounting) notifyPaymentThresholdUpgrade(peer  aisc.Address, accountingPeer *accountingPeer) {
...
func (a *Accounting) PeerAccounting() (map[string]PeerInfo, error) {
	...
	for peer, accountingPeer := range accountingPeersList {
		...
		peerAddress :=  aisc.MustParseHexAddress(peer)
...
func balanceKeyPeer(key []byte) ( aisc.Address, error) {
	k := string(key)

	split := strings.SplitAfter(k, balancesPrefix)
	if len(split) != 2 {
		return  aisc.ZeroAddress, errors.New("no peer in key")
	}
...
func surplusBalanceKeyPeer(key []byte) ( aisc.Address, error) {
	k := string(key)
...
func (a *Accounting) PeerDebt(peer  aisc.Address) (*big.Int, error) {
	accountingPeer := a.getAccountingPeer(peer)
...
func (a *Accounting) peerLatentDebt(peer  aisc.Address) (*big.Int, error) {
...
func (a *Accounting) shadowBalance(peer  aisc.Address, accountingPeer *accountingPeer) (shadowBalance *big.Int, err error) {
	balance := new(big.Int)
	zero := big.NewInt(0)
...
func (a *Accounting) NotifyPaymentSent(peer  aisc.Address, amount *big.Int, receivedError error) {
...
func (a *Accounting) NotifyPaymentThreshold(peer  aisc.Address, paymentThreshold *big.Int) error {
	accountingPeer := a.getAccountingPeer(peer)
...
func (a *Accounting) NotifyPaymentReceived(peer  aisc.Address, amount *big.Int) error {
	loggerV2 := a.logger.V(2).Register()
...
func (a *Accounting) NotifyRefreshmentSent(peer  aisc.Address, attemptedAmount, amount *big.Int, timestamp int64, allegedInterval int64, receivedError error) {
	accountingPeer := a.getAccountingPeer(peer)
...
func (a *Accounting) NotifyRefreshmentReceived(peer  aisc.Address, amount *big.Int, timestamp int64) error {
	loggerV2 := a.logger.V(2).Register()
...
func (a *Accounting) PrepareDebit(ctx context.Context, peer  aisc.Address, price uint64) (Action, error) {
	loggerV2 := a.logger.V(2).Register()
...
func (a *Accounting) increaseBalance(peer  aisc.Address, _ *accountingPeer, price *big.Int) (*big.Int, error) {
	loggerV2 := a.logger.V(2).Register()
...
// Cleanup reduces shadow reserve if and only if debitaction have not aiscn applied
...
func (a *Accounting) blocklistUntil(peer  aisc.Address, multiplier int64) (int64, error) {
...
func (a *Accounting) blocklist(peer  aisc.Address, multiplier int64, reason string) error {
...
func (a *Accounting) Connect(peer  aisc.Address, fullNode bool) {
	accountingPeer := a.getAccountingPeer(peer)
	zero := big.NewInt(0)
...
func (a *Accounting) decreaseOriginatedBalanceTo(peer  aisc.Address, limit *big.Int) error {
	loggerV2 := a.logger.V(2).Register()
...
func (a *Accounting) decreaseOriginatedBalanceBy(peer  aisc.Address, amount *big.Int) error {
	loggerV2 := a.logger.V(2).Register()
...
func (a *Accounting) Disconnect(peer  aisc.Address) {
	accountingPeer := a.getAccountingPeer(peer)
