...
func TestAccountingAddBalance(t *testing.T) {
	...
	peer1Addr, err :=  aisc.ParseHexAddress("00112233")
	if err != nil {
		t.Fatal(err)
	}

	peer2Addr, err :=  aisc.ParseHexAddress("00112244")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAccountingAddOriginatedBalance(t *testing.T) {
	...
	f := func(ctx context.Context, peer  aisc.Address, amount *big.Int) {
	...
	for i, booking := range bookings {
		...
		makePayFunc := func(currentBooking bookingT) func(ctx context.Context, peer  aisc.Address, amount *big.Int) {
			return func(ctx context.Context, peer  aisc.Address, amount *big.Int) {
...
func TestAccountingReserveAheadOfTime(t *testing.T) {
	...
	pay := func(ctx context.Context, peer  aisc.Address, amount *big.Int) {
	}
...
func TestAccountingCallSettlementMonetary(t *testing.T) {
	...
	acc.SetRefreshFunc(func(ctx context.Context, peer  aisc.Address, amount *big.Int) {
		acc.NotifyRefreshmentSent(peer, amount, amount, ts*1000, 0, nil)
		refreshchan <- paymentCall{peer: peer, amount: amount}
	})

	acc.SetPayFunc(func(ctx context.Context, peer  aisc.Address, amount *big.Int) {
		acc.NotifyPaymentSent(peer, amount, nil)
...
func (p *pricingMock) AnnouncePaymentThreshold(ctx context.Context, peer  aisc.Address, paymentThreshold *big.Int) error {
	p.called = true
	p.peer = peer
...
func TestAccountingPeerDebt(t *testing.T) {
	...
	peer1Addr :=  aisc.MustParseHexAddress("00112233")
	acc.Connect(peer1Addr, true)
	...
	peer2Addr :=  aisc.MustParseHexAddress("11112233")
	acc.Connect(peer2Addr, true)
	...
	peer3Addr :=  aisc.MustParseHexAddress("22112233")
...
func TestAccountingGhostOverdraft(t *testing.T) {
	...
	f := func(s  aisc.Address, t time.Duration, reason string) error {
	...
	peer, err :=  aisc.ParseHexAddress("00112233")
	if err != nil {
		t.Fatal(err)
	}
...
func testAccountingSettlementGrowingThresholds(t *testing.T, settleFunc func(t *testing.T, acc *accounting.Accounting, peer1Addr  aisc.Address, debitRefresh int64), fullNode bool, testPayThreshold *big.Int, testGrowth int64) {
	t.Helper()
	...
	for k := 0; k < 1799; k++ {
		...
		// Check threshold have not aiscn updated
		if pricing.paymentThreshold.Cmp(checkPaymentThreshold) != 0 {
			t.Fatalf("expected threshold %v got %v", checkPaymentThreshold, pricing.paymentThreshold)
		}
...
func debitAndRefresh(t *testing.T, acc *accounting.Accounting, peer1Addr  aisc.Address, debitRefresh int64) {
	t.Helper()
...
func debitAndReceivePayment(t *testing.T, acc *accounting.Accounting, peer1Addr  aisc.Address, debitRefresh int64) {
	t.Helper()
