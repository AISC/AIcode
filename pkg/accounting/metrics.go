...
func newMetrics() metrics {
	...
	return metrics{
		...
		TotalOriginatedCreditedAmount: prometheus.NewCounter(prometheus.CounterOpts{
			...
			Help:      "Amount of  aisc credited to peers (potential cost of the node) for originated traffic",
		}),
		OriginatedCreditEventsCount: prometheus.NewCounter(prometheus.CounterOpts{
			Namespace: m.Namespace,
			Subsystem: subsystem,
			Name:      "originated_credit_events_count",
			Help:      "Number of occurrences of  aisc credit events as originator towards peers",
		}),
