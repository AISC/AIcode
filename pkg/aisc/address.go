...
// Package aisc exposes the data structure and operations
// necessary on the aisc.Address type which used in the handshake
...
// Address represents the aisc address in  aisc.
...
func NewAddress(signer crypto.Signer, underlay ma.Multiaddr, overlay  aisc.Address, networkID uint64, nonce []byte) (*Address, error) {
...
func ParseAddress(underlay, overlay, signature, nonce []byte, validateOverlay bool, networkID uint64) (*Address, error) {
	...
	return &Address{
		...
		Overlay:          aisc.NewAddress(overlay),
		Signature:       signature,
		Nonce:           nonce,
...
func generateSignData(underlay, overlay []byte, networkID uint64) []byte {
	...
	signData := append([]byte("aisc-handshake-"), underlay...)
...
func (a *Address) UnmarshalJSON(b []byte) error {
	...
	addr, err :=  aisc.ParseHexAddress(v.Overlay)
	if err != nil {
		return err
	}
...
// ShortString returns shortened versions of aisc address in a format: [Overlay, Underlay]
