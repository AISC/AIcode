...
// Package api provides the functionality of the Aisc
...
import (
	...
	"github.com/aisc/pkg/resolver/client/ens"
	"github.com/aisc/pkg/sctx"
	"github.com/aisc/pkg/settlement"
...
const (
	AiscPinHeader                    = "Aisc-Pin"
	AiscTagHeader                    = "Aisc-Tag"
	AiscEncryptHeader                = "Aisc-Encrypt"
	AiscIndexDocumentHeader          = "Aisc-Index-Document"
	AiscErrorDocumentHeader          = "Aisc-Error-Document"
	AiscFeedIndexHeader              = "Aisc-Feed-Index"
	AiscFeedIndexNextHeader          = "Aisc-Feed-Index-Next"
	AiscCollectionHeader             = "Aisc-Collection"
	AiscPostageBatchIdHeader         = "Aisc-Postage-Batch-Id"
	AiscDeferredUploadHeader         = "Aisc-Deferred-Upload"
	AiscRedundancyLevelHeader        = "Aisc-Redundancy-Level"
	AiscRedundancyStrategyHeader     = "Aisc-Redundancy-Strategy"
	AiscRedundancyFallbackModeHeader = "Aisc-Redundancy-Fallback-Mode"
	AiscChunkRetrievalTimeoutHeader  = "Aisc-Chunk-Retrieval-Timeout"
	AiscLookAheadBufferSizeHeader    = "Aisc-Lookahead-Buffer-Size"
...
var (
	errInvalidNameOrAddress             = errors.New("invalid name or aisc address")
...
func (s *Service) SetAiscAddress(addr * aisc.Address) {
	if s != nil {
		s.overlay = addr
	}
}
...
func (s *Service) resolveNameOrAddress(str string) ( aisc.Address, error) {
	// Try and mapStructure the name as a aisc address.
	addr, err :=  aisc.ParseHexAddress(str)
	if err == nil {
		s.loggerV1.Debug("resolve name: parsing aisc address successful", "string", str, "address", addr)
		return addr, nil
	}

	// If no resolver is not available, return an error.
	if s.resolver == nil {
		return  aisc.ZeroAddress, errNoResolver
	}
	...
	return  aisc.ZeroAddress, fmt.Errorf("%w: %w", errInvalidNameOrAddress, err)
}
...
func (s *Service) corsHandler(h http.Handler) http.Handler {
	allowedHeaders := []string{
		...
		AiscTagHeader, AiscPinHeader, AiscEncryptHeader, AiscIndexDocumentHeader, AiscErrorDocumentHeader, AiscCollectionHeader, AiscPostageBatchIdHeader, AiscDeferredUploadHeader, AiscRedundancyLevelHeader, AiscRedundancyStrategyHeader, AiscRedundancyFallbackModeHeader, AiscChunkRetrievalTimeoutHeader, AiscLookAheadBufferSizeHeader, AiscFeedIndexHeader, AiscFeedIndexNextHeader, GasPriceHeader, GasLimitHeader, ImmutableHeader,
	}
...
func (p *putterSessionWrapper) Put(ctx context.Context, chunk  aisc.Chunk) error {
...
func (p *putterSessionWrapper) Done(ref  aisc.Address) error {
...
func (s *Service) newStamperPutter(ctx context.Context, opts putterOptions) (storer.PutterSession, error) {
	if !opts.Deferred && s.aiscMode == DevMode {
...
type pipelineFunc func(context.Context, io.Reader) ( aisc.Address, error)

func requestPipelineFn(s storage.Putter, encrypt bool, rLevel redundancy.Level) pipelineFunc {
	return func(ctx context.Context, r io.Reader) ( aisc.Address, error) {
...
func CalculateNumberOfChunks(contentLength int64, isEncrypted bool) int64 {
	if contentLength <=  aisc.ChunkSize {
		return 1
	}
	branchingFactor :=  aisc.Branches
	if isEncrypted {
		branchingFactor =  aisc.EncryptedBranches
	}

	dataChunks := math.Ceil(float64(contentLength) / float64( aisc.ChunkSize))
