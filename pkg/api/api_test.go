...
import (
	...
	mockauth "github.com/aisc/pkg/auth/mock"
	...
	contractMock "github.com/aisc/pkg/postage/postagecontract/mock"
	...
	chequebookmock "github.com/aisc/pkg/settlement/swap/chequebook/mock"
	"github.com/aisc/pkg/settlement/swap/erc20"
	erc20mock "github.com/aisc/pkg/settlement/swap/erc20/mock"
	swapmock "github.com/aisc/pkg/settlement/swap/mock"
	"github.com/aisc/pkg/spinlock"
	statestore "github.com/aisc/pkg/statestore/mock"
	...
	mock2 "github.com/aisc/pkg/storageincentives/staking/mock"
	...
	transactionmock "github.com/aisc/pkg/transaction/mock"
...
func newTestServer(t *testing.T, o testServerOptions) (*http.Client, *websocket.Conn, string, *chanStorer) {
	...
	// By default aisc mode is set to full mode.
	if o.AiscMode == api.UnknownMode {
		o.AiscMode = api.FullMode
	}

	s := api.New(o.PublicKey, o.PSSPublicKey, o.EthereumAddress, o.Logger, transaction, o.BatchStore, o.AiscMode, true, true, backend, o.CORSAllowedOrigins, inmemstore.New())
...
func TestParseName(t *testing.T) {
	...
	const aiscHash = "89c17d0d8018a19057314aa035e61c9d23c47581a61dd3a79a7839692c617e4d"
	log := log.Noop

	var errInvalidNameOrAddress = errors.New("invalid name or aisc address")
	...
	testCases := []struct {
		...
		noResolver bool
		wantAdr     aisc.Address
		wantErr    error
	}{
		{
			desc:    "empty name",
			name:    "",
			wantAdr:  aisc.ZeroAddress,
		},
		{
			desc:    "aisc hash",
			name:    aiscHash,
			wantAdr:  aisc.MustParseHexAddress(aiscHash),
		},
		{
			desc:       "no resolver connected with aisc hash",
			name:       aiscHash,
			noResolver: true,
			wantAdr:     aisc.MustParseHexAddress(aiscHash),
		},
		...
		},
		{
			...
			res: resolverMock.NewResolver(
				resolverMock.WithResolveFunc(func(string) ( aisc.Address, error) {
					return  aisc.ZeroAddress, errInvalidNameOrAddress
				}),
			),
		...
		},
		{
			...
			wantAdr:  aisc.MustParseHexAddress("89c17d0d8018a19057314aa035e61c9d23c47581a61dd3a79a7839692c617e4d"),
		},
	}
	...
	for _, endpoint := range endpoints {
		...
		t.Run(endpoint+": empty batch", func(t *testing.T) {
			...
			jsonhttptest.Request(t, client, http.MethodPost, "/"+endpoint, expCode,
				jsonhttptest.WithRequestHeader(api.AiscPostageBatchIdHeader, hexbatch),
...
func TestPostageDirectAndDeferred(t *testing.T) {
	...
	for _, endpoint := range []string{"bytes", "aisc", "chunks"} {
		endpoint := endpoint
...
func (c *chanStorer) Has(addr  aisc.Address) bool {
	c.lock.Lock()
