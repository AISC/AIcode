...
func TestBytes(t *testing.T) {
	...
	content, err := g.SequentialBytes( aisc.ChunkSize * 2)
	if err != nil {
		t.Fatal(err)
	}

	t.Run("upload", func(t *testing.T) {
		chunkAddr :=  aisc.MustParseHexAddress(expHash)
	...
	t.Run("not found", func(t *testing.T) {
		jsonhttptest.Request(t, client, http.MethodGet, resource+"/"+ aisc.EmptyAddress.String(), http.StatusNotFound)
	})
}
...
func TestBytesInvalidStamp(t *testing.T) {
	...
	t.Run("upload tag not found", func(t *testing.T) {
		...
		jsonhttptest.Request(t, clientTagExists, http.MethodPost, resource, http.StatusNotFound,
			jsonhttptest.WithRequestHeader(api.AiscTagHeader, strconv.FormatUint(invalidTag, 10)),
			jsonhttptest.WithRequestHeader(api.AiscDeferredUploadHeader, "true"),
			jsonhttptest.WithRequestHeader(api.AiscPostageBatchIdHeader, batchOkStr),
			jsonhttptest.WithRequestBody(bytes.NewReader(content)),
		)
...
func TestBytesUploadHandlerInvalidInputs(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "invalid stamp",
			hdrKey: api.AiscPostageBatchIdHeader,
			hdrVal: batchOkStr,
			want: jsonhttp.StatusResponse{
