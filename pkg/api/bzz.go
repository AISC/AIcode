...
import (
	...
	"github.com/aisc/pkg/file/redundancy/getter"
...
func (s *Service) aiscUploadHandler(w http.ResponseWriter, r *http.Request) {
	span, logger, ctx := s.tracer.StartSpanFromContext(r.Context(), "post_aisc", s.logger.WithName("post_aisc").Build())
	defer span.Finish()
	...
	headers := struct {
		...
		IsDir       bool             `map:"Aisc-Collection"`
...
type aiscUploadResponse struct {
...
func (s *Service) fileUploadHandler(
	...
	err = m.Add(ctx, manifest.RootPath, manifest.NewEntry( aisc.ZeroAddress, rootMetadata))
	if err != nil {
	...
	fileMtdt := map[string]string{
		manifest.EntryMetadataContentTypeKey: r.Header.Get(ContentTypeHeader), // Content-Type has already aiscn validated.
	...
	if tagID != 0 {
		w.Header().Set(AiscTagHeader, fmt.Sprint(tagID))
	...
	jsonhttp.Created(w, aiscUploadResponse{
		Reference: manifestReference,
	})
}

func (s *Service) aiscDownloadHandler(w http.ResponseWriter, r *http.Request) {
	logger := tracing.NewLoggerWithTraceID(r.Context(), s.logger.WithName("get_aisc_by_path").Build())
...
func (s *Service) aiscHeadHandler(w http.ResponseWriter, r *http.Request) {
	logger := tracing.NewLoggerWithTraceID(r.Context(), s.logger.WithName("head_aisc_by_path").Build())
...
func (s *Service) serveReference(logger log.Logger, address  aisc.Address, pathVar string, w http.ResponseWriter, r *http.Request, headerOnly bool) {
	loggerV1 := logger.V(1).Build()

	headers := struct {
		Cache                 *bool            `map:"Aisc-Cache"`
		Strategy              *getter.Strategy `map:"Aisc-Redundancy-Strategy"`
		FallbackMode          *bool            `map:"Aisc-Redundancy-Fallback-Mode"`
		ChunkRetrievalTimeout *string          `map:"Aisc-Chunk-Retrieval-Timeout"`
	}{}
...
FETCH:
	...
	if err != nil {
		logger.Debug("aisc download: not manifest", "address", address, "error", err)
	...
	if !feedDereferenced {
		if l, err := s.manifestFeed(ctx, m); err == nil {
			...
			if err != nil {
				logger.Debug("aisc download: feed lookup failed", "error", err)
				logger.Error(nil, "aisc download: feed lookup failed")
				jsonhttp.NotFound(w, "feed not found")
				return
			}
			if ch == nil {
				logger.Debug("aisc download: feed lookup: no updates")
				logger.Error(nil, "aisc download: feed lookup")
				jsonhttp.NotFound(w, "no update found")
				return
			}
			ref, _, err := parseFeedUpdate(ch)
			if err != nil {
				logger.Debug("aisc download: mapStructure feed update failed", "error", err)
				logger.Error(nil, "aisc download: mapStructure feed update failed")
			...
			if err != nil {
				s.logger.Debug("aisc download: marshal feed index failed", "error", err)
				s.logger.Error(nil, "aisc download: marshal index failed")
				jsonhttp.InternalServerError(w, "marshal index")
				return
			}

			w.Header().Set(AiscFeedIndexHeader, hex.EncodeToString(curBytes))
			...
			w.Header().Set("Access-Control-Expose-Headers", AiscFeedIndexHeader)
			goto FETCH
		}
	}

	if pathVar == "" {
		loggerV1.Debug("aisc download: handle empty path", "address", address)
		...
		if indexDocumentSuffixKey, ok := manifestMetadataLoad(ctx, m, manifest.RootPath, manifest.WebsiteIndexDocumentSuffixKey); ok {
			...
			if err == nil {
				...
				logger.Debug("aisc download: serving path", "path", pathWithIndex)

				s.serveManifestEntry(logger, w, r, indexDocumentManifestEntry, !feedDereferenced, headerOnly)
				return
			}
		}
		logger.Debug("aisc download: address not found or incorrect", "address", address, "path", pathVar)
	...
	if err != nil {
		loggerV1.Debug("aisc download: invalid path", "address", address, "path", pathVar, "error", err)
		logger.Error(nil, "aisc download: invalid path")
		...
		if errors.Is(err, manifest.ErrNotFound) {
			...
			if !strings.HasPrefix(pathVar, "/") {
				...
				if err == nil && exists {
					...
					logger.Debug("aisc download: redirecting failed", "url", redirectURL, "error", err)
			...
			if errorDocumentPath, ok := manifestMetadataLoad(ctx, m, manifest.RootPath, manifest.WebsiteErrorDocumentPathKey); ok {
				if pathVar != errorDocumentPath {
					...
					if err == nil {
						...
						logger.Debug("aisc download: serving path", "path", errorDocumentPath)
...
// downloadHandler contains common logic for dowloading Aisc file from API
func (s *Service) downloadHandler(logger log.Logger, w http.ResponseWriter, r *http.Request, reference  aisc.Address, additionalHeaders http.Header, etag, headersOnly bool) {
	headers := struct {
		Strategy              *getter.Strategy `map:"Aisc-Redundancy-Strategy"`
		FallbackMode          *bool            `map:"Aisc-Redundancy-Fallback-Mode"`
		ChunkRetrievalTimeout *string          `map:"Aisc-Chunk-Retrieval-Timeout"`
		LookaheadBufferSize   *int             `map:"Aisc-Lookahead-Buffer-Size"`
		Cache                 *bool            `map:"Aisc-Cache"`
	}{}
