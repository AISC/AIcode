...
import (
	...
	"github.com/aisc/pkg/util/testutil/pseudorand"
)

// nolint:paralleltest,tparallel,thelper

// Test aiscUploadDownloadWithRedundancy tests the API for upload and download files
// with all combinations of redundancy level, encryption and size (levels, i.e., the
//
//	height of the  aisc hash tree).
//
...
//     During path traversal in the  aisc hash tree, the underlying mocksore (forgetting)
...
//
// nolint:thelper
func Test aiscUploadDownloadWithRedundancy_FLAKY(t *testing.T) {
	t.Parallel()
	fileUploadResource := "/aisc"
	fileDownloadResource := func(addr string) string { return "/aisc/" + addr + "/" }
	...
	testRedundancy := func(t *testing.T, rLevel redundancy.Level, encrypt bool, levels int, chunkCnt int, shardCnt int, parityCnt int) {
		...
		dataReader := pseudorand.NewReader(seed, chunkCnt* aisc.ChunkSize)

		var refResponse api. aiscUploadResponse
		jsonhttptest.Request(t, client, http.MethodPost, fileUploadResource,
			...
			jsonhttptest.WithRequestHeader(api.AiscEncryptHeader, fmt.Sprintf("%t", encrypt)),
			jsonhttptest.WithRequestHeader(api.AiscRedundancyLevelHeader, fmt.Sprintf("%d", rLevel)),
		...
		t.Run("download multiple ranges without redundancy should succeed", func(t *testing.T) {
			...
			respHeaders := jsonhttptest.Request(t, client, http.MethodGet,
				...
				jsonhttptest.WithRequestHeader(api.AiscLookAheadBufferSizeHeader, "0"),
				// set for the replicas so that no replica gets deleted
				jsonhttptest.WithRequestHeader(api.AiscRedundancyLevelHeader, "0"),
				jsonhttptest.WithRequestHeader(api.AiscRedundancyStrategyHeader, "0"),
				jsonhttptest.WithRequestHeader(api.AiscRedundancyFallbackModeHeader, "false"),
		...
		t.Run("download without redundancy should NOT succeed", func(t *testing.T) {
			...
			req.Header.Set(api.AiscRedundancyStrategyHeader, "0")
			req.Header.Set(api.AiscRedundancyFallbackModeHeader, "false")
		...
		t.Run("download with redundancy should succeed", func(t *testing.T) {
			...
			req.Header.Set(api.AiscRedundancyStrategyHeader, "3")
			req.Header.Set(api.AiscRedundancyFallbackModeHeader, "true")
...
func Test aiscFiles(t *testing.T) {
	t.Parallel()
	...
	t.Run("tar-file-upload", func(t *testing.T) {
		...
		address :=  aisc.MustParseHexAddress("f30c0aa7e9e2a0ef4c9b1b750ebfeaeb7c7c24da700bb089da19a46e3677824b")
		rcvdHeader := jsonhttptest.Request(t, client, http.MethodPost, fileUploadResource, http.StatusCreated,
			...
			jsonhttptest.WithNonEmptyResponseHeader(api.AiscTagHeader),
		)
	...
	t.Run("tar-file-upload-with-pinning", func(t *testing.T) {
		...
		reference :=  aisc.MustParseHexAddress("f30c0aa7e9e2a0ef4c9b1b750ebfeaeb7c7c24da700bb089da19a46e3677824b")
		rcvdHeader := jsonhttptest.Request(t, client, http.MethodPost, fileUploadResource, http.StatusCreated,
			jsonhttptest.WithRequestHeader(api.AiscDeferredUploadHeader, "true"),
			jsonhttptest.WithRequestHeader(api.AiscPostageBatchIdHeader, batchOkStr),
	...
	t.Run("redundancy", func(t *testing.T) {
		...
		jsonhttptest.Request(t, client, http.MethodPost, fileUploadResource+"?name="+fileName, http.StatusCreated,
			...
			jsonhttptest.WithRequestBody(bytes.NewReader(simpleData)),
			jsonhttptest.WithRequestHeader(api.AiscEncryptHeader, "True"),
			jsonhttptest.WithRequestHeader(api.AiscRedundancyLevelHeader, "4"),
			jsonhttptest.WithRequestHeader(api.ContentTypeHeader, "image/jpeg; charset=utf-8"),
			jsonhttptest.WithUnmarshalJSONResponse(&resp),
		)
...
func Test aiscFilesRangeRequests(t *testing.T) {
	t.Parallel()
	...
	for _, upload := range uploads {
		...
		t.Run(upload.name, func(t *testing.T) {
			...
			if upload.name == "dir" {
				testOpts = append(testOpts, jsonhttptest.WithRequestHeader(api.AiscCollectionHeader, "True"))
			}
...
func TestFeedIndirection(t *testing.T) {
	...
	var (
		updateData      = []byte("<h1>Aisc Feeds Hello World!</h1>")
		logger          = log.Noop
		storer          = mockstorer.New()
		client, _, _, _ = newTestServer(t, testServerOptions{
			Storer: storer,
	...
	options := []jsonhttptest.Option{
		...
		jsonhttptest.WithUnmarshalJSONResponse(&resp),
		jsonhttptest.WithRequestHeader(api.AiscIndexDocumentHeader, "index.html"),
	}

	// verify directory tar upload response
	jsonhttptest.Request(t, client, http.MethodPost, "/aisc", http.StatusCreated, options...)
	...
	// called from the aisc endpoint. then call the aisc endpoint with
	...
	var (
		...
		aiscDownloadResource = func(addr, path string) string { return "/aisc/" + addr + "/" + path }
	...
	err = m.Add(ctx, manifest.RootPath, manifest.NewEntry( aisc.NewAddress(emptyAddr), map[string]string{
	...
	jsonhttptest.Request(t, client, http.MethodGet, aiscDownloadResource(manifRef.String(), ""), http.StatusOK,
		jsonhttptest.WithExpectedResponse(updateData),
		jsonhttptest.WithExpectedContentLength(len(updateData)),
	)
}

func Test_aiscDownloadHandler_invalidInputs(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			jsonhttptest.Request(t, client, http.MethodGet, fmt.Sprintf("/aisc/%s/abc", tc.address), tc.want.Code,
...
func TestInvalid aiscParams(t *testing.T) {
	t.Parallel()
	...
	t.Run("upload, invalid tag", func(t *testing.T) {
		...
		jsonhttptest.Request(t, clientInvalidTag, http.MethodPost, fileUploadResource, http.StatusBadRequest,
			jsonhttptest.WithRequestHeader(api.AiscTagHeader, "tag"),
			jsonhttptest.WithRequestHeader(api.AiscDeferredUploadHeader, "true"),
			jsonhttptest.WithRequestHeader(api.AiscPostageBatchIdHeader, batchOkStr),
			jsonhttptest.WithRequestBody(tr),
			jsonhttptest.WithRequestHeader(api.ContentTypeHeader, api.ContentTypeTar))
	...
	t.Run("upload, tag not found", func(t *testing.T) {
		...
		jsonhttptest.Request(t, clientTagExists, http.MethodPost, fileUploadResource, http.StatusNotFound,
			jsonhttptest.WithRequestHeader(api.AiscTagHeader, strconv.FormatUint(10000, 10)),
			jsonhttptest.WithRequestHeader(api.AiscDeferredUploadHeader, "true"),
			jsonhttptest.WithRequestHeader(api.AiscPostageBatchIdHeader, batchOkStr),
			jsonhttptest.WithRequestBody(tr),
			jsonhttptest.WithRequestHeader(api.ContentTypeHeader, api.ContentTypeTar))
	...
	t.Run("address not found", func(t *testing.T) {
		...
		jsonhttptest.Request(t, client, http.MethodGet, fmt.Sprintf("/aisc/%s/", address), http.StatusNotFound)
	})

}

// TestDirectUpload aisc tests that the direct upload endpoint give correct error message in dev mode
func TestDirectUpload aisc(t *testing.T) {
	t.Parallel()
