...
func (s *Service) dirUploadHandler(
	...
	reference, err := storeDir(
		...
		putter,
		s.storer.ChunkStore(),
		r.Header.Get(AiscIndexDocumentHeader),
		r.Header.Get(AiscErrorDocumentHeader),
		rLevel,
	)
	if err != nil {
...
func storeDir(
	...
	if indexFilename != "" && strings.ContainsRune(indexFilename, '/') {
		return  aisc.ZeroAddress, errors.New("index document suffix must not include slash character")
	}
	...
	for {
		...
		if errors.Is(err, io.EOF) {
			...
			return  aisc.ZeroAddress, fmt.Errorf("read dir stream: %w", err)
		}

		fileReference, err := p(ctx, fileInfo.Reader)
		if err != nil {
			return  aisc.ZeroAddress, fmt.Errorf("store dir file: %w", err)
		}
		loggerV1.Debug("aisc upload dir: file dir uploaded", "file_path", fileInfo.Path, "address", fileReference)
		...
		if err != nil {
			return  aisc.ZeroAddress, fmt.Errorf("add to manifest: %w", err)
		}
	...
	if filesAdded == 0 {
		return  aisc.ZeroAddress, errEmptyDir
	}
	...
	if indexFilename != "" || errorFilename != "" {
		...
		rootManifestEntry := manifest.NewEntry( aisc.ZeroAddress, metadata)
	...
	if err != nil {
		return  aisc.ZeroAddress, fmt.Errorf("store manifest: %w", err)
	}
	loggerV1.Debug("aisc upload dir: uploaded dir finished", "address", manifestReference)
...
func (t *tarReader) Next() (*FileInfo, error) {
	for {
		...
		if !fileHeader.FileInfo().Mode().IsRegular() {
			t.logger.Warning("aisc upload dir: skipping file upload as it is not a regular file", "file_path", filePath)
			continue
		}
