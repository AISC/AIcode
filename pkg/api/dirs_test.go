...
func TestDirs(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			expectedReference:  aisc.MustParseHexAddress("f3312af64715d26b5e1a3dc90f012d2c9cc74a167899dab1d07cdee8c107f939"),
			files: []f{
				{
		...
		},
		{
			...
			expectedReference:  aisc.MustParseHexAddress("9e178dbd1ed4b748379e25144e28dfb29c07a4b5114896ef454480115a56b237"),
			doMultipart:       true,
			files: []f{
				{
					data: []byte("<h1>Aisc"),
		...
		},
		{
			...
			expectedReference:    aisc.MustParseHexAddress("a58484e3d77bbdb40323ddc9020c6e96e5eb5deb52015d3e0f63cce629ac1aa6"),
		...
		},
		{
			...
			expectedReference:    aisc.MustParseHexAddress("3e2f008a578c435efa7a1fce146e21c4ae8c20b80fbb4c4e0c1c87ca08fef414"),
			wantIndexFilename:   "index.html",
			indexFilenameOption: jsonhttptest.WithRequestHeader(api.AiscIndexDocumentHeader, "index.html"),
			files: []f{
				{
		...
		},
		{
			...
			expectedReference:  aisc.MustParseHexAddress("133c92414c047708f3d6a8561571a0cc96512899ff0edbd9690c857f01ab6883"),
			files: []f{
				{
					data:     []byte("<h1>Aisc"),
					name:     "index.html",
		...
		verify := func(t *testing.T, resp api. aiscUploadResponse) {
			t.Helper()
			...
			validateFile := func(t *testing.T, file f, filePath string) {
				...
				jsonhttptest.Request(t, client, http.MethodGet,
					aiscDownloadResource(resp.Reference.String(), filePath),
					http.StatusOK,
			...
			validateIsPermanentRedirect := func(t *testing.T, fromPath, toPath string) {
				...
				expectedResponse := fmt.Sprintf("<a href=\"%s\">Permanent Redirect</a>.\n\n",
					aiscDownloadResource(resp.Reference.String(), toPath))

				jsonhttptest.Request(t, client, http.MethodGet,
					aiscDownloadResource(resp.Reference.String(), fromPath),
			...
			validateAltPath := func(t *testing.T, fromPath, toPath string) {
				...
				jsonhttptest.Request(t, client, http.MethodGet,
					aiscDownloadResource(resp.Reference.String(), toPath), http.StatusOK,
					jsonhttptest.WithPutResponseBody(&respBytes),
				)

				jsonhttptest.Request(t, client, http.MethodGet,
					aiscDownloadResource(resp.Reference.String(), fromPath), http.StatusOK,
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Run("tar_upload", func(t *testing.T) {
				...
				if tc.encrypt {
					options = append(options, jsonhttptest.WithRequestHeader(api.AiscEncryptHeader, "true"))
				}
	...
	t.Run("upload tag not found", func(t *testing.T) {
		...
		jsonhttptest.Request(t, client, http.MethodPost, dirUploadResource, http.StatusNotFound,
			jsonhttptest.WithRequestHeader(api.AiscTagHeader, strconv.FormatUint(uint64(10000), 10)),
			jsonhttptest.WithRequestHeader(api.AiscDeferredUploadHeader, "true"),
			jsonhttptest.WithRequestHeader(api.AiscPostageBatchIdHeader, batchOkStr),
			jsonhttptest.WithRequestBody(tr),
			jsonhttptest.WithRequestHeader(api.ContentTypeHeader, api.ContentTypeTar))
