...
const (
	feedMetadataEntryOwner = " aisc-feed-owner"
	feedMetadataEntryTopic = " aisc-feed-topic"
	feedMetadataEntryType  = " aisc-feed-type"
)
...
func (s *Service) feedGetHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set(AiscFeedIndexNextHeader, hex.EncodeToString(nextBytes))
	w.Header().Set("Access-Control-Expose-Headers", fmt.Sprintf("%s, %s", AiscFeedIndexHeader, AiscFeedIndexNextHeader))
...
func (s *Service) feedPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = feedManifest.Add(r.Context(), "/", manifest.NewEntry( aisc.NewAddress(emptyAddr), meta))
	if err != nil {
...
func parseFeedUpdate(ch  aisc.Chunk) ( aisc.Address, int64, error) {
	s, err := soc.FromChunk(ch)
	if err != nil {
		return  aisc.ZeroAddress, 0, fmt.Errorf("soc unmarshal: %w", err)
	}
	...
	if len(update) != 48 && len(update) != 80 {
		return  aisc.ZeroAddress, 0, errInvalidFeedUpdate
	}
	ts := binary.BigEndian.Uint64(update[8:16])
	ref :=  aisc.NewAddress(update[16:])
