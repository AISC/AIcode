...
var expReference =  aisc.MustParseHexAddress("891a1d1c8436c792d02fc2e8883fef7ab387eaeaacd25aa9f518be7be7856d54")
...
func TestFeed_Get(t *testing.T) {
	...
	t.Run("with at", func(t *testing.T) {
		...
		jsonhttptest.Request(t, client, http.MethodGet, feedResource(ownerString, "aabbcc", "12"), http.StatusOK,
			...
			jsonhttptest.WithExpectedResponseHeader(api.AiscFeedIndexHeader, hex.EncodeToString(idBytes)),
		)
	})
...
func newMockLookup(at int64, after uint64, ch  aisc.Chunk, err error, cur, next feeds.Index) *mockLookup {
	return &mockLookup{at: at, after: after, chunk: ch, err: err, cur: cur, next: next}
}

func (l *mockLookup) At(_ context.Context, at int64, after uint64) ( aisc.Chunk, feeds.Index, feeds.Index, error) {
	if l.at == -1 {
...
func toChunk(t *testing.T, at uint64, payload []byte)  aisc.Chunk {
	t.Helper()
