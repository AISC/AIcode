...
func (s *Service) peerConnectHandler(w http.ResponseWriter, r *http.Request) {
	...
	aiscAddr, err := s.p2p.Connect(r.Context(), paths.MultiAddress)
	if err != nil {
	...
	if err := s.topologyDriver.Connected(r.Context(), p2p.Peer{Address: aiscAddr.Overlay}, true); err != nil {
		_ = s.p2p.Disconnect(aiscAddr.Overlay, "failed to notify topology")
	...
	jsonhttp.OK(w, peerConnectResponse{
		Address: aiscAddr.Overlay.String(),
	})
}
