...
func TestPssWebsocketSingleHandler(t *testing.T) {
	...
	cl.SetReadLimit( aisc.ChunkSize)
...
func TestPssSend(t *testing.T) {
	var (
		...
		sendFn = func(ctx context.Context, targets pss.Targets, chunk  aisc.Chunk) error {
			mtx.Lock()
		...
		jsonhttptest.Request(t, client, http.MethodPost, "/pss/send/to/12", http.StatusBadRequest,
			...
			jsonhttptest.WithExpectedJSONResponse(jsonhttp.StatusResponse{
				...
				Reasons: []jsonhttp.Reason{
					{
						Field: api.AiscPostageBatchIdHeader,
...
type pssSendFn func(context.Context, pss.Targets,  aisc.Chunk) error
type mpss struct {
	f pssSendFn
}
...
func (m *mpss) TryUnwrap(_  aisc.Chunk) {
	panic("not implemented") // TODO: Implement
}
