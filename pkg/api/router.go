...
import (
	...
	"github.com/aisc/pkg/log/httpaccess"
...
func (s *Service) mountAPI() {
	subdomainRouter := s.router.Host("{subdomain:.*}. aisc.localhost").Subrouter()
	...
	s.router.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprintln(w, "Ethereum Aisc Aisc")
	})
	...
	handle("/chunks", jsonhttp.MethodHandler{
		"POST": web.ChainHandlers(
			jsonhttp.NewMaxBodyBytesHandler( aisc.SocMaxChunkSize),
	...
	handle("/soc/{owner}/{id}", jsonhttp.MethodHandler{
		"POST": web.ChainHandlers(
			jsonhttp.NewMaxBodyBytesHandler( aisc.ChunkWithSpanSize),
	...
	handle("/aisc", jsonhttp.MethodHandler{
		"POST": web.ChainHandlers(
			s.contentLengthMetricMiddleware(),
			s.newTracingHandler("aisc-upload"),
			web.FinalHandlerFunc(s.aiscUploadHandler),
		),
	})

	handle("/aisc/{address}", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		u := r.URL
		u.Path += "/"
		http.Redirect(w, r, u.String(), http.StatusPermanentRedirect)
	}))

	handle("/aisc/{address}/{path:.*}", jsonhttp.MethodHandler{
		"GET": web.ChainHandlers(
			s.contentLengthMetricMiddleware(),
			s.newTracingHandler("aisc-download"),
			web.FinalHandlerFunc(s.aiscDownloadHandler),
		),
		"HEAD": web.ChainHandlers(
			web.FinalHandlerFunc(s.aiscHeadHandler),
		),
	})
	...
	handle("/pss/send/{topic}/{targets}", web.ChainHandlers(
		web.FinalHandler(jsonhttp.MethodHandler{
			"POST": web.ChainHandlers(
				jsonhttp.NewMaxBodyBytesHandler( aisc.ChunkSize),
