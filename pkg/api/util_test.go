...
type (
	...
	mapAiscAddressTest struct {
		AiscAddressVal  aisc.Address `map:" aiscAddressVal"`
	}
)
...
func TestMapStructure(t *testing.T) {
	...
	tests := []struct {
		...
		name: " aisc.Address value",
		src:  map[string]string{" aiscAddressVal": "1234567890abcdef"},
		want: &mapAiscAddressTest{AiscAddressVal:  aisc.MustParseHexAddress("1234567890abcdef")},
	}}
