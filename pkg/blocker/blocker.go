...
type Blocker struct {
	...
	blocklistCallback func( aisc.Address)
}

func New(blocklister p2p.Blocklister, flagTimeout, blockDuration, wakeUpTime time.Duration, callback func( aisc.Address), logger log.Logger) *Blocker {
...
func (b *Blocker) Flag(addr  aisc.Address) {
	if b.blocklister.NetworkStatus() != p2p.NetworkStatusAvailable {
		return
	}
...
func (b *Blocker) Unflag(addr  aisc.Address) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *Blocker) PruneUnseen(seen [] aisc.Address) {
