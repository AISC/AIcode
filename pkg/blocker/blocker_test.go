...
func TestBlocksAfterFlagTimeout(t *testing.T) {
	...
	blockedC := make(chan  aisc.Address, 10)

	mock := mockBlockLister(func(a  aisc.Address, d time.Duration, r string) error {
		blockedC <- a
...
func mockBlockLister(f func( aisc.Address, time.Duration, string) error) *blocklister {
	return &blocklister{
		blocklistFunc: f,
	}
}

func (b *blocklister) Blocklist(addr  aisc.Address, t time.Duration, r string) error {
