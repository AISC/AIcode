...
import (
	...
	"github.com/aisc/pkg/bmt/reference"
...
var (
	...
	hashSize          =  aisc.NewHasher().Size()
	seed              = time.Now().Unix()
)

func refHash(count int, data []byte) ([]byte, error) {
	rbmt := reference.NewRefHasher( aisc.NewHasher(), count)
...
func TestHasherEmptyData(t *testing.T) {
	...
	for _, count := range testSegmentCounts {
		...
		t.Run(fmt.Sprintf("%d_segments", count), func(t *testing.T) {
			...
			pool := bmt.NewPool(bmt.NewConf( aisc.NewHasher, count, 1))
			h := pool.Get()
...
func TestSyncHasherCorrectness(t *testing.T) {
	...
	for _, count := range testSegmentCounts {
		...
		t.Run(fmt.Sprintf("segments_%v", count), func(t *testing.T) {
			...
			var incr int
			capacity := 1
			pool := bmt.NewPool(bmt.NewConf( aisc.NewHasher, count, capacity))
...
func testHasherReuse(t *testing.T, poolsize int) {
	...
	pool := bmt.NewPool(bmt.NewConf( aisc.NewHasher, testSegmentCount, poolsize))
	h := pool.Get()
	defer pool.Put(h)
...
func TestBMTConcurrentUse(t *testing.T) {
	...
	testData := testutil.RandBytesWithSeed(t, 4096, seed)
	pool := bmt.NewPool(bmt.NewConf( aisc.NewHasher, testSegmentCount, testPoolSize))
	cycles := 100
...
func TestBMTWriterBuffers(t *testing.T) {
	...
	for i, count := range testSegmentCounts {
		...
		t.Run(fmt.Sprintf("%d_segments", count), func(t *testing.T) {
			...
			pool := bmt.NewPool(bmt.NewConf( aisc.NewHasher, count, testPoolSize))
			h := pool.Get()
			defer pool.Put(h)
