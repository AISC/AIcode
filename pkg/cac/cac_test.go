...
func TestNew(t *testing.T) {
	...
	address :=  aisc.MustParseHexAddress(bmtHashOfData)
...
func TestChunkInvariantsNew(t *testing.T) {
	...
	for _, cc := range []struct {
		...
		},
		{
			...
			data:    testutil.RandBytes(t,  aisc.ChunkSize+1),
			wantErr: cac.ErrChunkDataLarge,
		},
		{
			name:    "ok",
			data:    testutil.RandBytes(t, rand.Intn( aisc.ChunkSize)+1),
			wantErr: nil,
		},
	} {
		cc := cc
...
func TestChunkInvariantsNewWithDataSpan(t *testing.T) {
	...
	for _, cc := range []struct {
		...
		{
			name:    "small data",
			data:    make([]byte,  aisc.SpanSize),
			wantErr: nil,
		},
		{
			name:    "too large data chunk",
			data:    testutil.RandBytes(t,  aisc.ChunkSize+ aisc.SpanSize+1),
			wantErr: cac.ErrChunkDataLarge,
		},
		{
			name:    "ok",
			data:    dataWithSpan(testutil.RandBytes(t, rand.Intn( aisc.ChunkSize)+1)),
			wantErr: nil,
		},
	} {
		cc := cc
...
func TestValid(t *testing.T) {
	...
	ch :=  aisc.NewChunk(address, dataWithSpan([]byte(data)))
...
func TestInvalid(t *testing.T) {
	...
	chunker := func(addr string, dataBytes []byte)  aisc.Chunk {
		addrBytes, _ := hex.DecodeString(addr)
		address :=  aisc.NewAddress(addrBytes)
		return  aisc.NewChunk(address, dataBytes)
	}
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			chunk: chunker(
				...
				testutil.RandBytes(t,  aisc.ChunkSize+ aisc.SpanSize+1),
			),
		},
	} {
		tc := tc
...
func assertValidChunk(t *testing.T, ch  aisc.Chunk, expectValid bool) {
	t.Helper()
...
func dataWithSpan(inputData []byte) []byte {
	...
	dataBytes := make([]byte,  aisc.SpanSize+dataLength)
	binary.LittleEndian.PutUint64(dataBytes, uint64(dataLength))
	copy(dataBytes[ aisc.SpanSize:], inputData)
	return dataBytes
}
