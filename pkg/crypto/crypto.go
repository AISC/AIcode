...
// NewOverlayAddress constructs a Aisc Address from ECDSA public key.
func NewOverlayAddress(p ecdsa.PublicKey, networkID uint64, nonce []byte) ( aisc.Address, error) {
	...
	if len(nonce) != 32 {
		return  aisc.ZeroAddress, ErrBadHashLength
	}

	return NewOverlayFromEthereumAddress(ethAddr, networkID, nonce)
}

// NewOverlayFromEthereumAddress constructs a Aisc Address for an Ethereum address.
func NewOverlayFromEthereumAddress(ethAddr []byte, networkID uint64, nonce []byte) ( aisc.Address, error) {
