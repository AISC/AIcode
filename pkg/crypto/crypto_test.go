...
func TestNewOverlayFromEthereumAddress(t *testing.T) {
	...
	testCases := []struct {
		wantAddress      aisc.Address
		overlayID       uint64
		hash            []byte
		expectedAddress string
	}{
		{
			wantAddress:      aisc.MustParseHexAddress("1815cac638d1525b47f848daf02b7953e4edd15c"),
			overlayID:       1,
		...
		{
			wantAddress:      aisc.MustParseHexAddress("d26bc1715e933bd5f8fad16310042f13abc16159"),
			overlayID:       2,
			hash:            common.HexToHash("0x1").Bytes(),
			expectedAddress: "9f421f9149b8e31e238cfbdc6e5e833bacf1e42f77f60874d49291292858968e",
		},
		{
			wantAddress:      aisc.MustParseHexAddress("ac485e3c63dcf9b4cda9f007628bb0b6fed1c063"),
			overlayID:       1,
	...
	for _, tc := range testCases {
		...
		if l := len(gotAddress.Bytes()); l !=  aisc.HashSize {
			t.Errorf("got address length %v, want %v", l,  aisc.HashSize)
		}

		wantAddress, err :=  aisc.ParseHexAddress(tc.expectedAddress)
		if err != nil {
			t.Fatal(err)
		}
