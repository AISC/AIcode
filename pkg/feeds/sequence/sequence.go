...
func (f *finder) At(ctx context.Context, at int64, _ uint64) (ch  aisc.Chunk, current, next feeds.Index, err error) {
	for i := uint64(0); ; i++ {
...
type result struct {
	chunk     aisc.Chunk // the chunk found
...
func (f *asyncFinder) At(ctx context.Context, at int64, after uint64) (ch  aisc.Chunk, cur, next feeds.Index, err error) {
...
func (f *asyncFinder) asyncGet(ctx context.Context, at int64, index uint64) <-chan  aisc.Chunk {
	c := make(chan  aisc.Chunk, 1)
	go func() {
...
func (f *asyncFinder) get(ctx context.Context, at int64, idx uint64) ( aisc.Chunk, error) {
