...
const (
	maxBufferSize =  aisc.ChunkSize * 2
)
...
func (c *ChunkPipe) Write(b []byte) (int, error) {
	...
	for {
		...
		if c.cursor >=  aisc.ChunkSize {
			// NOTE: the Write method contract requires all sent data to be
			// written before returning (without error)
			written, err := c.writer.Write(c.data[: aisc.ChunkSize])
			if err != nil {
				return nw, err
			}
			if  aisc.ChunkSize != written {
				return nw, io.ErrShortWrite
			}

			c.cursor -=  aisc.ChunkSize

			copy(c.data, c.data[ aisc.ChunkSize:])
		}
	}
