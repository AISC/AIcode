...
func fingerprint(addrs [] aisc.Address) string {
	h :=  aisc.NewHasher()
...
func (g *decoderCache) GetOrCreate(addrs [] aisc.Address, shardCnt int) storage.Getter {
...
func New(ctx context.Context, g storage.Getter, putter storage.Putter, address  aisc.Address) (file.Joiner, int64, error) {
...
func (j *joiner) ReadAt(buffer []byte, off int64) (read int, err error) {
	// since offset is int64 and  aisc spans are uint64 it means we cannot seek beyond int64 max value
	if off >= j.span {
		return 0, io.EOF
	}
...
func (j *joiner) readAtOffset(
	...
	for cursor := 0; cursor < len(data); cursor += j.refLength {
		...
		addr :=  aisc.NewAddress(data[cursor : cursor+j.refLength])
		...
		func(address  aisc.Address, b []byte, cur, subTrieSize, off, bufferOffset, bytesToRead, subtrieSpanLimit int64) {
			eg.Go(func() error {
...
func (j *joiner) getShards(payloadSize, parities int) int {
	return (payloadSize - parities* aisc.HashSize) / j.refLength
}
...
func (j *joiner) subtrieSection(startIdx, payloadSize, parities int, subtrieSize int64) int64 {
	...
	var (
		...
		branchSize = int64( aisc.ChunkSize)
	)
	for {
...
func (j *joiner) IterateChunkAddresses(fn  aisc.AddressIterFunc) error {
	// report root address
	err := fn(j.addr)
	if err != nil {
		return err
...
func (j *joiner) processChunkAddresses(ctx context.Context, fn  aisc.AddressIterFunc, data []byte, subTrieSize int64, parity int) error {
	// we are at a leaf data chunk
	if subTrieSize <= int64(len(data)) {
		return nil
	}
	...
	for i, addr := range addrs {
		...
		eg.Go(func() error {
			...
			if j.refLength == encryption.ReferenceSize && i < shardCnt {
				addr =  aisc.NewAddress(data[cursor : cursor+ aisc.HashSize*2])
			}
			...
			chunkData := ch.Data()[8:]
			subtrieLevel, subtrieSpan := j.chunkToSpan(ch.Data())
			_, parities := file.ReferenceCount(uint64(subtrieSpan), subtrieLevel, j.refLength !=  aisc.HashSize)
...
func chunkToSpan(data []byte) (redundancy.Level, int64) {
	level, spanBytes := redundancy.DecodeSpan(data[: aisc.SpanSize])
