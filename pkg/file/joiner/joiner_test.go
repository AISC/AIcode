...
func TestJoiner_ErrReferenceLength(t *testing.T) {
	...
	_, _, err := joiner.New(context.Background(), store, store,  aisc.ZeroAddress)

	if !errors.Is(err, storage.ErrReferenceLength) {
		t.Fatalf("expected ErrReferenceLength %x but got %v",  aisc.ZeroAddress, err)
	}
}
...
func TestJoinerSingleChunk(t *testing.T) {
	...
	mockAddr :=  aisc.MustParseHexAddress(mockAddrHex)
	mockData := []byte("foo")
	mockDataLengthBytes := make([]byte, 8)
	mockDataLengthBytes[0] = 0x03
	mockChunk :=  aisc.NewChunk(mockAddr, append(mockDataLengthBytes, mockData...))
...
func TestJoinerWithReference(t *testing.T) {
	...
	if l != int64( aisc.ChunkSize*2) {
		t.Fatalf("expected join data length %d, got %d",  aisc.ChunkSize*2, l)
	}

	resultBuffer := make([]byte,  aisc.ChunkSize)
...
func TestJoinerMalformed(t *testing.T) {
	...
	binary.LittleEndian.PutUint64(rootChunkData[:8], uint64( aisc.ChunkSize*2))
...
func TestSeek(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name: "a few chunks",
			size: 10 *  aisc.ChunkSize,
		},
		{
			name: "a few chunks and a change",
			size: 10* aisc.ChunkSize + 84,
		},
		{
			name: "a few chunks more",
			size: 2* aisc.ChunkSize* aisc.ChunkSize + 1000,
		},
	} {
		tc := tc
...
func TestJoinerReadAt(t *testing.T) {
	...
	b := make([]byte,  aisc.ChunkSize)
	_, err = j.ReadAt(b,  aisc.ChunkSize)
	if err != nil {
		t.Fatal(err)
	}
...
func TestJoinerOneLevel(t *testing.T) {
	...
	outBuffer := make([]byte,  aisc.ChunkSize)
	c, err := j.Read(outBuffer)
	if err != nil {
		t.Fatal(err)
	}
	if c !=  aisc.ChunkSize {
		t.Fatalf("expected firstchunk read count %d, got %d",  aisc.ChunkSize, c)
	}
	...
	if c !=  aisc.ChunkSize {
		t.Fatalf("expected secondchunk read count %d, got %d",  aisc.ChunkSize, c)
	}
...
func TestJoinerTwoLevelsAcrossChunk(t *testing.T) {
	...
	rootChunk := filetest.GenerateTestRandomFileChunk( aisc.ZeroAddress,  aisc.ChunkSize* aisc.Branches+42,  aisc.SectionSize*2)
	err := store.Put(ctx, rootChunk)
	if err != nil {
		t.Fatal(err)
	}
	...
	firstAddress :=  aisc.NewAddress(rootChunk.Data()[8 :  aisc.SectionSize+8])
	firstChunk := filetest.GenerateTestRandomFileChunk(firstAddress,  aisc.ChunkSize* aisc.Branches,  aisc.ChunkSize)
	err = store.Put(ctx, firstChunk)
	if err != nil {
		t.Fatal(err)
	}
	...
	secondAddress :=  aisc.NewAddress(rootChunk.Data()[ aisc.SectionSize+8:])
	secondChunk := filetest.GenerateTestRandomFileChunk(secondAddress, 42,  aisc.SectionSize)
	err = store.Put(ctx, secondChunk)
	if err != nil {
		t.Fatal(err)
	}
	...
	cursor := 8
	for i := 0; i <  aisc.Branches; i++ {
		chunkAddressBytes := firstChunk.Data()[cursor : cursor+ aisc.SectionSize]
		chunkAddress :=  aisc.NewAddress(chunkAddressBytes)
		ch := filetest.GenerateTestRandomFileChunk(chunkAddress,  aisc.ChunkSize,  aisc.ChunkSize)
		err := store.Put(ctx, ch)
		if err != nil {
			t.Fatal(err)
		}
		cursor +=  aisc.SectionSize
	}
	...
	for i := 0; i <  aisc.Branches; i++ {
		...
		if c !=  aisc.ChunkSize {
			t.Fatalf("chunk %d expected read %d bytes; got %d", i,  aisc.ChunkSize, c)
		}
	}
	c, err := j.Read(b)
	if err != nil {
...
func TestJoinerIterateChunkAddresses(t *testing.T) {
	...
	err = j.IterateChunkAddresses(func(addr  aisc.Address) error {
...
type mockPutter struct {
	storage.ChunkStore
	shards, parities chan  aisc.Chunk
	done             chan struct{}
}
...
func newMockPutter(store storage.ChunkStore, shardCnt, parityCnt int) *mockPutter {
	return &mockPutter{
		...
		shards:     make(chan  aisc.Chunk, shardCnt),
		parities:   make(chan  aisc.Chunk, parityCnt),
	}
}

func (m *mockPutter) Put(ctx context.Context, ch  aisc.Chunk) error {
...
func TestJoinerRedundancy(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(fmt.Sprintf("redundancy=%d encryption=%t", tc.rLevel, tc.encryptChunk), func(t *testing.T) {
			...
			dataChunks := make([] aisc.Chunk, shardCnt)
			chunkSize :=  aisc.ChunkSize
			size := chunkSize
			...
			if err != nil {
			 ...
			 aiscAddr :=  aisc.NewAddress(sum)
			putter.wait(ctx)
			_, err = store.Get(ctx,  aisc.NewAddress(sum[: aisc.HashSize]))
			if err != nil {
				t.Fatal(err)
			}
			...
			readCheck := func(t *testing.T, expErr error) {
				...
				joinReader, rootSpan, err := joiner.New(ctx, store, store,  aiscAddr)
				if err != nil {
					t.Fatal(err)
				}
				// sanity checks
			...
			scnt:
				for ; i < shardCnt; i++ {
					...
					eg.Go(func() error {
						...
						expectedChunkData := dataChunks[i].Data()[ aisc.SpanSize:]
			...
			if err := store.Delete(ctx,  aisc.NewAddress( aiscAddr.Bytes()[: aisc.HashSize])); err != nil {
				t.Fatal(err)
			}
...
// redundancy level, encryption and size (levels, i.e., the	height of the  aisc hash tree).
//
...
func TestJoinerRedundancyMultilevel(t *testing.T) {
	...
	test := func(t *testing.T, rLevel redundancy.Level, encrypt bool, size int) {
		...
		dataReader := pseudorand.NewReader(seed, size* aisc.ChunkSize)
		ctx := context.Background()
		...
		store.Unmiss( aisc.NewAddress(addr.Bytes()[: aisc.HashSize]))
