...
func NewHashTrieWriter(
	...
	h := &hashTrieWriter{
		...
		refSize:                refLen,
		cursors:                make([]int, 9),
		buffer:                 make([]byte,  aisc.ChunkWithSpanSize*9*2), // double size as temp workaround for weak calculation of needed buffer space
		rParams:                rParams,
...
func (h *hashTrieWriter) ChainWrite(p *pipeline.PipeWriteArgs) error {
	oneRef := h.refSize +  aisc.SpanSize
...
func (h *hashTrieWriter) writeToIntermediateLevel(level int, parityChunk bool, span, ref, key []byte) error {
	...
	if h.chunkCounters[level] == h.maxChildrenChunks {
		// at this point the erasure coded chunks have aiscn written
...
func (h *hashTrieWriter) wrapFullLevel(level int) error {
	...
	for i := uint8(0); i < h.effectiveChunkCounters[level]; i++ {
		...
		sp += binary.LittleEndian.Uint64(data[offset : offset+ aisc.SpanSize])
		offset += + aisc.SpanSize
	...
	for offset < len(data) {
		...
		offset += + aisc.SpanSize
		hash := data[offset : offset+ aisc.HashSize] // parity reference has always hash length
		offset +=  aisc.HashSize
		hashes = append(hashes, hash...)
		parities++
	}
	spb := make([]byte, 8)
...
// Sum returns the Aisc merkle-root content-addressed hash
...
func (h *hashTrieWriter) Sum() ([]byte, error) {
	...
	if h.rParams.Level() != redundancy.NONE {
		...
		err = h.replicaPutter.Put(h.ctx,  aisc.NewChunk( aisc.NewAddress(rootHash[: aisc.HashSize]), rootData))
		if err != nil {
