...
import (
	...
	bmtUtils "github.com/aisc/pkg/bmt"
	"github.com/aisc/pkg/cac"
	"github.com/aisc/pkg/encryption"
	dec "github.com/aisc/pkg/encryption/store"
	...
	"github.com/aisc/pkg/file/pipeline/mock"
...
func TestLevels(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			rootch, err := s.Get(ctx,  aisc.NewAddress(ref))
			if err != nil {
				t.Fatal(err)
			}

			//check the span. since write spans are 1 value 1, then expected span == tc.writes
			sp := binary.LittleEndian.Uint64(rootch.Data()[: aisc.SpanSize])
...
// described in https://github.com/aisc/issues/1175
...
func (r *replicaPutter) Put(ctx context.Context, chunk  aisc.Chunk) error {
...
func TestRedundancy(t *testing.T) {
	...
	ch, err := cac.New(make([]byte,  aisc.ChunkSize))
	if err != nil {
		t.Fatal(err)
	}
	chData := ch.Data()
	chSpan := chData[: aisc.SpanSize]
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			rootch, err := s.Get(subCtx,  aisc.NewAddress(ref[: aisc.HashSize]))
			if err != nil {
				t.Fatal(err)
			}
			chData := rootch.Data()
			if tc.encryption {
				chData, err = dec.DecryptChunkData(chData, ref[ aisc.HashSize:])
				if err != nil {
					t.Fatal(err)
				}
			}
			...
			// span check
			level, sp := redundancy.DecodeSpan(chData[: aisc.SpanSize])
			expectedSpan := bmtUtils.LengthToSpan(int64(tc.writes *  aisc.ChunkSize))
