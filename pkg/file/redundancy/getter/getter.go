...
type decoder struct {
	...
	addrs        [] aisc.Address // all addresses of the intermediate chunk
...
func New(addrs [] aisc.Address, shardCnt int, g storage.Getter, p storage.Putter, remove func(error), conf Config) Getter {
...
func (g *decoder) Get(ctx context.Context, addr  aisc.Address) ( aisc.Chunk, error) {
	...
	return  aisc.NewChunk(addr, g.getData(i)), nil
}
...
func (g *decoder) setData(i int, chdata []byte) {
	...
	data := chdata
	// pad the chunk with zeros if it is smaller than  aisc.ChunkSize
	if len(data) <  aisc.ChunkWithSpanSize {
		g.lastLen = len(data)
		data = make([]byte,  aisc.ChunkWithSpanSize)
		copy(data, chdata)
	}
	g.rsbuf[i] = data
}
...
func (g *decoder) save(ctx context.Context, missing []int) error {
	...
	for _, i := range missing {
		if err := g.putter.Put(ctx,  aisc.NewChunk(g.addrs[i], g.rsbuf[i])); err != nil {
			return err
		}
	}
	return nil
