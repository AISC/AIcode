...
func initData(t *testing.T, buf [][]byte, shardCnt int, s storage.ChunkStore) [] aisc.Address {
	t.Helper()
	spanBytes := make([]byte, 8)
	binary.LittleEndian.PutUint64(spanBytes,  aisc.ChunkSize)

	for i := 0; i < len(buf); i++ {
		buf[i] = make([]byte,  aisc.ChunkWithSpanSize)
		if i >= shardCnt {
			continue
		}
	...
	addrs := make([] aisc.Address, len(buf))
	ctx := context.TODO()
	for i := 0; i < len(buf); i++ {
...
func checkShardsAvailable(t *testing.T, s storage.ChunkStore, addrs [] aisc.Address, data [][]byte) {
	t.Helper()
...
func forget(t *testing.T, store storage.ChunkStore, addrs [] aisc.Address, erasureCnt int) (erasures []int) {
	t.Helper()
