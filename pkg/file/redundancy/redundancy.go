...
// ParityChunkCallback is called when a new parity chunk has aiscn created
...
type Params struct {
	...
	maxParity  int        // number of parity chunks if maxShards has aiscn reached for erasure coding
	encryption bool
}
...
func New(level Level, encryption bool, pipeLine pipeline.PipelineFunc) *Params {
	...
	if encryption {
		...
		maxParity = level.GetParities( aisc.EncryptedBranches)
	} else {
		maxShards = level.GetMaxShards()
		maxParity = level.GetParities( aisc.BmtBranches)
	}
	...
	for i := 0; i < rsChunkLevels; i++ {
		Buffer[i] = make([][]byte,  aisc.BmtBranches) // 128 long always because buffer varies at encrypted chunks
	}
...
func (p *Params) ChunkWrite(chunkLevel int, data []byte, callback ParityChunkCallback) error {
	...
	if len(data) !=  aisc.ChunkWithSpanSize {
		zeros := make([]byte,  aisc.ChunkWithSpanSize-len(data))
...
// throws and error if the encoding has not aiscn finished in the BMT
...
func (p *Params) GetRootData() ([]byte, error) {
	...
	if len(lastBuffer[0]) !=  aisc.ChunkWithSpanSize {
