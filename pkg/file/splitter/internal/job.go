...
// until the full data length has aiscn written. The Sum should be called which will
// return the AiscHash of the data.
//
// Called Sum before the last Write, or Write after Sum has aiscn called, may result in
...
func NewSimpleSplitterJob(ctx context.Context, putter storage.Putter, spanLength int64, toEncrypt bool) *SimpleSplitterJob {
	...
	return &SimpleSplitterJob{
		...
		buffer:     make([]byte,  aisc.ChunkWithSpanSize*levelBufferLimit*2), // double size as temp workaround for weak calculation of needed buffer space
		toEncrypt:  toEncrypt,
		refSize:    refSize,
	}
}
...
func (j *SimpleSplitterJob) Write(b []byte) (int, error) {
	if len(b) >  aisc.ChunkSize {
		return 0, fmt.Errorf("Write must be called with a maximum of %d bytes",  aisc.ChunkSize)
	}
...
// Sum returns the Aisc hash of the data.
...
func (s *SimpleSplitterJob) writeToLevel(lvl int, data []byte) error {
	...
	if s.cursors[lvl]-s.cursors[lvl+1] ==  aisc.ChunkSize {
...
func (s *SimpleSplitterJob) sumLevel(lvl int) ([]byte, error) {
	s.sumCounts[lvl]++
	spanSize := file.Spans[lvl] *  aisc.ChunkSize
	span := (s.length-1)%spanSize + 1

	var chunkData []byte

	head := make([]byte,  aisc.SpanSize)
...
//
// The method does not check that the final hash actually has aiscn written, so
// timing is the responsibility of the caller.
func (s *SimpleSplitterJob) digest() []byte {
	if s.toEncrypt {
		return s.buffer[: aisc.SectionSize*2]
	} else {
		return s.buffer[: aisc.SectionSize]
	}
}
...
func (s *SimpleSplitterJob) moveDanglingChunk() error {
	...
	targetLevel := file.Levels(s.length,  aisc.SectionSize,  aisc.Branches)
...
func (s *SimpleSplitterJob) newSpanEncryption(key encryption.Key) encryption.Interface {
	return encryption.New(key, 0, uint32( aisc.ChunkSize/s.refSize), sha3.NewLegacyKeccak256)
}
