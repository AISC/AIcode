...
func TestSplitterJobPartialSingleChunk(t *testing.T) {
	...
	addressResult :=  aisc.NewAddress(hashResult)

	bmtHashOfFoo := "2387e8e7d8a48c2a9339c97c1dc3461a9a7aa07e994c5cb8b38fd7c1b3e6ea48"
	address :=  aisc.MustParseHexAddress(bmtHashOfFoo)
...
func testSplitterJobVector(t *testing.T) {
	...
	for i := 0; i < len(data); i +=  aisc.ChunkSize {
		l :=  aisc.ChunkSize
		if len(data)-i <  aisc.ChunkSize {
			l = len(data) - i
		}
	...
	actual :=  aisc.NewAddress(actualBytes)
