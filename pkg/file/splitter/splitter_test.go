...
func TestSplitSingleChunk(t *testing.T) {
	...
	testData, err := g.SequentialBytes( aisc.ChunkSize)
	if err != nil {
		t.Fatal(err)
	}
	...
	testHashAddress :=  aisc.MustParseHexAddress(testHashHex)
...
func TestSplitThreeLevels(t *testing.T) {
	...
	g := mockbytes.New(0, mockbytes.MockTypeStandard).WithModulus(255)
	testData, err := g.SequentialBytes( aisc.ChunkSize * 128)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i <  aisc.ChunkSize; i +=  aisc.SectionSize {
		dataAddressBytes := rootData[i : i+ aisc.SectionSize]
		dataAddress :=  aisc.NewAddress(dataAddressBytes)
...
func TestUnalignedSplit(t *testing.T) {
	...
	var (
		dataLen       int64 =  aisc.ChunkSize*2 + 32
	...
	writeSizes := []int{ aisc.ChunkSize - 40, 40 + 32,  aisc.ChunkSize}
	...
	select {
	case addr := <-doneC:
		expectAddr :=  aisc.MustParseHexAddress(expectAddrHex)
...
goos: linux
goarch: amd64
pkg: github.com/aisc/pkg/file/splitter
BenchmarkSplitter
...
PASS
ok  	github.com/aisc/pkg/file/splitter	22.476s
*/
