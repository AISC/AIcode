...
type Service struct {
	...
	addPeersHandler   func(... aisc.Address)
	networkID         uint64
	logger            log.Logger
	metrics           metrics
...
func New(streamer p2p.StreamerPinger, addressbook addressbook.GetPutter, networkID uint64, bootnode bool, allowPrivateCIDRs bool, logger log.Logger) *Service {
	svc := &Service{
		...
		sem:               semaphore.NewWeighted(int64( aisc.MaxBins)),
		bootnode:          bootnode,
...
func (s *Service) BroadcastPeers(ctx context.Context, addressee  aisc.Address, peers ... aisc.Address) error {
	max := maxBatchSize
...
func (s *Service) SetAddPeersHandler(h func(addr ... aisc.Address)) {
	s.addPeersHandler = h
}
...
func (s *Service) sendPeers(ctx context.Context, peer  aisc.Address, peers [] aisc.Address) (err error) {
	...
	for _, p := range peers {
		...
		peersRequest.Peers = append(peersRequest.Peers, &pb. aiscAddress{
...
func (s *Service) checkAndAddPeers(ctx context.Context, peers pb.Peers) {
	...
	var peersToAdd [] aisc.Address
	mtx := sync.Mutex{}
	wg := sync.WaitGroup{}

	addPeer := func(newPeer *pb. aiscAddress, multiUnderlay ma.Multiaddr) {
		...
		go func() {
			...
			aiscAddress := aisc.Address{
				Overlay:    aisc.NewAddress(newPeer.Overlay),
				Underlay:  multiUnderlay,
				Signature: newPeer.Signature,
				Nonce:     newPeer.Nonce,
			}

			err := s.addressBook.Put(aiscAddress.Overlay, aiscAddress)
			if err != nil {
			...
			mtx.Lock()
			peersToAdd = append(peersToAdd, aiscAddress.Overlay)
			mtx.Unlock()
		}()
	...
	for _, p := range peers.Peers {
		...
		addr, err := s.addressBook.Get( aisc.NewAddress(p.Overlay))
