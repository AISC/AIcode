...
import (
	...
	ab "github.com/aisc/pkg/addressbook"
	"github.com/aisc/pkg/aisc"
	"github.com/aisc/pkg/crypto"
	"github.com/aisc/pkg/hive"
	"github.com/aisc/pkg/hive/pb"
...
func TestHandlerRateLimit(t *testing.T) {
	...
	serverAddress :=  aisc.RandAddress(t)
	...
	peers := make([] aisc.Address, hive.LimitBurst+1)
	for i := range peers {
		...
		aiscAddr, err := aisc.NewAddress(signer, underlay, overlay, networkID, nonce)
		if err != nil {
			t.Fatal(err)
		}

		err = addressbook.Put(aiscAddr.Overlay, *aiscAddr)
		if err != nil {
			t.Fatal(err)
		}
		peers[i] = aiscAddr.Overlay
	}
...
func TestBroadcastPeers_FLAKY(t *testing.T) {
	...
	var aiscAddresses []aisc.Address
	var overlays [] aisc.Address
	var wantMsgs []pb.Peers

	for i := 0; i < 2; i++ {
		wantMsgs = append(wantMsgs, pb.Peers{Peers: []*pb. aiscAddress{}})
	}
	...
	for i := 0; i < 2*hive.MaxBatchSize; i++ {
		...
		aiscAddresses = append(aiscAddresses, *aiscAddr)
		overlays = append(overlays, aiscAddr.Overlay)
		err = addressbook.Put(aiscAddr.Overlay, *aiscAddr)
		if err != nil {
			t.Fatal(err)
		}

		wantMsgs[i/hive.MaxBatchSize].Peers = append(wantMsgs[i/hive.MaxBatchSize].Peers, &pb. aiscAddress{
			Overlay:   aiscAddresses[i].Overlay.Bytes(),
			Underlay:  aiscAddresses[i].Underlay.Bytes(),
			Signature: aiscAddresses[i].Signature,
			Nonce:     nonce,
		})
	}

	testCases := map[string]struct {
		addresee           aisc.Address
		peers             [] aisc.Address
		wantMsgs          []pb.Peers
		wantOverlays      [] aisc.Address
		want aiscAddresses  []aisc.Address
		allowPrivateCIDRs bool
		pingErr           func(addr ma.Multiaddr) (time.Duration, error)
	}{
		"OK - single record": {
			addresee:           aisc.MustParseHexAddress("ca1e9f3938cc1425c6061b96ad9eb93e134dfe8734ad490164ef20af9d1cf59c"),
			peers:             [] aisc.Address{overlays[0]},
			wantMsgs:          []pb.Peers{{Peers: wantMsgs[0].Peers[:1]}},
			wantOverlays:      [] aisc.Address{overlays[0]},
			want aiscAddresses:  []aisc.Address{aiscAddresses[0]},
		...
		"OK - single batch - multiple records": {
			...
			want aiscAddresses:  aiscAddresses[:15],
			allowPrivateCIDRs: true,
		},
		"OK - single batch - max number of records": {
			...
			want aiscAddresses:  aiscAddresses[:hive.MaxBatchSize],
			allowPrivateCIDRs: true,
		},
		"OK - multiple batches": {
			addresee:           aisc.MustParseHexAddress("ca1e9f3938cc1425c6061b96ad9eb93e134dfe8734ad490164ef20af9d1cf59c"),
			...
			want aiscAddresses:  aiscAddresses[:hive.MaxBatchSize+10],
			allowPrivateCIDRs: true,
		},
		"OK - multiple batches - max number of records": {
			...
			want aiscAddresses:  aiscAddresses[:2*hive.MaxBatchSize],
			allowPrivateCIDRs: true,
		},
		"OK - single batch - skip ping failures": {
			...
			want aiscAddresses:  aiscAddresses[:10],
			allowPrivateCIDRs: true,
			pingErr: func(addr ma.Multiaddr) (rtt time.Duration, err error) {
				for _, v := range aiscAddresses[10:15] {
	...
	for name, tc := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			expect aiscAddresessEventually(t, addressbookclean, tc.want aiscAddresses)
		})
	}
}

func expectOverlaysEventually(t *testing.T, exporter ab.Interface, wantOverlays [] aisc.Address) {
	t.Helper()

	var (
		overlays [] aisc.Address
		err      error
	)
	...
	for _, v := range wantOverlays {
		if ! aisc.ContainsAddress(overlays, v) {
...
func expect aiscAddresessEventually(t *testing.T, exporter ab.Interface, want aiscAddresses []aisc.Address) {
	t.Helper()
	...
	err = spinlock.Wait(spinTimeout, func() bool {
		...
		return len(addresses) == len(want aiscAddresses)
	})
	if err != nil {
		t.Fatal("timed out waiting for aisc addresses")
	}

	for _, v := range want aiscAddresses {
		if !aisc.ContainsAddress(addresses, &v) {
			t.Errorf("address %s expected but not found", v.Overlay.String())
		}
	}

	if t.Failed() {
		t.Errorf("aisc addresses got %v, want %v", addresses, want aiscAddresses)
	}
}
