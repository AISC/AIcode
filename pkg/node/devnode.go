...
import (
	...
	mockAccounting "github.com/aisc/pkg/accounting/mock"
	...
	mockP2P "github.com/aisc/pkg/p2p/mock"
	mockPingPong "github.com/aisc/pkg/pingpong/mock"
	...
	mockPostContract "github.com/aisc/pkg/postage/postagecontract/mock"
	postagetesting "github.com/aisc/pkg/postage/testing"
	"github.com/aisc/pkg/pss"
	"github.com/aisc/pkg/pushsync"
	mockPushsync "github.com/aisc/pkg/pushsync/mock"
	...
	mockSteward "github.com/aisc/pkg/steward/mock"
	"github.com/aisc/pkg/storage/inmemstore"
	"github.com/aisc/pkg/storageincentives/staking"
	stakingContractMock "github.com/aisc/pkg/storageincentives/staking/mock"
	"github.com/aisc/pkg/storer"
	"github.com/aisc/pkg/ aisc"
	"github.com/aisc/pkg/topology/lightnode"
	mockTopology "github.com/aisc/pkg/topology/mock"
...
// NewDevAisc starts the aisc instance in 'development' mode
// this implies starting an API and a Debug endpoints while mocking all their services.
func NewDevAisc(logger log.Logger, o *DevOptions) (b *DevAisc, err error) {
	...
	pssService.SetPushSyncer(mockPushsync.New(func(ctx context.Context, chunk  aisc.Chunk) (*pushsync.Receipt, error) {
	...
	var (
		lightNodes = lightnode.NewContainer( aisc.NewAddress(nil))
		pingPong   = mockPingPong.New(pong)
		p2ps       = mockP2P.New(
			mockP2P.WithConnectFunc(func(ctx context.Context, addr multiaddr.Multiaddr) (address *aisc.Address, err error) {
				return &aisc.Address{}, nil
			}), mockP2P.WithDisconnectFunc(
				func( aisc.Address, string) error {
					return nil
				},
		...
		mockSwap  = swapmock.New(swapmock.WithCashoutStatusFunc(
			func(ctx context.Context, peer  aisc.Address) (*chequebook.CashoutStatus, error) {
			...
			func(a  aisc.Address) (*chequebook.SignedCheque, error) {
	...
	if o.DebugAPIAddr != "" {
		...
		debugApiService.SetAiscAddress(& aiscAddress)
...
func pong(_ context.Context, _  aisc.Address, _ ...string) (rtt time.Duration, err error) {
	return time.Millisecond, nil
}

func randomAddress() ( aisc.Address, error) {
