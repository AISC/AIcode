...
// Package node defines the concept of a Aisc node
...
import (
	...
	"github.com/aisc/pkg/auth"
	"github.com/aisc/pkg/config"
	"github.com/aisc/pkg/crypto"
	"github.com/aisc/pkg/feeds/factory"
	"github.com/aisc/pkg/hive"
	"github.com/aisc/pkg/log"
	"github.com/aisc/pkg/metrics"
	"github.com/aisc/pkg/p2p"
	"github.com/aisc/pkg/p2p/libp2p"
	"github.com/aisc/pkg/pingpong"
	"github.com/aisc/pkg/postage"
	"github.com/aisc/pkg/postage/batchservice"
	"github.com/aisc/pkg/postage/batchstore"
	"github.com/aisc/pkg/postage/listener"
	"github.com/aisc/pkg/postage/postagecontract"
	"github.com/aisc/pkg/pricer"
	"github.com/aisc/pkg/pricing"
	"github.com/aisc/pkg/pss"
	"github.com/aisc/pkg/puller"
	"github.com/aisc/pkg/pullsync"
	"github.com/aisc/pkg/pusher"
	"github.com/aisc/pkg/pushsync"
	"github.com/aisc/pkg/resolver/multiresolver"
	...
	"github.com/aisc/pkg/settlement/swap/erc20"
	"github.com/aisc/pkg/settlement/swap/priceoracle"
	"github.com/aisc/pkg/status"
	"github.com/aisc/pkg/steward"
	"github.com/aisc/pkg/storageincentives"
	"github.com/aisc/pkg/storageincentives/redistribution"
	"github.com/aisc/pkg/storageincentives/staking"
	...
	"github.com/aisc/pkg/util/abiutil"
	"github.com/aisc/pkg/util/ioutil"
	"github.com/aisc/pkg/util/nbhdutil"
	"github.com/aisc/pkg/util/syncutil"
...
func NewAisc(
	...
	defer func() {
		// if there's aiscn an error on this function
	...
	if err != nil {
	 ...
	 aiscAddress, err := crypto.NewOverlayAddress(*pubKey, networkID, nonce)
	if err != nil {
		return nil, fmt.Errorf("compute overlay address: %w", err)
	}

	if nonceExists && o.TargetNeighborhood != "" {
		logger.Warning("an overlay has already aiscn created before, skipping targeting the selected neighborhood")
	}
	...
	if !nonceExists {
		...
		if targetNeighborhood != "" {
			logger.Info("mining an overlay address for the fresh node to target the selected neighborhood", "target", targetNeighborhood)
			 aiscAddress, nonce, err = nbhdutil.MineOverlay(ctx, *pubKey, networkID, targetNeighborhood)
			if err != nil {
				return nil, fmt.Errorf("mine overlay address: %w", err)
			}
		}

		err = setOverlay(stateStore,  aiscAddress, nonce)
		if err != nil {
			return nil, fmt.Errorf("statestore: save new overlay: %w", err)
		}
	}

	logger.Info("using overlay address", "address",  aiscAddress)

	if err = checkOverlay(stateStore,  aiscAddress); err != nil {
	...
	aiscNodeMode := api.LightMode
	if o.FullNodeMode {
		aiscNodeMode = api.FullMode
	} else if !chainEnabled {
		aiscNodeMode = api.UltraLightMode
	}

	// Create api.Probe in healthy state and switch to ready state after all components have aiscn constructed
	probe := api.NewProbe()
	...
	apiService.SetAiscAddress(& aiscAddress)

	lightNodes := lightnode.NewContainer( aiscAddress)
	...
	if networkID == mainnetNetworkID && o.UsePostageSnapshot && (!batchStoreExists || o.Resync) {
		start := time.Now()
		logger.Info("cold postage start detected. fetching postage stamp snapshot from  aisc")
	...
	p2ps, err := libp2p.New(ctx, signer, networkID,  aiscAddress, addr, addressbook, stateStore, lightNodes, logger, tracer, libp2p.Options{
	...
	aiscTokenAddress, err := postagecontract.LookupERC20Address(ctx, transactionService, postageStampContractAddress, postageStampContractABI, chainEnabled)
	if err != nil {
		return nil, err
	}
	...
	pushSyncProtocol := pushsync.New( aiscAddress, nonce, p2ps, localStore, kad, o.FullNodeMode, pssService.TryUnwrap, validStamp, logger, acc, pricer, signer, tracer, warmupTime)
	b.pushSyncCloser = pushSyncProtocol

	// set the pushSyncer in the PSS
	pssService.SetPushSyncer(pushSyncProtocol)

	nodeStatus := status.NewService(logger, p2ps, kad, aiscNodeMode.String(), batchStore, localStore)
	...
	networkR.Store(uint32( aisc.MaxBins))
	...
	waitNetworkRFunc := func() (uint8, error) {
		if networkR.Load() == uint32( aisc.MaxBins) {
			select {
			case <-initialRadiusC:
			case <-ctx.Done():
				return 0, ctx.Err()
	...
	retrieval := retrieval.New( aiscAddress, waitNetworkRFunc, localStore, p2ps, kad, logger, acc, pricer, tracer, o.RetrievalCaching)
	...
	stakingContract := staking.New( aiscAddress, overlayEthAddress, stakingContractAddress, abiutil.MustParseABI(chainCfg.StakingABI), aiscTokenAddress, transactionService, common.BytesToHash(nonce))
	...
	if o.FullNodeMode && !o.BootnodeMode {
		...
		if o.EnableStorageIncentives {
			...
			redistributionContract := redistribution.New( aiscAddress, logger, transactionService, redistributionContractAddress, abiutil.MustParseABI(chainCfg.RedistributionABI))
	...
	if o.APIAddr != "" {
		if apiService == nil {
			apiService = api.New(*publicKey, pssPrivateKey.PublicKey, overlayEthAddress, logger, transactionService, batchStore, aiscNodeMode, o.ChequebookEnable, o.SwapEnable, chainBackend, o.CORSAllowedOrigins, stamperStore)
			apiService.SetProbe(probe)
	...
	if o.DebugAPIAddr != "" {
		...
		debugService.SetAiscAddress(& aiscAddress)
...
func (b *Aisc) SyncingStopped() chan struct{} {
	return b.syncingStopped.C
}

func (b *Aisc) Shutdown() error {
	var mErr error
