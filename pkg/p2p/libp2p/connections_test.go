...
import (
	...
	"github.com/aisc/pkg/p2p/libp2p/internal/handshake"
	...
	bhost "github.com/libp2p/go-libp2p/p2p/host/basic"
	 aisct "github.com/libp2p/go-libp2p/p2p/net/ aisc/testing"
...
func TestConnectDisconnect(t *testing.T) {
	...
	aiscAddr, err := s2.Connect(ctx, addr)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := s2.Disconnect(aiscAddr.Overlay, testDisconnectMsg); err != nil {
		t.Fatal(err)
	}
...
func TestLightPeerLimit(t *testing.T) {
	...
	var (
		limit     = 3
		container = lightnode.NewContainer( aisc.RandAddress(t))
...
func TestDoubleDisconnect(t *testing.T) {
	...
	if err := s2.Disconnect(aiscAddr.Overlay, testDisconnectMsg); !errors.Is(err, p2p.ErrPeerNotFound) {
...
func TestTopologyNotifier(t *testing.T) {
	...
	aiscAddr2, err := s1.Connect(ctx, addr2)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := s1.Disconnect(aiscAddr2.Overlay, testDisconnectMsg); err != nil {
		t.Fatal(err)
	}
	expectPeers(t, s1)
...
func TestTopologyAnnounce(t *testing.T) {
	...
	var (
		...
		n1a = func(context.Context,  aisc.Address, bool) error {
			mtx.Lock()
			announceCalled = true
			mtx.Unlock()
			return nil
		}
		n1at = func(context.Context,  aisc.Address,  aisc.Address, bool) error {
			mtx.Lock()
	...
	s1, _ := newService(t, 1, libp2pServiceOpts{
		libp2pOpts: libp2p.WithHostFactory(
			func(_ ...libp2pm.Option) (host.Host, error) {
				return bhost.NewHost( aisct.GenAisc(t), &bhost.HostOpts{})
			},
		),
	})
...
func expectZeroAddress(t *testing.T, addrs ... aisc.Address) {
	t.Helper()
	for i, a := range addrs {
		if !a.Equal( aisc.ZeroAddress) {
			t.Fatalf("address did not equal zero address. index %d", i)
		}
	}
}

func waitAddrSet(t *testing.T, addr * aisc.Address, mtx *sync.Mutex, exp  aisc.Address) {
	t.Helper()
...
func checkAddressbook(t *testing.T, ab addressbook.Getter, overlay  aisc.Address, underlay ma.Multiaddr) {
	t.Helper()
...
func (n *notifiee) Announce(ctx context.Context, a  aisc.Address, full bool) error {
	return n.announce(ctx, a, full)
}

func (n *notifiee) AnnounceTo(ctx context.Context, a, b  aisc.Address, full bool) error {
...
func (n *notifiee) Reachable(addr  aisc.Address, status p2p.ReachabilityStatus) {
...
type (
	...
	dFunc            func(p2p.Peer)
	announceFunc     func(context.Context,  aisc.Address, bool) error
	announceToFunc   func(context.Context,  aisc.Address,  aisc.Address, bool) error
	reachabilityFunc func(p2p.ReachabilityStatus)
	reachableFunc    func( aisc.Address, p2p.ReachabilityStatus)
)

var noopCf = func(context.Context, p2p.Peer, bool) error { return nil }
var noopDf = func(p2p.Peer) {}
var noopAnnounce = func(context.Context,  aisc.Address, bool) error { return nil }
var noopAnnounceTo = func(context.Context,  aisc.Address,  aisc.Address, bool) error { return nil }
var noopReachability = func(p2p.ReachabilityStatus) {}
var noopReachable = func( aisc.Address, p2p.ReachabilityStatus) {}
