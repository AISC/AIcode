...
func (b *Blocklist) Exists(overlay  aisc.Address) (bool, error) {
...
func (b *Blocklist) Add(overlay  aisc.Address, duration time.Duration, reason string, full bool) (err error) {
	key := generateKey(overlay)
	_, d, err := b.get(key)
	if err != nil {
		if !errors.Is(err, storage.ErrNotFound) {
...
func generateKey(overlay  aisc.Address) string {
	return keyPrefix + overlay.String()
}

func unmarshalKey(s string) ( aisc.Address, error) {
	addr := s[len(keyPrefix):] // trim prefix
	return  aisc.ParseHexAddress(addr)
}
