...
func New(signer crypto.Signer, advertisableAddresser AdvertisableAddressResolver, overlay  aisc.Address, networkID uint64, fullNode bool, nonce []byte, welcomeMessage string, validateOverlay bool, ownPeerID libp2ppeer.ID, logger log.Logger) (*Service, error) {
...
func (s *Service) Handshake(ctx context.Context, stream p2p.Stream, peerMultiaddr ma.Multiaddr, peerID libp2ppeer.ID) (i *Info, err error) {
	...
	aiscAddress, err := aisc.NewAddress(s.signer, advertisableUnderlay, s.overlay, s.networkID, s.nonce)
	if err != nil {
		return nil, err
	}

	advertisableUnderlayBytes, err := aiscAddress.Underlay.MarshalBinary()
	if err != nil {
		return nil, err
	}
	...
	remote aiscAddress, err := s.parseCheckAck(resp.Ack)
	if err != nil {
		return nil, err
	}
	...
	msg := &pb.Ack{
		Address: &pb. aiscAddress{
			...
			Overlay:   aiscAddress.Overlay.Bytes(),
			Signature: aiscAddress.Signature,
		},
	...
	loggerV1.Debug("handshake finished for peer (outbound)", "peer_address", remote aiscAddress.Overlay)
	if len(resp.Ack.WelcomeMessage) > 0 {
		s.logger.Debug("greeting message from peer", "peer_address", remote aiscAddress.Overlay, "message", resp.Ack.WelcomeMessage)
	}

	return &Info{
		 aiscAddress: remote aiscAddress,
...
func (s *Service) Handle(ctx context.Context, stream p2p.Stream, remoteMultiaddr ma.Multiaddr, remotePeerID libp2ppeer.ID) (i *Info, err error) {
	...
	overlay :=  aisc.NewAddress(ack.Address.Overlay)
	...
	remote aiscAddress, err := s.parseCheckAck(&ack)
	if err != nil {
		return nil, err
	}

	loggerV1.Debug("handshake finished for peer (inbound)", "peer_address", remote aiscAddress.Overlay)
	if len(ack.WelcomeMessage) > 0 {
		loggerV1.Debug("greeting message from peer", "peer_address", remote aiscAddress.Overlay, "message", ack.WelcomeMessage)
	}
...
func (s *Service) parseCheckAck(ack *pb.Ack) (*aisc.Address, error) {
	aiscAddress, err := aisc.ParseAddress(ack.Address.Underlay, ack.Address.Overlay, ack.Address.Signature, ack.Nonce, s.validateOverlay, s.networkID)
	if err != nil {
		return nil, ErrInvalidAck
	}

	return aiscAddress, nil
}
