...
import (
	...
	"github.com/aisc/pkg/p2p/libp2p/internal/handshake/mock"
	"github.com/aisc/pkg/p2p/libp2p/internal/handshake/pb"
...
func TestHandshake(t *testing.T) {
	...
	node1 aiscAddress, err := aisc.NewAddress(signer1, node1ma, addr, networkID, nonce)
	if err != nil {
		t.Fatal(err)
	}
	...
	node2 aiscAddress, err := aisc.NewAddress(signer2, node2ma, addr2, networkID, nonce)
	if err != nil {
		t.Fatal(err)
	}

	node1Info := handshake.Info{
		 aiscAddress: node1 aiscAddress,
		FullNode:   true,
	}
	node2Info := handshake.Info{
		 aiscAddress: node2 aiscAddress,
		FullNode:   true,
	}

	aaddresser := &AdvertisableAddresserMock{}

	handshakeService, err := handshake.New(signer1, aaddresser, node1Info. aiscAddress.Overlay, networkID, true, nonce, testWelcomeMessage, true, node1AddrInfo.ID, logger)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("Handshake - OK", func(t *testing.T) {
		...
		if err := w.WriteMsg(&pb.SynAck{
			...
			Ack: &pb.Ack{
				Address: &pb. aiscAddress{
					Underlay:  node2maBinary,
					Overlay:   node2 aiscAddress.Overlay.Bytes(),
					Signature: node2 aiscAddress.Signature,
				},
				NetworkID:      networkID,
				FullNode:       true,
				Nonce:          nonce,
		...
		if !bytes.Equal(ack.Address.Overlay, node1 aiscAddress.Overlay.Bytes()) {
			t.Fatal("bad ack - overlay")
		}
		if !bytes.Equal(ack.Address.Underlay, node1maBinary) {
			t.Fatal("bad ack - underlay")
		}
		if !bytes.Equal(ack.Address.Signature, node1 aiscAddress.Signature) {
	...
	t.Run("Handshake - picker error", func(t *testing.T) {
		handshakeService, err := handshake.New(signer1, aaddresser, node1Info. aiscAddress.Overlay, networkID, true, nonce, "", true, node1AddrInfo.ID, logger)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("Handshake - welcome message too long", func(t *testing.T) {
		...
		_, err := handshake.New(signer1, aaddresser, node1Info. aiscAddress.Overlay, networkID, true, nil, LongMessage, true, node1AddrInfo.ID, logger)
		...
		if err := w.WriteMsg(&pb.SynAck{
			...
			Ack: &pb.Ack{
				Address: &pb. aiscAddress{
					Underlay:  node2maBinary,
					Overlay:   node2 aiscAddress.Overlay.Bytes(),
					Signature: node1 aiscAddress.Signature,
				},
				NetworkID: networkID,
				FullNode:  true,
			},
	...
	t.Run("Handle - OK", func(t *testing.T) {
		...
		aiscAddress, err := aisc.ParseAddress(got.Ack.Address.Underlay, got.Ack.Address.Overlay, got.Ack.Address.Signature, got.Ack.Nonce, true, got.Ack.NetworkID)
		if err != nil {
			t.Fatal(err)
		}

		testInfo(t, node1Info, handshake.Info{
			 aiscAddress: aiscAddress,
			FullNode:   got.Ack.FullNode,
		})
	})

	t.Run("Handle - read error ", func(t *testing.T) {
		handshakeService, err := handshake.New(signer1, aaddresser, node1Info. aiscAddress.Overlay, networkID, true, nil, "", true, node1AddrInfo.ID, logger)
		if err != nil {
			t.Fatal(err)
		}
		testErr := errors.New("test error")
...
func testInfo(t *testing.T, got, want handshake.Info) {
	t.Helper()
	if !got. aiscAddress.Equal(want. aiscAddress) || got.FullNode != want.FullNode {
