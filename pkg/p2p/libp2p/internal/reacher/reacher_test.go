...
func TestPingSuccess(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		reachableFunc func(chan struct{}) func(addr  aisc.Address, got p2p.ReachabilityStatus)
	}{
		{
			name: "ping success",
			pingFunc: func(context.Context, ma.Multiaddr) (time.Duration, error) {
			...
			reachableFunc: func(done chan struct{}) func(addr  aisc.Address, got p2p.ReachabilityStatus) {
				return func(addr  aisc.Address, got p2p.ReachabilityStatus) {
...
func TestDisconnected(t *testing.T) {
	...
	var (
		disconnectedOverlay =  aisc.RandAddress(t)
	...
	*/
	...
	reachableFunc := func(addr  aisc.Address, b p2p.ReachabilityStatus) {}
	...
	r.Connected( aisc.RandAddress(t), nil)
...
func newMock(ping func(context.Context, ma.Multiaddr) (time.Duration, error), reach func( aisc.Address, p2p.ReachabilityStatus)) *mock {
	return &mock{
		pingFunc:      ping,
		reachableFunc: reach,
	}
...
func (m *mock) Reachable(addr  aisc.Address, status p2p.ReachabilityStatus) {
