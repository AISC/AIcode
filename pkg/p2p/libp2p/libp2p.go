...
import (
	...
	"github.com/aisc/pkg/p2p/libp2p/internal/blocklist"
	"github.com/aisc/pkg/p2p/libp2p/internal/breaker"
	handshake "github.com/aisc/pkg/p2p/libp2p/internal/handshake"
	"github.com/aisc/pkg/p2p/libp2p/internal/reacher"
	...
	lp2p aisc "github.com/libp2p/go-libp2p/p2p/net/ aisc"
	...
	m2 "github.com/aisc/pkg/metrics"
...
type lightnodes interface {
	...
	Count() int
	RandomPeer( aisc.Address) ( aisc.Address, error)
...
func New(ctx context.Context, signer aisccrypto.Signer, networkID uint64, overlay  aisc.Address, addr string, ab addressbook.Putter, storer storage.StateStorer, lightNodes *lightnode.Container, logger log.Logger, tracer *tracing.Tracer, o Options) (*Service, error) {
	...
	// Construct protocols.
	id := protocol.ID(p2p.NewAiscStreamName(handshake.ProtocolName, handshake.ProtocolVersion, handshake.StreamName))
...
func (s *Service) handleIncoming(stream network.Stream) {
	...
	if i.FullNode {
		err = s.addressbook.Put(i. aiscAddress.Overlay, *i. aiscAddress)
		if err != nil {
			...
			_ = s.Disconnect(i. aiscAddress.Overlay, "unable to persist peer in addressbook")
			return
		}
	}

	peer := p2p.Peer{Address: overlay, FullNode: i.FullNode, EthereumAddress: i. aiscAddress.EthereumAddress}
	...
	if s.notifier != nil {
		if !i.FullNode {
			...
			if err := s.notifier.Connected(s.ctx, peer, false); err != nil {
				s.logger.Debug("stream handler: notifier.Connected: peer disconnected", "peer", i. aiscAddress.Overlay, "error", err)
				...
				// the other side.
				// that is why the Pick method has aiscn added to the notifier
			...
			// a solid topology.
			_ = s.lightNodes.EachPeer(func(addr  aisc.Address, _ uint8) (bool, bool, error) {
				go func(addressee, peer  aisc.Address, fullnode bool) {
	...
	if s.reacher != nil {
		s.reacher.Connected(overlay, i. aiscAddress.Underlay)
	}

	peerUserAgent := appendSpace(s.peerUserAgent(s.ctx, peerID))

	loggerV1.Debug("stream handler: successfully connected to peer (inbound)", "addresses", i. aiscAddress.ShortString(), "light", i.LightString(), "user_agent", peerUserAgent)
	s.logger.Debug("stream handler: successfully connected to peer (inbound)", "address", i. aiscAddress.Overlay, "light", i.LightString(), "user_agent", peerUserAgent)
}
...
func (s *Service) AddProtocol(p p2p.ProtocolSpec) (err error) {
	for _, ss := range p.StreamSpecs {
		ss := ss
		id := protocol.ID(p2p.NewAiscStreamName(p.Name, p.Version, ss.Name))
		matcher, err := s.protocolSemverMatcher(id)
		if err != nil {
...
func (s *Service) Blocklist(overlay  aisc.Address, duration time.Duration, reason string) error {
	loggerV1 := s.logger.V(1).Register()
...
func (s *Service) Connect(ctx context.Context, addr ma.Multiaddr) (address *aisc.Address, err error) {
	loggerV1 := s.logger.V(1).Register()
	...
	if i.FullNode {
		err = s.addressbook.Put(overlay, *i. aiscAddress)
		if err != nil {
			_ = s.Disconnect(overlay, "failed storing peer in addressbook")
			return nil, fmt.Errorf("storing aisc address: %w", err)
		}
	}
	...
	for _, tn := range s.protocols {
		if tn.ConnectOut != nil {
			if err := tn.ConnectOut(ctx, p2p.Peer{Address: overlay, FullNode: i.FullNode, EthereumAddress: i. aiscAddress.EthereumAddress}); err != nil {
	...
	loggerV1.Debug("successfully connected to peer (outbound)", "addresses", i. aiscAddress.ShortString(), "light", i.LightString(), "user_agent", peerUserAgent)
	s.logger.Debug("successfully connected to peer (outbound)", "address", i. aiscAddress.Overlay, "light", i.LightString(), "user_agent", peerUserAgent)
	return i. aiscAddress, nil
}

func (s *Service) Disconnect(overlay  aisc.Address, reason string) (err error) {
...
func (s *Service) disconnected(address  aisc.Address) {
...
func (s *Service) Blocklisted(overlay  aisc.Address) (bool, error) {
...
func (s *Service) NewStream(ctx context.Context, overlay  aisc.Address, headers p2p.Headers, protocolName, protocolVersion, streamName string) (p2p.Stream, error) {
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	default:
...
func (s *Service) newStreamForPeerID(ctx context.Context, peerID libp2ppeer.ID, protocolName, protocolVersion, streamName string) (network.Stream, error) {
	 aiscStreamName := p2p.NewAiscStreamName(protocolName, protocolVersion, streamName)
	st, err := s.host.NewStream(ctx, peerID, protocol.ID( aiscStreamName))
	if err != nil {
		if st != nil {
		...
		return nil, fmt.Errorf("create stream %s to %s: %w",  aiscStreamName, peerID, err)
	}
...
func (s *Service) determineCurrentNetworkStatus(err error) error {
	...
	case errors.Is(err, lp2p aisc.ErrDialBackoff):
...
func userAgent() string {
	return fmt.Sprintf("aisc/%s %s %s/%s", aisc.Version, runtime.Version(), runtime.GOOS, runtime.GOARCH)
}
...
func isNetworkOrHostUnreachableError(err error) bool {
	...
	for i := range de.DialErrors {
		var te *lp2p aisc.TransportError
