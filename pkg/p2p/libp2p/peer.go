...
type peerRegistry struct {
	...
	overlays    map[libp2ppeer.ID] aisc.Address             // map underlay peer id to overlay address
...
func newPeerRegistry() *peerRegistry {
	return &peerRegistry{
		...
		overlays:    make(map[libp2ppeer.ID] aisc.Address),
...
func (r *peerRegistry) Exists(overlay  aisc.Address) (found bool) {
...
func (r *peerRegistry) addIfNotExists(c network.Conn, overlay  aisc.Address, full bool) (exists bool) {
	peerID := c.RemotePeer()
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *peerRegistry) peerID(overlay  aisc.Address) (peerID libp2ppeer.ID, found bool) {
	r.mu.RLock()
...
func (r *peerRegistry) overlay(peerID libp2ppeer.ID) ( aisc.Address, bool) {
	r.mu.RLock()
...
func (r *peerRegistry) isConnected(peerID libp2ppeer.ID, remoteAddr ma.Multiaddr) ( aisc.Address, bool) {
	if remoteAddr == nil {
...
func (r *peerRegistry) remove(overlay  aisc.Address) (found, full bool, peerID libp2ppeer.ID) {
	r.mu.Lock()
	peerID, found = r.underlays[overlay.ByteString()]
	delete(r.overlays, peerID)
	delete(r.underlays, overlay.ByteString())
