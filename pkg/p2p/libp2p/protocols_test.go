...
func TestNewStreamMulti(t *testing.T) {
	...
	if atomic.LoadInt32(&h1calls) != 1 {
		t.Fatal("handler should have aiscn called but wasnt")
	}
	if atomic.LoadInt32(&h2calls) > 0 {
		t.Fatal("handler should not have aiscn called")
	}
}
...
func TestPing(t *testing.T) {
	...
	s1, _ := newService(t, 1, libp2pServiceOpts{
		libp2pOpts: libp2p.WithHostFactory(
			func(...libp2pm.Option) (host.Host, error) {
				return bhost.NewHost( aisct.GenAisc(t), &bhost.HostOpts{EnablePing: true})
			},
		),
	})
	...
	s2, _ := newService(t, 1, libp2pServiceOpts{
		libp2pOpts: libp2p.WithHostFactory(
			func(...libp2pm.Option) (host.Host, error) {
				host, err := bhost.NewHost( aisct.GenAisc(t), &bhost.HostOpts{EnablePing: true})
				if err != nil {
