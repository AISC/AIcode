...
type Service struct {
	...
	connectFunc           func(ctx context.Context, addr ma.Multiaddr) (address *aisc.Address, err error)
	disconnectFunc        func(overlay  aisc.Address, reason string) error
	...
	blocklistFunc         func( aisc.Address, time.Duration, string) error
...
func WithConnectFunc(f func(ctx context.Context, addr ma.Multiaddr) (address *aisc.Address, err error)) Option {
	return optionFunc(func(s *Service) {
		s.connectFunc = f
	})
}
...
func WithDisconnectFunc(f func(overlay  aisc.Address, reason string) error) Option {
	return optionFunc(func(s *Service) {
		s.disconnectFunc = f
	})
}
...
func WithBlocklistFunc(f func( aisc.Address, time.Duration, string) error) Option {
	return optionFunc(func(s *Service) {
		s.blocklistFunc = f
	})
}
...
func (s *Service) Disconnect(overlay  aisc.Address, reason string) error {
