...
// across different protocols in Aisc.
package p2p
...
type Service interface {
	...
	Connect(ctx context.Context, addr ma.Multiaddr) (address *aisc.Address, err error)
	Disconnecter
	Peers() []Peer
	Blocklisted( aisc.Address) (bool, error)
...
type Disconnecter interface {
	Disconnect(overlay  aisc.Address, reason string) error
	Blocklister
}
...
type Blocklister interface {
	...
	Blocklist(overlay  aisc.Address, duration time.Duration, reason string) error
}
...
type ReachableNotifier interface {
	Reachable( aisc.Address, ReachabilityStatus)
}

type Reacher interface {
	Connected( aisc.Address, ma.Multiaddr)
	Disconnected( aisc.Address)
	Close() error
}
...
type Notifier interface {
	...
	Disconnected(Peer)
	Announce(ctx context.Context, peer  aisc.Address, fullnode bool) error
	AnnounceTo(ctx context.Context, addressee, peer  aisc.Address, fullnode bool) error
}
...
type Streamer interface {
	NewStream(ctx context.Context, address  aisc.Address, h Headers, protocol, version, stream string) (Stream, error)
}
...
// Pinger interface is used to ping a underlay address which is not yet known to the aisc node.
...
// headers.
type HeadlerFunc func(Headers,  aisc.Address) Headers
...
// NewAiscStreamName constructs a libp2p compatible stream name out of
// protocol name and version and stream name.
func NewAiscStreamName(protocol, version, stream string) string {
	return "/ aisc/" + protocol + "/" + version + "/" + stream
}
