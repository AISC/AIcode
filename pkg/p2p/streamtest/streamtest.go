...
type Recorder struct {
	...
	streamErr          func( aisc.Address, string, string, string) error
...
func WithBaseAddr(a  aisc.Address) Option {
	return optionFunc(func(r *Recorder) {
		r.base = a
	})
}
...
func WithStreamError(streamErr func( aisc.Address, string, string, string) error) Option {
	return optionFunc(func(r *Recorder) {
...
func (r *Recorder) NewStream(ctx context.Context, addr  aisc.Address, h p2p.Headers, protocolName, protocolVersion, streamName string) (p2p.Stream, error) {
	if r.streamErr != nil {
	...
	id := addr.String() + p2p.NewAiscStreamName(protocolName, protocolVersion, streamName)
...
func (r *Recorder) Records(addr  aisc.Address, protocolName, protocolVersio, streamName string) ([]*Record, error) {
	id := addr.String() + p2p.NewAiscStreamName(protocolName, protocolVersio, streamName)
...
func (r *Recorder) WaitRecords(t *testing.T, addr  aisc.Address, proto, version, stream string, msgs, timeoutSec int) []*Record {
	t.Helper()
...
func (r *RecorderDisconnecter) Disconnect(overlay  aisc.Address, _ string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *RecorderDisconnecter) Blocklist(overlay  aisc.Address, d time.Duration, _ string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *RecorderDisconnecter) IsDisconnected(overlay  aisc.Address) bool {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *RecorderDisconnecter) IsBlocklisted(overlay  aisc.Address) (bool, time.Duration) {
	r.mu.RLock()
	defer r.mu.RUnlock()
