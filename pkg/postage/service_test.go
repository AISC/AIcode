...
import (
	...
	pstoremock "github.com/aisc/pkg/postage/batchstore/mock"
...
func TestSetExpired(t *testing.T) {
	...
	batch :=  aisc.RandAddress(t).Bytes()
	notExistsBatch :=  aisc.RandAddress(t).Bytes()
	...
	itemExists := postage.NewStampItem().WithChunkAddress( aisc.RandAddress(t)).WithBatchID(batch)
	err = store.Put(itemExists)
	if err != nil {
		t.Fatal(err)
	}

	itemNotExists := postage.NewStampItem().WithChunkAddress( aisc.RandAddress(t)).WithBatchID(notExistsBatch)
