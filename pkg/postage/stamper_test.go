...
func TestStamperStamping(t *testing.T) {
	...
	createStamp := func(t *testing.T, stamper postage.Stamper) ( aisc.Address, *postage.Stamp) {
		t.Helper()

		chunkAddr :=  aisc.RandAddress(t)
	...
	t.Run("bucket mismatch", func(t *testing.T) {
		...
		a := chunkAddr.Bytes()
		a[0] ^= 0xff
		if err := stamp.Valid( aisc.NewAddress(a), owner, 12, 8, true); !errors.Is(err, postage.ErrBucketMismatch) {
	...
	t.Run("invalid index", func(t *testing.T) {
		...
		for i := 0; i < 14; i++ {
			_, err = stamper.Stamp( aisc.RandAddressAt(t, chunkAddr, 8))
			if err != nil {
				t.Fatalf("error adding stamp at step %d: %v", i, err)
			}
		}
		stamp, err := stamper.Stamp( aisc.RandAddressAt(t, chunkAddr, 8))
		if err != nil {
	...
	t.Run("bucket full", func(t *testing.T) {
		...
		if _, err = stamper.Stamp( aisc.RandAddressAt(t, chunkAddr, 8)); !errors.Is(err, postage.ErrBucketFull) {
