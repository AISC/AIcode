...
const stampItemSize =  aisc.HashSize +  aisc.HashSize +  aisc.StampIndexSize +  aisc.StampTimestampSize
...
func (si StampItem) Marshal() ([]byte, error) {
	switch {
	case len(si.BatchID) !=  aisc.HashSize:
		return nil, errStampItemMarshalBatchIDInvalid
	case len(si.chunkAddress.Bytes()) !=  aisc.HashSize:
	...
	l := 0
	copy(buf[l:l+ aisc.HashSize], si.BatchID)
	l +=  aisc.HashSize
	copy(buf[l:l+ aisc.HashSize], si.chunkAddress.Bytes())
	l +=  aisc.HashSize
	copy(buf[l:l+ aisc.StampIndexSize], si.BatchIndex)
	l +=  aisc.StampIndexSize
	copy(buf[l:l+ aisc.StampTimestampSize], si.BatchTimestamp)
...
func (si *StampItem) Unmarshal(bytes []byte) error {
	...
	l +=  aisc.HashSize
	ni.chunkAddress =  aisc.NewAddress(bytes[l : l+ aisc.HashSize])
	l +=  aisc.HashSize
	ni.BatchIndex = append(make([]byte, 0,  aisc.StampIndexSize), bytes[l:l+ aisc.StampIndexSize]...)
	l +=  aisc.StampIndexSize
	ni.BatchTimestamp = append(make([]byte, 0,  aisc.StampTimestampSize), bytes[l:l+ aisc.StampTimestampSize]...)
...
func (si *StampIssuer) increment(addr  aisc.Address) (batchIndex []byte, batchTimestamp []byte, err error) {
...
// toBucket calculates the index of the collision bucket for a  aisc address
// bucket index := collision bucket depth number of bits as bigendian uint32
func toBucket(depth uint8, addr  aisc.Address) uint32 {
