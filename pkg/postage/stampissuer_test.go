...
func TestStampItem(t *testing.T) {
	...
	tests := []struct {
		...
		test: &storagetest.ItemMarshalAndUnmarshalTest{
			Item:       postage.NewStampItem().WithBatchID([]byte{ aisc.HashSize - 1: 9}),
			Factory:    func() storage.Item { return postage.NewStampItem() },
		...
		test: &storagetest.ItemMarshalAndUnmarshalTest{
			Item: postage.NewStampItem().
				WithBatchID([]byte{ aisc.HashSize - 1: 9}).
				WithChunkAddress( aisc.RandAddress(t)).
				WithBatchIndex([]byte{ aisc.StampIndexSize - 1: 9}).
				WithBatchTimestamp([]byte{ aisc.StampTimestampSize - 1: 9}),
			Factory: func() storage.Item { return postage.NewStampItem() },
			CmpOpts: []cmp.Option{cmp.AllowUnexported(postage.StampItem{})},
		},
	}, {
		...
		test: &storagetest.ItemMarshalAndUnmarshalTest{
			Item: postage.NewStampItem().
				...
				WithChunkAddress( aisc.NewAddress(storagetest.MaxAddressBytes[:])).
...
func Test_StampIssuer_inc(t *testing.T) {
	...
	addr :=  aisc.NewAddress([]byte{1, 2, 3, 4})
...
func TestUtilization(t *testing.T) {
	...
	for depth := uint8(17); depth < 25; depth++ {
		...
		for i := 0; i < 8; i++ {
			eg.Go(func() error {
				for {
					_, _, err := sti.Increment( aisc.RandAddress(t))
					if err != nil {
						return err
					}
