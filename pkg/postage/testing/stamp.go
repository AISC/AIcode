...
func MustNewValidSignature(signer crypto.Signer, addr  aisc.Address, id, index, timestamp []byte) []byte {
...
func MustNewValidStamp(signer crypto.Signer, addr  aisc.Address) *postage.Stamp {
	id := MustNewID()
	index := MustNewID()[:8]
