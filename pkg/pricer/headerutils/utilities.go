...
func MakePricingHeaders(chunkPrice uint64, addr  aisc.Address) (p2p.Headers, error) {
...
func MakePricingResponseHeaders(chunkPrice uint64, addr  aisc.Address, index uint8) (p2p.Headers, error) {
...
func ParsePricingHeaders(receivedHeaders p2p.Headers) ( aisc.Address, uint64, error) {

	target, err := ParseTargetHeader(receivedHeaders)
	if err != nil {
		return  aisc.ZeroAddress, 0, err
	}
...
func ParsePricingResponseHeaders(receivedHeaders p2p.Headers) ( aisc.Address, uint64, uint8, error) {
	target, err := ParseTargetHeader(receivedHeaders)
	if err != nil {
		return  aisc.ZeroAddress, 0, 0, err
	}
	price, err := ParsePriceHeader(receivedHeaders)
	if err != nil {
		return  aisc.ZeroAddress, 0, 0, err
...
func ParseTargetHeader(receivedHeaders p2p.Headers) ( aisc.Address, error) {
	if receivedHeaders[targetFieldName] == nil {
		return  aisc.ZeroAddress, ErrNoTargetHeader
	}

	target :=  aisc.NewAddress(receivedHeaders[targetFieldName])
