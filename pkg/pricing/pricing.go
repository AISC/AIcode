...
import (
	...
	"github.com/aisc/pkg/pricing/pb"
...
type Interface interface {
	AnnouncePaymentThreshold(ctx context.Context, peer  aisc.Address, paymentThreshold *big.Int) error
}

// PaymentThresholdObserver is used for being notified of payment threshold updates
type PaymentThresholdObserver interface {
	NotifyPaymentThreshold(peer  aisc.Address, paymentThreshold *big.Int) error
}
...
func (s *Service) AnnouncePaymentThreshold(ctx context.Context, peer  aisc.Address, paymentThreshold *big.Int) error {
	loggerV1 := s.logger.V(1).Register()
