...
import (
	...
	"github.com/aisc/pkg/encryption/elgamal"
...
const (
	...
	MaxPayloadSize =  aisc.ChunkSize - 3* aisc.HashSize
)
...
// message:
func Wrap(ctx context.Context, topic Topic, msg []byte, recipient *ecdsa.PublicKey, targets Targets) ( aisc.Chunk, error) {
	...
	enc, ephpub, err := elgamal.NewEncryptor(recipient, topic[:], 4032,  aisc.NewHasher)
	if err != nil {
		return nil, err
	}
	...
	f := func(nonce []byte) ( aisc.Chunk, error) {
		hash, err := h(nonce)
		if err != nil {
			return nil, err
		}
		...
		chunk :=  aisc.NewChunk( aisc.NewAddress(hash), append(hint, append(nonce, payload...)...))
		return chunk, nil
	}
...
func Unwrap(ctx context.Context, key *ecdsa.PrivateKey, chunk  aisc.Chunk, topics []Topic) (topic Topic, msg []byte, err error) {
...
func mine(ctx context.Context, odd bool, f func(nonce []byte) ( aisc.Chunk, error)) ( aisc.Chunk, error) {
...
func matchTopic(key *ecdsa.PrivateKey, pubkey *ecdsa.PublicKey, hint, topic []byte) (encryption.Decrypter, error) {
	dec, err := elgamal.NewDecrypter(key, pubkey, topic,  aisc.NewHasher)
	if err != nil {
		return nil, err
	}
