...
import (
	...
	"github.com/aisc/pkg/puller/intervalstore"
	"github.com/aisc/pkg/pullsync"
	"github.com/aisc/pkg/rate"
...
func New(
	...
	p := &Puller{
		...
		limiter:     ratelimit.NewLimiter(ratelimit.Every(time.Second/4), int( aisc.MaxBins)), // allows for 2 syncs per second, max bins bursts
	}
...
func (p *Puller) manage(ctx context.Context) {
	...
	onChange := func() {
		...
		_ = p.topology.EachConnectedPeerRev(func(addr  aisc.Address, po uint8) (stop, jumpToNext bool, err error) {
...
func (p *Puller) disconnectPeer(addr  aisc.Address) {
...
func (p *Puller) syncPeerBin(parentCtx context.Context, peer *syncPeer, bin uint8, cursor uint64) {
	...
	sync := func(isHistorical bool, address  aisc.Address, start uint64, bin uint8, done func()) {
...
func (p *Puller) addPeerInterval(peer  aisc.Address, bin uint8, start, end uint64) (err error) {
	p.intervalMtx.Lock()
...
func (p *Puller) getPeerEpoch(peer  aisc.Address) (uint64, error) {
	p.intervalMtx.Lock()
	defer p.intervalMtx.Unlock()
...
func (p *Puller) setPeerEpoch(peer  aisc.Address, epoch uint64) error {
	p.intervalMtx.Lock()
	defer p.intervalMtx.Unlock()
...
func (p *Puller) resetPeerIntervals(peer  aisc.Address) (err error) {
	p.intervalMtx.Lock()
	defer p.intervalMtx.Unlock()
...
func (p *Puller) nextPeerInterval(peer  aisc.Address, bin uint8) (uint64, error) {
	p.intervalMtx.Lock()
	defer p.intervalMtx.Unlock()
...
func (p *Puller) getOrCreateInterval(peer  aisc.Address, bin uint8) (*intervalstore.Intervals, error) {
...
func peerEpochKey(peer  aisc.Address) string {
	return fmt.Sprintf("%s_epoch_%s", intervalPrefix, peer.ByteString())
}

func peerIntervalKey(peer  aisc.Address, bin uint8) string {
...
func newSyncPeer(addr  aisc.Address, bins, po uint8) *syncPeer {
	return &syncPeer{
		address:        addr,
