...
func toID(a  aisc.Address, bin uint8, start uint64) string {
...
func (p *PullSyncMock) Sync(ctx context.Context, peer  aisc.Address, bin uint8, start uint64) (topmost uint64, count int, err error) {
...
func (p *PullSyncMock) GetCursors(_ context.Context, peer  aisc.Address) ([]uint64, uint64, error) {
	p.mtx.Lock()
	defer p.mtx.Unlock()
...
func (p *PullSyncMock) ResetCalls(peer  aisc.Address) {
	p.mtx.Lock()
	defer p.mtx.Unlock()
	p.syncCalls = nil
}

func (p *PullSyncMock) SyncCalls(peer  aisc.Address) (res []SyncReply) {
	p.mtx.Lock()
	defer p.mtx.Unlock()
...
func (p *PullSyncMock) CursorsCalls(peer  aisc.Address) bool {
	p.mtx.Lock()
	defer p.mtx.Unlock()
	return  aisc.ContainsAddress(p.getCursorsPeers, peer)
}
