...
import (
	...
	"github.com/aisc/pkg/pullsync/pb"
...
type Interface interface {
	...
	Sync(ctx context.Context, peer  aisc.Address, bin uint8, start uint64) (topmost uint64, count int, err error)
	// GetCursors retrieves all cursors from a downstream peer.
	GetCursors(ctx context.Context, peer  aisc.Address) ([]uint64, uint64, error)
}
...
// It returns the BinID of highest chunk that was synced from the given
// batch and the total number of chunks the downstream peer has sent.
func (s *Syncer) Sync(ctx context.Context, peer  aisc.Address, bin uint8, start uint64) (uint64, int, error) {
	...
	for i := 0; i < len(offer.Chunks); i++ {
		...
		if len(addr) !=  aisc.HashSize {
			return 0, 0, fmt.Errorf("inconsistent hash length")
		}

		a :=  aisc.NewAddress(addr)
	...
	chunksToPut := make([] aisc.Chunk, 0, ctr)
	...
	for ; ctr > 0; ctr-- {
		...
		addr :=  aisc.NewAddress(delivery.Address)
		if addr.Equal( aisc.ZeroAddress) {
			s.logger.Debug("received zero address chunk", "peer_address", peer)
			s.metrics.ReceivedZeroAddress.Inc()
			continue
		}

		newChunk :=  aisc.NewChunk(addr, delivery.Data)
		...
		if cac.Valid(chunk) {
			...
			s.logger.Debug("invalid cac/soc chunk", "error",  aisc.ErrInvalidChunk, "peer_address", peer, "chunk", chunk)
			chunkErr = errors.Join(chunkErr,  aisc.ErrInvalidChunk)
...
func (s *Syncer) collectAddrs(ctx context.Context, bin uint8, start uint64) ([]*storer.BinC, uint64, error) {
	...
	LOOP:
		for limit > 0 {
			...
			case c, ok := <-chC:
				if !ok {
					break LOOP // The stream has aiscn closed.
				}
...
func (s *Syncer) processWant(ctx context.Context, o *pb.Offer, w *pb.Want) ([] aisc.Chunk, error) {
	bv, err := bitvector.NewFromBytes(w.BitVector, len(o.Chunks))
	if err != nil {
		return nil, err
	}

	chunks := make([] aisc.Chunk, 0, len(o.Chunks))
	for i := 0; i < len(o.Chunks); i++ {
		if bv.Get(i) {
			ch := o.Chunks[i]
			addr :=  aisc.NewAddress(ch.Address)
			...
			if err != nil {
				...
				chunks = append(chunks,  aisc.NewChunk( aisc.ZeroAddress, nil))
...
func (s *Syncer) GetCursors(ctx context.Context, peer  aisc.Address) (retr []uint64, epoch uint64, err error) {
	loggerV2 := s.logger.V(2).Register()
