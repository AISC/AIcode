...
type PushSyncer interface {
	PushChunkToClosest(ctx context.Context, ch  aisc.Chunk) (*Receipt, error)
}
...
type Storer interface {
	...
	IsWithinStorageRadius( aisc.Address) bool
	StorageRadius() uint8
}
...
func (ps *PushSync) handler(ctx context.Context, p p2p.Peer, stream p2p.Stream) (err error) {
	...
	chunk :=  aisc.NewChunk( aisc.NewAddress(ch.Address), ch.Data)
...
func (ps *PushSync) PushChunkToClosest(ctx context.Context, ch  aisc.Chunk) (*Receipt, error) {
	...
	return &Receipt{
		Address:    aisc.NewAddress(r.Address),
...
func (ps *PushSync) pushToClosest(ctx context.Context, ch  aisc.Chunk, origin bool) (*pb.Receipt, error) {
	...
	for sentErrorsLeft > 0 {
		...
		case <-retryC:
			...
			if  aisc.Proximity(peer.Bytes(), ch.Address().Bytes()) >= ps.store.StorageRadius() {
...
func (ps *PushSync) closestPeer(chunkAddress  aisc.Address, origin bool) ( aisc.Address, error) {
...
func (ps *PushSync) push(parentCtx context.Context, resultChan chan<- receiptResult, peer  aisc.Address, ch  aisc.Chunk, action accounting.Action) {
...
func (ps *PushSync) pushChunkToPeer(ctx context.Context, peer  aisc.Address, ch  aisc.Chunk) (receipt *pb.Receipt, err error) {
	...
	if !ch.Address().Equal( aisc.NewAddress(rec.Address)) {
...
func (ps *PushSync) prepareCredit(ctx context.Context, peer  aisc.Address, ch  aisc.Chunk, origin bool) (accounting.Action, error) {
...
func (ps *PushSync) validStampWrapper(f postage.ValidStampFn) postage.ValidStampFn {
	return func(c  aisc.Chunk) ( aisc.Chunk, error) {
		t := time.Now()
		chunk, err := f(c)
		if err != nil {
