...
type testGetter struct {
	...
	addresses  [17] aisc.Address
	latencies  [17]time.Duration
}

func (tg *testGetter) Get(ctx context.Context, addr  aisc.Address) (ch  aisc.Chunk, err error) {
...
func newTestGetter(ch  aisc.Chunk, firstFound int, errf func(int) chan struct{}) *testGetter {
	return &testGetter{
		ch:         ch,
		errf:       errf,
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.found <= tc.count {
				...
				t.Run("returns correct error", func(t *testing.T) {
					if !errors.Is(err, replicas.ErrAiscageddon) {
						t.Fatalf("incorrect error. want Aiscageddon. got %v", err)
					}
