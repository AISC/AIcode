...
type testBasePutter struct {
	getErrors func(context.Context,  aisc.Address) error
	putErrors func(context.Context,  aisc.Address) error
	store     storage.ChunkStore
}

func (tbp *testBasePutter) Get(ctx context.Context, addr  aisc.Address) ( aisc.Chunk, error) {
...
func (tbp *testBasePutter) Put(ctx context.Context, ch  aisc.Chunk) error {
...
func TestPutter(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(fmt.Sprintf("redundancy:%d, size:%d", tc.level, tc.length), func(t *testing.T) {
			...
			orig := false
			_ = store.Iterate(ctx, func(chunk  aisc.Chunk) (stop bool, err error) {
	...
	t.Run("error handling", func(t *testing.T) {
		tcs := []struct {
			...
			{"put errors", 4, 4096, func(tbp *testBasePutter) *testBasePutter {
				...
				tbp.putErrors = func(ctx context.Context, _  aisc.Address) error {
