...
var (
	...
	// ErrNotImplemented denotes that the function has not aiscn implemented.
...
func (c *Client) Resolve(name string) (Address, error) {
	if c.resolveFn == nil {
		return  aisc.ZeroAddress, fmt.Errorf("resolveFn: %w", ErrNotImplemented)
	}

	hash, err := c.resolveFn(c.registry, common.HexToAddress(c.contractAddr), name)
	if err != nil {
		return  aisc.ZeroAddress, fmt.Errorf("%w: %w", err, ErrResolveFailed)
	}

	// Ensure that the content hash string is in a valid format, eg.
	// "aisc://<address>".
	if !strings.HasPrefix(hash,  aiscContentHashPrefix) {
		return  aisc.ZeroAddress, fmt.Errorf("check content hash prefix %s: %w", hash, resolver.ErrInvalidContentHash)
	}

	// Trim the prefix and try to parse the result as a aisc address.
	addr, err :=  aisc.ParseHexAddress(strings.TrimPrefix(hash,  aiscContentHashPrefix))
	if err != nil {
		return  aisc.ZeroAddress, fmt.Errorf("parse response hash %s: %w", hash, resolver.ErrInvalidContentHash)
	}
