...
type Interface interface {
	...
	// originates at the current node (i.e. no forwarding involved), the caller should use  aisc.ZeroAddress
	// as the value for sourcePeerAddress.
	RetrieveChunk(ctx context.Context, address, sourcePeerAddr  aisc.Address) (chunk  aisc.Chunk, err error)
}
...
type Service struct {
	...
	storer        Storer
	singleflight  singleflight.Group[string,  aisc.Chunk]
	logger        log.Logger
...
func (s *Service) RetrieveChunk(ctx context.Context, chunkAddr, sourcePeerAddr  aisc.Address) ( aisc.Chunk, error) {
	loggerV1 := s.logger
	...
	v, _, err := s.singleflight.Do(ctx, flightRoute, func(ctx context.Context) ( aisc.Chunk, error) {
		...
		for errorsLeft > 0 {
			...
			case <-retryC:
				...
				if errors.Is(err, topology.ErrNotFound) {
					if skip.PruneExpiresAfter(chunkAddr, overDraftRefresh) == 0 { //no overdraft peers, we have depleted ALL peers
						if inflight == 0 {
							loggerV1.Debug("no peers left", "chunk_address", chunkAddr, "errors_left", errorsLeft, "isOrigin", origin, "own_proximity",  aisc.Proximity(s.addr.Bytes(), chunkAddr.Bytes()), "error", err)
							return nil, err
						}
				...
				if radius, err := s.radiusFunc(); err == nil &&  aisc.Proximity(peer.Bytes(), chunkAddr.Bytes()) >= radius {
			...
			case res := <-resultC:
				...
				if res.err == nil {
					loggerV1.Debug("retrieved chunk", "chunk_address", chunkAddr, "peer_address", res.peer, "peer_proximity",  aisc.Proximity(res.peer.Bytes(), chunkAddr.Bytes()))
					return res.chunk, nil
				}

				loggerV1.Debug("failed to get chunk", "chunk_address", chunkAddr, "peer_address", res.peer,
					"peer_proximity",  aisc.Proximity(res.peer.Bytes(), chunkAddr.Bytes()), "error", res.err)
...
func (s *Service) retrieveChunk(ctx context.Context, quit chan struct{}, chunkAddr, peer  aisc.Address, result chan retrievalResult, action accounting.Action, isOrigin bool, span opentracing.Span) {
	...
	chunk =  aisc.NewChunk(chunkAddr, d.Data)
	if !cac.Valid(chunk) {
		if !soc.Valid(chunk) {
			s.metrics.InvalidChunkRetrieved.Inc()
			err =  aisc.ErrInvalidChunk
			return
		}
	}
...
func (s *Service) prepareCredit(ctx context.Context, peer, chunk  aisc.Address, origin bool) (accounting.Action, error) {
...
func (s *Service) closestPeer(addr  aisc.Address, skipPeers [] aisc.Address, allowUpstream bool) ( aisc.Address, error) {
	...
	if err != nil {
		return  aisc.Address{}, fmt.Errorf("distance compare addr %s closest %s base address %s: %w", addr.String(), closest.String(), s.addr.String(), err)
	}
	if !closer {
