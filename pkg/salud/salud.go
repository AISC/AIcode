...
type topologyDriver interface {
	UpdatePeerHealth(peer  aisc.Address, health bool, dur time.Duration)
	topology.PeerIterator
}

type peerStatus interface {
	PeerSnapshot(ctx context.Context, peer  aisc.Address) (*status.Snapshot, error)
}
...
func (s *service) salud(mode string, minPeersPerbin int, durPercentile float64, connsPercentile float64) {
	...
	var (
		...
		totaldur float64
		peers    []peer
		bins     [ aisc.MaxBins]int
	)

	_ = s.topology.EachConnectedPeer(func(addr  aisc.Address, bin uint8) (stop bool, jumpToNext bool, err error) {
		wg.Add(1)
		go func() {
			defer wg.Done()
...
func (s *service) radius(peers []peer) (uint8, uint8) {
	...
	var networkRadius [ aisc.MaxBins]int
	var nHoodRadius [ aisc.MaxBins]int

	for _, peer := range peers {
		if peer.status.StorageRadius < uint32( aisc.MaxBins) {
			if peer.neighbor {
