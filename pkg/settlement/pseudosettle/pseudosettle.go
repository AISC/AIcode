...
import (
	...
	pb "github.com/aisc/pkg/settlement/pseudosettle/pb"
...
func totalKey(peer  aisc.Address, prefix string) string {
	return fmt.Sprintf("%v%v", prefix, peer.String())
}

func totalKeyPeer(key []byte, prefix string) (peer  aisc.Address, err error) {
	k := string(key)
	...
	return  aisc.ParseHexAddress(split[1])
}
...
func (s *Service) peerAllowance(peer  aisc.Address, fullNode bool) (limit *big.Int, stamp int64, err error) {
