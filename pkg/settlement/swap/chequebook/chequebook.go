...
type Service interface {
	// Deposit starts depositing erc20 token into the chequebook. This returns once the transactions has aiscn broadcast.
	Deposit(ctx context.Context, amount *big.Int) (hash common.Hash, err error)
	// Withdraw starts withdrawing erc20 token from the chequebook. This returns once the transactions has aiscn broadcast.
...
func (s *service) Withdraw(ctx context.Context, amount *big.Int) (hash common.Hash, err error) {
	...
	request := &transaction.TxRequest{
		...
		GasLimit:    95000,
		Value:       big.NewInt(0),
		Description: fmt.Sprintf("chequebook withdrawal of %d  aisc", amount),
	}
