	...
	for {
		...
		if insufficientERC20 || insufficientETH {
			...
			if mod.Cmp(big.NewInt(0)) > 0 {
				// always round up the division as the aiscaar cannot handle decimals
			...
			ccfg, _ := chaincfg.GetByChainID(chainId)
			 aiscTokenName := ccfg.AiscTokenSymbol
			nativeTokenName := ccfg.NativeTokenSymbol

			if insufficientETH && insufficientERC20 {
				msg := fmt.Sprintf("cannot continue until there is at least min %s (for Gas) and at least min %s available on address", nativeTokenName,  aiscTokenName)
				logger.Warning(msg, "min_amount", neededETH, "min_aisc_amount", neededERC20, "address", overlayEthAddress)
				...
				msg := fmt.Sprintf("cannot continue until there is at least min %s available on address",  aiscTokenName)
				logger.Warning(msg, "min_amount", neededERC20, "address", overlayEthAddress)
			}
			if chainId == chaincfg.Testnet.ChainID {
				logger.Warning("learn how to fund your node by visiting our docs at https://docs.aisc.org/docs/installation/fund-your-node")
			}
			select {
			case <-time.After(balanceCheckBackoffDuration):
			case <-timeoutCtx.Done():
				if insufficientERC20 {
					return fmt.Errorf("insufficient %s for initial deposit",  aiscTokenName)
				} else {
