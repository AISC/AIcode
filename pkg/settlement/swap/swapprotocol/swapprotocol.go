...
import (
	...
	swap "github.com/aisc/pkg/settlement/swap/headers"
	"github.com/aisc/pkg/settlement/swap/priceoracle"
	"github.com/aisc/pkg/settlement/swap/swapprotocol/pb"
...
type Interface interface {
	...
	EmitCheque(ctx context.Context, peer  aisc.Address, beneficiary common.Address, amount *big.Int, issue IssueFunc) (balance *big.Int, err error)
}

// Swap is the interface the settlement layer should implement to receive cheques.
type Swap interface {
	// ReceiveCheque is called by the swap protocol if a cheque is received.
	ReceiveCheque(ctx context.Context, peer  aisc.Address, cheque *chequebook.SignedCheque, exchangeRate, deduction *big.Int) error
	// Handshake is called by the swap protocol when a handshake is received.
	Handshake(peer  aisc.Address, beneficiary common.Address) error
	GetDeductionForPeer(peer  aisc.Address) (bool, error)
	GetDeductionByPeer(peer  aisc.Address) (bool, error)
	AddDeductionByPeer(peer  aisc.Address) error
}
...
func (s *Service) headler(receivedHeaders p2p.Headers, peerAddress  aisc.Address) (returnHeaders p2p.Headers) {
...
func (s *Service) EmitCheque(ctx context.Context, peer  aisc.Address, beneficiary common.Address, amount *big.Int, issue IssueFunc) (balance *big.Int, err error) {
