...
func (l *List) Add(chunk, peer  aisc.Address, expire time.Duration) {
...
func (l *List) ChunkPeers(ch  aisc.Address) (peers [] aisc.Address) {
	l.mtx.Lock()
	defer l.mtx.Unlock()
	...
	if p, ok := l.skip[ch.ByteString()]; ok {
		for peer, exp := range p {
			if exp > now {
				peers = append(peers,  aisc.NewAddress([]byte(peer)))
			}
		}
	}
...
func (l *List) PruneExpiresAfter(ch  aisc.Address, d time.Duration) int {
	l.mtx.Lock()
	defer l.mtx.Unlock()
