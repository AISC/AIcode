...
func NewSigned(id ID, ch  aisc.Chunk, owner, sig []byte) (*SOC, error) {
	s := New(id, ch)
...
func (s *SOC) Address() ( aisc.Address, error) {
	if len(s.owner) != crypto.AddressSize {
		return  aisc.ZeroAddress, errInvalidAddress
	}
	return CreateAddress(s.id, s.owner)
}

// WrappedChunk returns the chunk wrapped by the SOC.
func (s *SOC) WrappedChunk()  aisc.Chunk {
	return s.chunk
}

// Chunk returns the SOC chunk.
func (s *SOC) Chunk() ( aisc.Chunk, error) {
	socAddress, err := s.Address()
	if err != nil {
		return nil, err
	}
	return  aisc.NewChunk(socAddress, s.toBytes()), nil
}
...
func (s *SOC) Sign(signer crypto.Signer) ( aisc.Chunk, error) {
	// create owner
...
// FromChunk recreates a SOC representation from  aisc.Chunk data.
func FromChunk(sch  aisc.Chunk) (*SOC, error) {
	chunkData := sch.Data()
	if len(chunkData) <  aisc.SocMinChunkSize {
	...
	s.id = chunkData[cursor: aisc.HashSize]
	cursor +=  aisc.HashSize

	s.signature = chunkData[cursor : cursor+ aisc.SocSignatureSize]
...
func CreateAddress(id ID, owner []byte) ( aisc.Address, error) {
	sum, err := hash(id, owner)
	if err != nil {
		return  aisc.ZeroAddress, err
	}
	return  aisc.NewAddress(sum), nil
}
