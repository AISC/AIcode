...
func TestNew(t *testing.T) {
	...
	spanBytes := make([]byte,  aisc.SpanSize)
	binary.LittleEndian.PutUint64(spanBytes, uint64(len(payload)))
	if !bytes.Equal(chunkData[: aisc.SpanSize], spanBytes) {
		t.Fatalf("span mismatch. got %x want %x", chunkData[: aisc.SpanSize], spanBytes)
	}

	if !bytes.Equal(chunkData[ aisc.SpanSize:], payload) {
		t.Fatalf("payload mismatch. got %x want %x", chunkData[ aisc.SpanSize:], payload)
	}
}
...
func TestReplica(t *testing.T) {
	...
	id := make([]byte,  aisc.HashSize)
	s, err := soc.NewSigned(id, ch,  aisc.ReplicasOwner, sig)
	if err != nil {
		t.Fatal(err)
	}
	...
	sch, err := soc.FromChunk( aisc.NewChunk( aisc.EmptyAddress, ch.Data()))
	if err != nil {
		t.Fatal(err)
	}
	ch, err = sch.Chunk()
...
func TestChunk(t *testing.T) {
	...
	expectedSOCAddress :=  aisc.NewAddress(sum)
	...
	cursor := 0
	if !bytes.Equal(chunkData[cursor: aisc.HashSize], id) {
		t.Fatalf("id mismatch. got %x want %x", chunkData[cursor: aisc.HashSize], id)
	}
	cursor +=  aisc.HashSize

	signature := chunkData[cursor : cursor+ aisc.SocSignatureSize]
	if !bytes.Equal(signature, sig) {
		t.Fatalf("signature mismatch. got %x want %x", signature, sig)
	}
	cursor +=  aisc.SocSignatureSize

	spanBytes := make([]byte,  aisc.SpanSize)
	binary.LittleEndian.PutUint64(spanBytes, uint64(len(payload)))
	if !bytes.Equal(chunkData[cursor:cursor+ aisc.SpanSize], spanBytes) {
		t.Fatalf("span mismatch. got %x want %x", chunkData[cursor:cursor+ aisc.SpanSize], spanBytes)
	}
	cursor +=  aisc.SpanSize
...
func TestFromChunk(t *testing.T) {
	...
	chunkAddress :=  aisc.MustParseHexAddress("2387e8e7d8a48c2a9339c97c1dc3461a9a7aa07e994c5cb8b38fd7c1b3e6ea48")
	ch :=  aisc.NewChunk(chunkAddress, chunkData)
