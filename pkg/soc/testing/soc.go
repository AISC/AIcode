...
type MockSOC struct {
	...
	Owner        []byte
	Signature    []byte
	WrappedChunk  aisc.Chunk
}

// Address returns the SOC address of the mocked SOC.
func (ms MockSOC) Address()  aisc.Address {
	addr, _ := soc.CreateAddress(ms.ID, ms.Owner)
	return addr
}

// Chunk returns the SOC chunk of the mocked SOC.
func (ms MockSOC) Chunk()  aisc.Chunk {
	return  aisc.NewChunk(ms.Address(), append(ms.ID, append(ms.Signature, ms.WrappedChunk.Data()...)...))
}
