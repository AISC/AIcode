...
type Getter interface {
	// Get a chunk by its  aisc.Address. Returns the chunk associated with
...
type Hasser interface {
	...
	Has(context.Context,  aisc.Address) (bool, error)
}
...
// Putter that calls f.
type PutterFunc func(context.Context,  aisc.Chunk) error

// Put calls f(ctx, chunk).
func (f PutterFunc) Put(ctx context.Context, chunk  aisc.Chunk) error {
	return f(ctx, chunk)
}

type GetterFunc func(context.Context,  aisc.Address) ( aisc.Chunk, error)

func (f GetterFunc) Get(ctx context.Context, address  aisc.Address) ( aisc.Chunk, error) {
	return f(ctx, address)
}

type IterateChunkFn func( aisc.Chunk) (stop bool, err error)
...
type PushSubscriber interface {
	SubscribePush(ctx context.Context) (c <-chan  aisc.Chunk, stop func())
}
...
type PushReporter interface {
	Report(context.Context,  aisc.Chunk, ChunkState) error
}
