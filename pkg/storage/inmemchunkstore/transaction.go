...
type TxChunkStore struct {
	*storage.TxChunkStoreBase
	...
	revOps storage.TxRevertOpStore[ aisc.Address,  aisc.Chunk]
}
...
func (s *TxChunkStore) Put(ctx context.Context, chunk  aisc.Chunk) (err error) {
	err = s.TxChunkStoreBase.Put(ctx, chunk)
	if err == nil {
		err = s.revOps.Append(&storage.TxRevertOp[ aisc.Address,  aisc.Chunk]{
...
func (s *TxChunkStore) Delete(ctx context.Context, addr  aisc.Address) error {
...
func (s *TxChunkStore) NewTx(state *storage.TxState) storage.TxChunkStore {
	...
	return &TxChunkStore{
		...
		revOps: storage.NewInMemTxRevertOpStore(
			map[storage.TxOpCode]storage.TxRevertFn[ aisc.Address,  aisc.Chunk]{
				storage.PutOp: func(address  aisc.Address, _  aisc.Chunk) error {
					return s.ChunkStore.Delete(context.Background(), address)
				},
				storage.DeleteOp: func(_  aisc.Address, chunk  aisc.Chunk) error {
...
func NewTxChunkStore(store storage.ChunkStore) *TxChunkStore {
	return &TxChunkStore{
		...
		revOps:           new(storage.NoOpTxRevertOpStore[ aisc.Address,  aisc.Chunk]),
	}
}
