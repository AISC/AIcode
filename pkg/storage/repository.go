...
type ChunkLocker func(chunk  aisc.Address) func()
...
func (s *syncChunkStore) Put(ctx context.Context, chunk  aisc.Chunk) error {
	unlock := s.locker(chunk.Address())
	defer unlock()
	return s.TxChunkStore.Put(ctx, chunk)
}

func (s *syncChunkStore) Delete(ctx context.Context, addr  aisc.Address) error {
