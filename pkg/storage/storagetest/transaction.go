...
// as expected after the transaction has aiscn committed or rolled back.
...
func initChunkStore(t *testing.T, store storage.ChunkStore, chunks ... aisc.Chunk) {
	t.Helper()
...
func checkTxChunkStoreFinishedTxInvariants(t *testing.T, store storage.TxChunkStore, want error) {
	...
	if _, have := store.Has(ctx,  aisc.ZeroAddress); !errors.Is(have, want) {
		t.Fatalf("Has(...):\n\thave: %v\n\twant: %v", have, want)
	}

	if have := store.Iterate(ctx, func(_  aisc.Chunk) (stop bool, err error) {
		return false, nil
	}); !errors.Is(have, want) {
		t.Fatalf("Iterate(...):\n\thave: %v\n\twant: %v", have, want)
	}
