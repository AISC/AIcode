// nolint:goheader
// Copyright 2019 The Aisc Authors
// This file is part of the Aisc library.
//
// The Aisc library is free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// The Aisc library is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU Lesser General Public License for more details.
//
// You should have received a copy of the GNU Lesser General Public License
// along with the Aisc library. If not, see <http://www.gnu.org/licenses/>.
...
func GenerateTestRandomChunk()  aisc.Chunk {
...
func GenerateTestRandomSoChunk(tb testing.TB, cac  aisc.Chunk)  aisc.Chunk {
	tb.Helper()

	key, _ := crypto.GenerateSecp256k1Key()
	signer := crypto.NewDefaultSigner(key)
	id := testutil.RandBytes(tb,  aisc.HashSize)
...
// addressed chunk.
func GenerateTestRandomInvalidChunk()  aisc.Chunk {
	data := make([]byte,  aisc.ChunkSize)
	_, _ = rand.Read(data)
	key := make([]byte,  aisc.SectionSize)
	_, _ = rand.Read(key)
	stamp := postagetesting.MustNewStamp()
	return  aisc.NewChunk( aisc.NewAddress(key), data).WithStamp(stamp)
}

// GenerateTestRandomChunks generates a slice of random
// Chunks by using GenerateTestRandomChunk function.
func GenerateTestRandomChunks(count int) [] aisc.Chunk {
	chunks := make([] aisc.Chunk, count)
	for i := 0; i < count; i++ {
...
func GenerateTestRandomChunkAt(tb testing.TB, target  aisc.Address, po int)  aisc.Chunk {
	tb.Helper()

	data := make([]byte,  aisc.ChunkSize)
	_, _ = rand.Read(data)
	addr :=  aisc.RandAddressAt(tb, target, po)
	stamp := postagetesting.MustNewStamp()
	return  aisc.NewChunk(addr, data).WithStamp(stamp)
}

// GenerateTestRandomChunkAt generates an invalid (!) chunk with address of proximity order po wrt target.
func GenerateValidRandomChunkAt(target  aisc.Address, po int)  aisc.Chunk {
	data := make([]byte,  aisc.ChunkSize)
	...
	for {
		...
		if  aisc.Proximity(ch.Address().Bytes(), target.Bytes()) > uint8(po) {
			break
		}
	}
...
func FixtureChunk(prefix string)  aisc.Chunk {
	c, ok := fixtureChunks()[prefix]
	if !ok {
		panic("no fixture found")
	}
	return c.WithStamp(postagetesting.MustNewStamp())
}
...
func fixtureChunks() map[string] aisc.Chunk {
	return map[string] aisc.Chunk{
		"0025":  aisc.NewChunk(
			 aisc.MustParseHexAddress("0025737be11979e91654dffd2be817ac1e52a2dadb08c97a7cef12f937e707bc"),
		...
		"0033":  aisc.NewChunk(
			 aisc.MustParseHexAddress("0033153ac8cfb0c343db1795f578c15ed8ef827f3e68ed3c58329900bf0d7276"),
		...
		"02c2":  aisc.NewChunk(
			 aisc.MustParseHexAddress("02c2bd0db71efb7d245eafcc1c126189c1f598feb80e8f14e7ecef913c6a2ef5"),
		...
		"7000":  aisc.NewChunk(
			 aisc.MustParseHexAddress("70002115a015d40a1f5ef68c29d072f06fae58854934c1cb399fcb63cf336127"),
