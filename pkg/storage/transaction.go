...
// a transaction that has already aiscn committed or rolled back.
var ErrTxDone = errors.New("storage: transaction has already aiscn committed or rolled back")
...
// IsDone returns ErrTxDone if the transaction has already aiscn committed
...
// transaction has already aiscn committed or rolled back or if the transaction
...
func (s *TxChunkStoreBase) Get(ctx context.Context, address  aisc.Address) ( aisc.Chunk, error) {
	if err := s.IsDone(); err != nil {
		return nil, err
	}
...
func (s *TxChunkStoreBase) Put(ctx context.Context, chunk  aisc.Chunk) error {
	if err := s.IsDone(); err != nil {
		return err
	}
...
func (s *TxChunkStoreBase) Has(ctx context.Context, address  aisc.Address) (bool, error) {
	if err := s.IsDone(); err != nil {
		return false, err
	}
...
func (s *TxChunkStoreBase) Delete(ctx context.Context, address  aisc.Address) error {
	if err := s.IsDone(); err != nil {
		return err
	}
