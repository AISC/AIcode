...
func spanOffset(sampleItem storer.SampleItem) uint8 {
	ch :=  aisc.NewChunk(sampleItem.ChunkAddress, sampleItem.ChunkData)
	if soc.Valid(ch) {
		return  aisc.HashSize +  aisc.SocSignatureSize
	}
...
func makeInclusionProofs(
	...
	prefixHasherFactory := func() hash.Hash {
		return  aisc.NewPrefixHasher(anchor1)
	}
	prefixHasherPool := bmt.NewPool(bmt.NewConf(prefixHasherFactory,  aisc.BmtBranches, 8))

	// Sample chunk proofs
	rccontent := bmt.Prover{Hasher: bmtpool.Get()}
	rccontent.SetHeaderInt64( aisc.HashSize * storer.SampleSize * 2)
	rsc, err := sampleChunk(reserveSampleItems)
	if err != nil {
		return redistribution.ChunkInclusionProofs{}, errProofCreation
	}
	rscData := rsc.Data()
	_, err = rccontent.Write(rscData[ aisc.SpanSize:])
	if err != nil {
		return redistribution.ChunkInclusionProofs{}, errProofCreation
	}
	...
	chunk1Content.SetHeader(reserveSampleItems[require1].ChunkData[chunk1Offset : chunk1Offset+ aisc.SpanSize])
	chunk1ContentPayload := reserveSampleItems[require1].ChunkData[chunk1Offset+ aisc.SpanSize:]
	...
	chunk1TrContent.SetHeader(reserveSampleItems[require1].ChunkData[chunk1Offset : chunk1Offset+ aisc.SpanSize])
	...
	chunk2ContentPayload := reserveSampleItems[require2].ChunkData[chunk2Offset+ aisc.SpanSize:]
	chunk2Content.SetHeader(reserveSampleItems[require2].ChunkData[chunk2Offset : chunk2Offset+ aisc.SpanSize])
	...
	chunk2TrContent.SetHeader(reserveSampleItems[require2].ChunkData[chunk2Offset : chunk2Offset+ aisc.SpanSize])
	...
	chunkLastContent.SetHeader(reserveSampleItems[require3].ChunkData[chunkLastOffset : chunkLastOffset+ aisc.SpanSize])
	chunkLastContentPayload := reserveSampleItems[require3].ChunkData[chunkLastOffset+ aisc.SpanSize:]
	...
	chunkLastTrContent.SetHeader(reserveSampleItems[require3].ChunkData[chunkLastOffset : chunkLastOffset+ aisc.SpanSize])
...
func sampleChunk(items []storer.SampleItem) ( aisc.Chunk, error) {
	contentSize := len(items) * 2 *  aisc.HashSize
...
func sampleHash(items []storer.SampleItem) ( aisc.Address, error) {
