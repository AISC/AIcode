...
func TestMakeInclusionProofsRegression_FLAKY(t *testing.T) {
	...
	stamper := func(addr  aisc.Address) *postage.Stamp {
	...
	sampleChunks := make([] aisc.Chunk, 0, sampleSize)
	...
	if want :=  aisc.MustParseHexAddress("193bbea3dd0656d813c2c1e27b821f141286bbe6ab0dbf8e26fc7dd491e8f921"); !sch.Address().Equal(want) {
...
func TestSampleChunk(t *testing.T) {
	...
	for i := 0; i < len(sample.Items); i++ {
		...
		data := chunk.Data()[ aisc.SpanSize:]
		pos := 0
		for _, item := range items {
			if !bytes.Equal(data[pos:pos+ aisc.HashSize], item.ChunkAddress.Bytes()) {
				t.Error("expected chunk address")
			}
			pos +=  aisc.HashSize

			if !bytes.Equal(data[pos:pos+ aisc.HashSize], item.TransformedAddress.Bytes()) {
