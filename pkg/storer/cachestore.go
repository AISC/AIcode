...
func (db *DB) Lookup() storage.Getter {
	return getterWithMetrics{
		storage.GetterFunc(func(ctx context.Context, address  aisc.Address) ( aisc.Chunk, error) {
...
func (db *DB) CacheShallowCopy(ctx context.Context, store internal.Storage, addrs ... aisc.Address) error {
	defer db.triggerCacheEviction()
	dur := captureDuration(time.Now())
