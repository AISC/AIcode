...
const cacheEntrySize =  aisc.HashSize + 8
...
func (c *Cache) Putter(store internal.Storage) storage.Putter {
	return storage.PutterFunc(func(ctx context.Context, chunk  aisc.Chunk) error {
...
func (c *Cache) Getter(store internal.Storage) storage.Getter {
	return storage.GetterFunc(func(ctx context.Context, address  aisc.Address) ( aisc.Chunk, error) {
...
func (c *Cache) ShallowCopy(
	...
	for _, addr := range addrs {
		...
		if has, err := store.IndexStore().Has(entry); err == nil && has {
			...
			// See https://github.com/aisc/issues/4530.
...
func (c *cacheEntry) Marshal() ([]byte, error) {
	...
	copy(entryBuf[: aisc.HashSize], c.Address.Bytes())
	binary.LittleEndian.PutUint64(entryBuf[ aisc.HashSize:], uint64(c.AccessTimestamp))
	return entryBuf, nil
}
...
func (c *cacheEntry) Unmarshal(buf []byte) error {
	...
	newEntry.Address =  aisc.NewAddress(append(make([]byte, 0,  aisc.HashSize), buf[: aisc.HashSize]...))
	newEntry.AccessTimestamp = int64(binary.LittleEndian.Uint64(buf[ aisc.HashSize:]))
	*c = *newEntry
	return nil
}
...
func keyFromID(ts int64, addr  aisc.Address) string {
	tsStr := fmt.Sprintf("%d", ts)
	return tsStr + addr.ByteString()
}

func idFromKey(key string) (int64,  aisc.Address, error) {
	ts := key[:len(key)- aisc.HashSize]
	addr := key[len(key)- aisc.HashSize:]
	n, err := strconv.ParseInt(ts, 10, 64)
	if err != nil {
		return 0,  aisc.ZeroAddress, err
	}
	return n,  aisc.NewAddress([]byte(addr)), nil
}
