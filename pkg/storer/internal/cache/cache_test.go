...
func TestCache(t *testing.T) {
	...
	t.Run("fresh new cache", func(t *testing.T) {
		...
		verifyCacheState(t, st.IndexStore(), c,  aisc.ZeroAddress,  aisc.ZeroAddress, 0)
	})
...
func TestShallowCopy(t *testing.T) {
	...
	chunks := chunktest.GenerateTestRandomChunks(10)
	chunksToMove := make([] aisc.Address, 0, 10)
	...
	chunksToMove1 := make([] aisc.Address, 0, 10)
...
func TestShallowCopyOverCap(t *testing.T) {
	...
	chunksToMove := make([] aisc.Address, 0, 15)
...
func verifyCacheOrder(
	...
	idx := 0
	err := c.IterateOldToNew(st, state.Head, state.Tail, func(entry  aisc.Address) (bool, error) {
