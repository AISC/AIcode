...
// Load returns first found  aisc.Stamp related to the given address.
// The storage.ErrNoStampsForChunk is returned if no record is found.
func Load(s storage.Reader, namespace string, addr  aisc.Address) ( aisc.Stamp, error) {
	return LoadWithBatchID(s, namespace, addr, nil)
}

// LoadWithBatchID returns  aisc.Stamp related to the given address and batchID.
// The storage.ErrNoStampsForChunk is returned if no record is found.
func LoadWithBatchID(s storage.Reader, namespace string, addr  aisc.Address, batchID []byte) ( aisc.Stamp, error) {
	var stamp  aisc.Stamp
...
// DeleteAll removes all  aisc.Stamp related to the given address.
func DeleteAll(s storage.Store, namespace string, addr  aisc.Address) error {
	var stamps [] aisc.Stamp
	err := s.Iterate(
		storage.Query{
			Factory: func() storage.Item {
				return &item{
...
func Delete(s storage.Store, batch storage.Writer, namespace string, addr  aisc.Address, batchId []byte) error {
