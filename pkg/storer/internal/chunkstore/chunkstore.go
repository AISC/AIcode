...
const RetrievalIndexItemSize =  aisc.HashSize + 8 + sharky.LocationSize + 4

var _ storage.Item = (*RetrievalIndexItem)(nil)

// RetrievalIndexItem is the index which gives us the sharky location from the  aisc.Address.
...
func (r *RetrievalIndexItem) Marshal() ([]byte, error) {
	...
	copy(buf[i: aisc.HashSize], r.Address.Bytes())
	i +=  aisc.HashSize
...
func (r *RetrievalIndexItem) Unmarshal(buf []byte) error {
	...
	ni.Address =  aisc.NewAddress(slices.Clone(buf[i : i+ aisc.HashSize]))
	i +=  aisc.HashSize
...
func (c *ChunkStoreWrapper) readChunk(ctx context.Context, rIdx *RetrievalIndexItem) ( aisc.Chunk, error) {
	...
	return  aisc.NewChunk(rIdx.Address, buf), nil
}

func (c *ChunkStoreWrapper) Get(ctx context.Context, addr  aisc.Address) ( aisc.Chunk, error) {
...
func (c *ChunkStoreWrapper) Has(_ context.Context, addr  aisc.Address) (bool, error) {
	return c.store.Has(&RetrievalIndexItem{Address: addr})
}

func (c *ChunkStoreWrapper) Put(ctx context.Context, ch  aisc.Chunk) error {
	var (
...
func (c *ChunkStoreWrapper) Delete(ctx context.Context, addr  aisc.Address) error {
	rIdx := &RetrievalIndexItem{Address: addr}
	err := c.store.Get(rIdx)
	switch {
	case errors.Is(err, storage.ErrNotFound):
...
func IterateChunkEntries(st storage.Store, fn func( aisc.Address, bool) (bool, error)) error {
	return st.Iterate(
		storage.Query{
			Factory: func() storage.Item { return new(RetrievalIndexItem) },
		},
