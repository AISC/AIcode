...
func TestChunkStore(t *testing.T) {
	...
	sharky, err := sharky.New(&memFS{Fs: afero.NewMemMapFs()}, 1,  aisc.SocMaxChunkSize)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("iterate chunks", func(t *testing.T) {
		count := 0
		err := st.Iterate(context.TODO(), func(_  aisc.Chunk) (bool, error) {
			count++
			return false, nil
		})
		if err != nil {
	...
	t.Run("iterate chunk entries", func(t *testing.T) {
		count, shared := 0, 0
		err := chunkstore.IterateChunkEntries(store, func(_  aisc.Address, isShared bool) (bool, error) {
			count++
			if isShared {
				shared++
			}
