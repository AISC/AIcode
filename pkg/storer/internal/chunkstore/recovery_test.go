...
func TestTxChunkStore_Recovery(t *testing.T) {
	...
	lessFn := func(i, j  aisc.Chunk) int { return i.Address().Compare(j.Address()) }
	...
	if err := txChunkStore.Iterate(
		context.TODO(),
		func(chunk  aisc.Chunk) (stop bool, err error) {
	...
	opts := cmpopts.SortSlices(func(i, j  aisc.Chunk) bool {
		return i.Address().Compare(j.Address()) < 0
	})
