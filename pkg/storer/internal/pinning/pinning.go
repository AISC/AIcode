...
var (
	...
	//  aisc.Address. Root reference has to be set.
...
// The Address could be an encrypted  aisc hash. This hash has the key to decrypt the
// collection.
...
func (p *pinCollectionItem) Unmarshal(buf []byte) error {
	...
	if bytes.Equal(buf[ aisc.HashSize:encryption.ReferenceSize], emptyKey) {
		ni.Addr =  aisc.NewAddress(buf[: aisc.HashSize]).Clone()
	} else {
		ni.Addr =  aisc.NewAddress(buf[:encryption.ReferenceSize]).Clone()
	}
	off := encryption.ReferenceSize
...
func (c *collectionPutter) Put(ctx context.Context, st internal.Storage, writer storage.Writer, ch  aisc.Chunk) error {
	c.mtx.Lock()
	defer c.mtx.Unlock()
...
func (c *collectionPutter) Close(st internal.Storage, writer storage.Writer, root  aisc.Address) error {
	if root.IsZero() {
...
func HasPin(st storage.Store, root  aisc.Address) (bool, error) {
	collection := &pinCollectionItem{Addr: root}
...
// collection.
func DeletePin(ctx context.Context, tx internal.TxExecutor, root  aisc.Address) error {
	collection := &pinCollectionItem{Addr: root}
	err := tx.Execute(context.Background(), func(s internal.Storage) error {
...
func IterateCollection(st storage.Store, root  aisc.Address, fn func(addr  aisc.Address) (bool, error)) error {
	collection := &pinCollectionItem{Addr: root}
