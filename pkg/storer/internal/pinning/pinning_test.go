...
import (
	...
	storagetest "github.com/aisc/pkg/storage/storagetest"
...
type pinningCollection struct {
	root          aisc.Chunk
	uniqueChunks [] aisc.Chunk
	dupChunks    [] aisc.Chunk
}
...
func TestPinStore(t *testing.T) {
	...
	t.Run("verify internal state", func(t *testing.T) {
		for _, tc := range tests {
			count := 0
			err := pinstore.IterateCollection(st.IndexStore(), tc.root.Address(), func(addr  aisc.Address) (bool, error) {
				count++
				return false, nil
			})
			if err != nil {
	...
	t.Run("zero address close", func(t *testing.T) {
		...
		err = putter.Close(st, st.IndexStore(),  aisc.ZeroAddress)
...
func TestPinCollectionItem(t *testing.T) {
	...
	tests := []struct {
		...
		test: &storagetest.ItemMarshalAndUnmarshalTest{
			Item: &pinstore.PinCollectionItem{
				Addr:  aisc.NewAddress(storagetest.MinAddressBytes[:]),
			},
			Factory:    func() storage.Item { return new(pinstore.PinCollectionItem) },
		...
		test: &storagetest.ItemMarshalAndUnmarshalTest{
			Item: &pinstore.PinCollectionItem{
				Addr:  aisc.NewAddress(storagetest.MaxEncryptedRefBytes[:]),
				UUID: pinstore.NewUUID(),
