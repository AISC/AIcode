...
const batchRadiusItemSize = 1 +  aisc.HashSize +  aisc.HashSize + 8
...
func (b *BatchRadiusItem) Marshal() ([]byte, error) {
	...
	copy(buf[i:i+ aisc.HashSize], b.BatchID)
	i +=  aisc.HashSize

	copy(buf[i:i+ aisc.HashSize], b.Address.Bytes())
	i +=  aisc.HashSize
...
func (b *BatchRadiusItem) Unmarshal(buf []byte) error {
	...
	b.BatchID = copyBytes(buf[i : i+ aisc.HashSize])
	i +=  aisc.HashSize

	b.Address =  aisc.NewAddress(buf[i : i+ aisc.HashSize]).Clone()
	i +=  aisc.HashSize
...
const chunkBinItemSize = 1 + 8 +  aisc.HashSize +  aisc.HashSize + 1
...
func (c *ChunkBinItem) Marshal() ([]byte, error) {
	...
	copy(buf[i:i+ aisc.HashSize], c.Address.Bytes())
	i +=  aisc.HashSize

	copy(buf[i:i+ aisc.HashSize], c.BatchID)
	i +=  aisc.HashSize
...
func (c *ChunkBinItem) Unmarshal(buf []byte) error {
	...
	c.Address =  aisc.NewAddress(buf[i : i+ aisc.HashSize]).Clone()
	i +=  aisc.HashSize

	c.BatchID = copyBytes(buf[i : i+ aisc.HashSize])
	i +=  aisc.HashSize

	c.ChunkType =  aisc.ChunkType(buf[i])

	return nil
}

// BinItem stores the latest binIDs for each bin between 0 and  aisc.MaxBins
type BinItem struct {
	Bin   uint8
	BinID uint64
}
...
func ChunkType(ch  aisc.Chunk)  aisc.ChunkType {
	if cac.Valid(ch) {
		return  aisc.ChunkTypeContentAddressed
	} else if soc.Valid(ch) {
		return  aisc.ChunkTypeSingleOwner
	}
	return  aisc.ChunkTypeUnspecified
}
