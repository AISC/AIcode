...
import (
	...
	"github.com/aisc/pkg/storer/internal/stampindex"
...
type Reserve struct {
	...
	cacheCb  func(context.Context, internal.Storage, ... aisc.Address) error
...
func New(
	...
	radiusSetter topology.SetStorageRadiuser,
	logger log.Logger,
	cb func(context.Context, internal.Storage, ... aisc.Address) error,
) (*Reserve, error) {
...
func (r *Reserve) Put(ctx context.Context, store internal.Storage, chunk  aisc.Chunk) (bool, error) {
	indexStore := store.IndexStore()
	chunkStore := store.ChunkStore()

	po :=  aisc.Proximity(r.baseAddr.Bytes(), chunk.Address().Bytes())
	...
	if loaded {
		...
		// An older and different chunk with the same batchID and stamp index has aiscn previously
...
func (r *Reserve) Has(store storage.Store, addr  aisc.Address, batchID []byte) (bool, error) {
	item := &BatchRadiusItem{Bin:  aisc.Proximity(r.baseAddr.Bytes(), addr.Bytes()), BatchID: batchID, Address: addr}
	return store.Has(item)
}

func (r *Reserve) Get(ctx context.Context, storage internal.Storage, addr  aisc.Address, batchID []byte) ( aisc.Chunk, error) {
	item := &BatchRadiusItem{Bin:  aisc.Proximity(r.baseAddr.Bytes(), addr.Bytes()), BatchID: batchID, Address: addr}
...
func (r *Reserve) IterateBin(store storage.Store, bin uint8, startBinID uint64, cb func( aisc.Address, uint64, []byte) (bool, error)) error {
...
func (r *Reserve) IterateChunks(store internal.Storage, startBin uint8, cb func( aisc.Chunk) (bool, error)) error {
...
func (r *Reserve) EvictBatchBin(
	...
	for i := 0; i < len(evicted); i += batchCnt {
		...
		moveToCache := make([] aisc.Address, 0, end-i)
...
func (r *Reserve) DeleteChunk(
	...
	item := &BatchRadiusItem{
		Bin:      aisc.Proximity(r.baseAddr.Bytes(), chunkAddress.Bytes()),
		BatchID: batchID,
	...
	if stamp != nil {
		errs = errors.Join(
			stampindex.Delete(
				batch,
				reserveNamespace,
				 aisc.NewChunk(item.Address, nil).WithStamp(stamp),
			),
