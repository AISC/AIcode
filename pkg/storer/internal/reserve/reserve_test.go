...
import (
	...
	kademlia "github.com/aisc/pkg/topology/mock"
)

func noopCacher(_ context.Context, _ internal.Storage, _ ... aisc.Address) error {
	return nil
}
...
func TestReserveChunkType(t *testing.T) {
	...
	err = ts.IndexStore().Iterate(storage.Query{
		...
		if item.ChunkType ==  aisc.ChunkTypeContentAddressed {
			storedChunksCA--
		} else if item.ChunkType ==  aisc.ChunkTypeSingleOwner {
			storedChunksSO--
		} else {
...
func TestReplaceOldIndex(t *testing.T) {
	...
	r, err := reserve.New(
		...
		0, kademlia.NewTopologyDriver(),
		log.Noop,
		func(ctx context.Context, st internal.Storage, addrs ... aisc.Address) error {
...
func TestEvict(t *testing.T) {
	...
	for i, ch := range chunks {
		...
		b :=  aisc.Proximity(baseAddr.Bytes(), ch.Address().Bytes())
...
func TestIterate(t *testing.T) {
	...
	t.Run("iterate bin", func(t *testing.T) {
		...
		var id uint64 = 1
		err := r.IterateBin(ts.IndexStore(), 1, 0, func(ch  aisc.Address, binID uint64, _ []byte) (bool, error) {
			if binID != id {
	...
	t.Run("iterate chunks", func(t *testing.T) {
		...
		count := 0
		err := r.IterateChunks(ts, 2, func(_  aisc.Chunk) (bool, error) {
			count++
			return false, nil
		})
		if err != nil {
...
func checkChunk(t *testing.T, s internal.Storage, ch  aisc.Chunk, gone bool) {
	t.Helper()
