...
import (
	...
	"github.com/aisc/pkg/storer/internal/chunkstamp"
...
const pushItemSize = 8 + 2* aisc.HashSize + 8
...
func (i pushItem) Marshal() ([]byte, error) {
	...
	if len(i.BatchID) !=  aisc.HashSize {
	...
	copy(buf[8+ aisc.HashSize:8+2* aisc.HashSize], i.BatchID)
	binary.LittleEndian.PutUint64(buf[8+2* aisc.HashSize:], i.TagID)
	return buf, nil
}
...
func (i *pushItem) Unmarshal(bytes []byte) error {
	...
	ni.Address =  aisc.NewAddress(append(make([]byte, 0,  aisc.HashSize), bytes[8:8+ aisc.HashSize]...))
	ni.BatchID = append(make([]byte, 0,  aisc.HashSize), bytes[8+ aisc.HashSize:8+2* aisc.HashSize]...)
	ni.TagID = binary.LittleEndian.Uint64(bytes[8+2* aisc.HashSize:])
	*i = *ni
	return nil
}
...
const tagItemSize =  aisc.HashSize + 7*8
...
type TagItem struct {
	...
	Address    aisc.Address //  aisc.Address associated with this tag
...
func (i TagItem) Marshal() ([]byte, error) {
	...
	if len(addrBytes) == encryption.ReferenceSize {
		// in case of encrypted reference we use the  aisc hash as the address and
		// avoid storing the encryption key
		addrBytes = addrBytes[: aisc.HashSize]
	}
	copy(buf[48:], addrBytes)
	binary.LittleEndian.PutUint64(buf[48+ aisc.HashSize:], uint64(i.StartedAt))
	return buf, nil
}
...
func (i *TagItem) Unmarshal(bytes []byte) error {
	...
	ni.Address = internal.AddressOrZero(bytes[48 : 48+ aisc.HashSize])
	ni.StartedAt = int64(binary.LittleEndian.Uint64(bytes[48+ aisc.HashSize:]))
	*i = *ni
	return nil
}
...
var (
	...
	// after the putter has aiscn closed.
...
func (u *uploadPutter) Put(ctx context.Context, s internal.Storage, writer storage.Writer, chunk  aisc.Chunk) error {
	if u.closed {
	...
	if err := chunkstamp.Store(writer, chunkStampNamespace, chunk); err != nil {
...
// with a  aisc reference. This can be useful while keeping track of uploads through
// the tags. It will update the tag. This will be filled with the Split and Seen count
// by the Putter.
func (u *uploadPutter) Close(s internal.Storage, writer storage.Writer, addr  aisc.Address) error {
	if u.closed {
		return nil
	}
...
func remove(st internal.Storage, writer storage.Writer, address  aisc.Address, batchID []byte) error {
	ui := &uploadItem{
		Address: address,
		BatchID: batchID,
	}
	...
	if ui.Synced == 0 {
		...
		err = chunkstamp.DeleteWithStamp(writer, chunkStampNamespace, address, stamp)
		if err != nil {
...
func Iterate(ctx context.Context, s internal.Storage, consumerFn func(chunk  aisc.Chunk) (bool, error)) error {
...
func BatchIDForChunk(st storage.Store, addr  aisc.Address) ([]byte, error) {
	var batchID []byte
