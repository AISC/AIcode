...
func TestPushItem(t *testing.T) {
	...
	tests := []struct {
		...
		test: &storagetest.ItemMarshalAndUnmarshalTest{
			Item: &upload.PushItem{
				Timestamp: 1,
				Address:    aisc.NewAddress(storagetest.MinAddressBytes[:]),
				TagID:     1,
			},
			Factory:    func() storage.Item { return new(upload.PushItem) },
		...
		test: &storagetest.ItemMarshalAndUnmarshalTest{
			Item: &upload.PushItem{
				...
				Address:    aisc.NewAddress(storagetest.MaxAddressBytes[:]),
		...
		test: &storagetest.ItemMarshalAndUnmarshalTest{
			Item: &upload.PushItem{
				...
				BatchID:    aisc.RandAddress(t).Bytes(),
				TagID:     rand.Uint64(),
			},
			Factory: func() storage.Item { return new(upload.PushItem) },
		},
...
func TestUploadItem(t *testing.T) {
	...
	randomAddress :=  aisc.RandAddress(t)
	randomBatchId :=  aisc.RandAddress(t).Bytes()
...
func TestChunkPutter(t *testing.T) {
	...
	t.Run("iterate all", func(t *testing.T) {
		...
		err := ts.IndexStore().Iterate(
			...
			func(r storage.Result) (bool, error) {
				address :=  aisc.NewAddress([]byte(r.ID[:32]))
	...
	t.Run("close with reference", func(t *testing.T) {
		addr :=  aisc.RandAddress(t)

		err := putter.Close(ts, ts.IndexStore(), addr)
		if err != nil {
	...
	t.Run("error after close", func(t *testing.T) {
		err := putter.Put(context.Background(), ts, ts.IndexStore(), chunktest.GenerateTestRandomChunk())
...
func TestIterate(t *testing.T) {
	...
	t.Run("on empty storage does not call the callback fn", func(t *testing.T) {
		err := upload.Iterate(context.Background(), ts, func(chunk  aisc.Chunk) (bool, error) {
	...
	t.Run("iterates chunks", func(t *testing.T) {
		...
		err = putter.Put(context.Background(), ts, ts.IndexStore(), chunk1)
		if err != nil {
			t.Fatalf("session.Put(...): unexpected error: %v", err)
		}
		err = putter.Put(context.Background(), ts, ts.IndexStore(), chunk2)
		if err != nil {
			t.Fatalf("session.Put(...): unexpected error: %v", err)
		}
		...
		err = putter.Close(ts, ts.IndexStore(),  aisc.ZeroAddress)
		if err != nil {
...
func TestCleanup(t *testing.T) {
	...
	t.Run("cleanup putter", func(t *testing.T) {
		...
		putter, err := upload.NewPutter(ts, tag.TagID)
		if err != nil {
			t.Fatal("failed creating putter", err)
		}
		...
		count := 0
		_ = upload.Iterate(context.Background(), ts, func(chunk  aisc.Chunk) (bool, error) {
			count++
			return false, nil
		})
		if count != 0 {
