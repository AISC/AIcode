...
const oldRretrievalIndexItemSize =  aisc.HashSize + 8 + sharky.LocationSize + 1
...
// OldRetrievalIndexItem is the index which gives us the sharky location from the  aisc.Address.
...
func (r *OldRetrievalIndexItem) Marshal() ([]byte, error) {
	...
	binary.LittleEndian.PutUint64(buf[ aisc.HashSize:], r.Timestamp)
	copy(buf[ aisc.HashSize+8:], locBuf)
...
func (r *OldRetrievalIndexItem) Unmarshal(buf []byte) error {
	...
	if err := loc.UnmarshalBinary(buf[ aisc.HashSize+8:]); err != nil {
		return errUnmarshalInvalidRetrievalIndexLocationBytes
	}

	ni := new(OldRetrievalIndexItem)
	ni.Address =  aisc.NewAddress(append(make([]byte, 0,  aisc.HashSize), buf[: aisc.HashSize]...))
	ni.Timestamp = binary.LittleEndian.Uint64(buf[ aisc.HashSize:])
	ni.Location = *loc
