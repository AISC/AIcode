...
func WithChunks(chs ... aisc.Chunk) Option {
	return optionFunc(func(p *ReserveStore) {
		for _, c := range chs {
			c := c
			if c.Stamp() != nil {
...
func WithEvilChunk(addr  aisc.Address, ch  aisc.Chunk) Option {
	return optionFunc(func(p *ReserveStore) {
		p.evilAddr = addr
		p.evilChunk = ch
	})
...
func WithPutHook(f func( aisc.Chunk) error) Option {
	return optionFunc(func(p *ReserveStore) {
		p.putHook = f
	})
}
...
type ReserveStore struct {
	...
	chunks    map[string] aisc.Chunk
	evilAddr   aisc.Address
	evilChunk  aisc.Chunk
...
func NewReserve(opts ...Option) *ReserveStore {
	s := &ReserveStore{
		chunks: make(map[string] aisc.Chunk),
	}
	for _, v := range opts {
		v.apply(s)
	}
...
func (s *ReserveStore) IsWithinStorageRadius(addr  aisc.Address) bool        { return true }
...
// Get chunks.
func (s *ReserveStore) ReserveGet(ctx context.Context, addr  aisc.Address, batchID []byte) ( aisc.Chunk, error) {
...
func (s *ReserveStore) ReservePutter() storage.Putter {
	return storage.PutterFunc(
		func(ctx context.Context, c  aisc.Chunk) error {
			s.mtx.Lock()
			s.putCalls++
			s.mtx.Unlock()
			return s.put(ctx, c)
...
// Put chunks.
func (s *ReserveStore) put(_ context.Context, chs ... aisc.Chunk) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	for _, c := range chs {
		c := c
...
// Has chunks.
func (s *ReserveStore) ReserveHas(addr  aisc.Address, batchID []byte) (bool, error) {
