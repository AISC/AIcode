...
func (p *putterSession) Put(ctx context.Context, ch  aisc.Chunk) error {
	return p.chunkStore.Put(ctx, ch)
}

func (p *putterSession) Done(address  aisc.Address) error {
	if p.done != nil {
...
func (m *mockStorer) BatchHint(_  aisc.Address) ([]byte, error) {
	return postagetesting.MustNewID(), nil
}

func (m *mockStorer) DeletePin(_ context.Context, address  aisc.Address) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *mockStorer) Pins() ([] aisc.Address, error) {
	m.mu.Lock()
	defer m.mu.Unlock()

	pins := make([] aisc.Address, 0, len(m.pins))
...
func (m *mockStorer) HasPin(address  aisc.Address) (bool, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	for _, p := range m.pins {
		if p.Equal(address) {
...
func (m *mockStorer) DirectUpload() storer.PutterSession {
	return &putterSession{chunkStore: storage.PutterFunc(
		func(ctx context.Context, ch  aisc.Chunk) error {
...
func (m *mockStorer) IsWithinStorageRadius(_  aisc.Address) bool { return true }
