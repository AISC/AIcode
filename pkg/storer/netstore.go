...
func (db *DB) DirectUpload() PutterSession {
	...
	return &putterSession{
		Putter: putterWithMetrics{
			storage.PutterFunc(func(ctx context.Context, ch  aisc.Chunk) error {
		...
		done:    func( aisc.Address) error { return eg.Wait() },
...
func (db *DB) Download(cache bool) storage.Getter {
	return getterWithMetrics{
		storage.GetterFunc(func(ctx context.Context, address  aisc.Address) (ch  aisc.Chunk, err error) {
			...
			case errors.Is(err, storage.ErrNotFound):
				...
				if db.retrieval != nil {
					...
					ch, err = db.retrieval.RetrieveChunk(ctx, address,  aisc.ZeroAddress)
					if err == nil && cache {
						select {
						case <-ctx.Done():
						case <-db.quit:
