...
func (db *DB) evictExpiredBatches(ctx context.Context) error {
	...
	for _, batchID := range batches {
		evicted, err := db.evictBatch(ctx, batchID,  aisc.MaxBins)
		if err != nil {
			return err
		}
		if evicted > 0 {
...
func (db *DB) ReserveGet(ctx context.Context, addr  aisc.Address, batchID []byte) (chunk  aisc.Chunk, err error) {
...
func (db *DB) ReserveHas(addr  aisc.Address, batchID []byte) (has bool, err error) {
	dur := captureDuration(time.Now())
	defer func() {
...
func (db *DB) ReservePutter() storage.Putter {
	return putterWithMetrics{
		storage.PutterFunc(
			func(ctx context.Context, chunk  aisc.Chunk) (err error) {
...
func (db *DB) ReserveIterateChunks(cb func( aisc.Chunk) (bool, error)) error {
...
func (db *DB) IsWithinStorageRadius(addr  aisc.Address) bool {
	if db.reserve == nil {
		return false
	}
	return  aisc.Proximity(addr.Bytes(), db.baseAddr.Bytes()) >= db.reserve.Radius()
}
...
func (db *DB) SubscribeBin(ctx context.Context, bin uint8, start uint64) (<-chan *BinC, func(), <-chan error) {
	...
	go func() {
		...
		for {
			...
			err := db.reserve.IterateBin(db.repo.IndexStore(), bin, start, func(a  aisc.Address, binID uint64, batchID []byte) (bool, error) {
...
func (db *DB) po(addr  aisc.Address) uint8 {
	return  aisc.Proximity(db.baseAddr.Bytes(), addr.Bytes())
}
