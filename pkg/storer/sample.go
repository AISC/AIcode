...
import (
	...
	chunk "github.com/aisc/pkg/storage/testing"
...
type SampleItem struct {
	TransformedAddress  aisc.Address
...
func RandSample(t *testing.T, anchor []byte) Sample {
	...
	chunks := make([] aisc.Chunk, SampleSize)
...
func MakeSampleUsingChunks(chunks [] aisc.Chunk, anchor []byte) (Sample, error) {
	prefixHasherFactory := func() hash.Hash {
		return  aisc.NewPrefixHasher(anchor)
	}
	items := make([]SampleItem, len(chunks))
	for i, ch := range chunks {
		tr, err := transformedAddress(bmt.NewHasher(prefixHasherFactory), ch,  aisc.ChunkTypeContentAddressed)
		if err != nil {
			return Sample{}, err
		}
...
func newStamp(s  aisc.Stamp) *postage.Stamp {
...
func (db *DB) ReserveSample(
	...
	for i := 0; i < workers; i++ {
		g.Go(func() error {
			...
			for chItem := range chunkC {
				...
				if chItem.Type !=  aisc.ChunkTypeSingleOwner &&
					chItem.Type !=  aisc.ChunkTypeContentAddressed {
					wstat.RogueChunk++
					continue
				}
	...
	insert := func(item SampleItem) {
		...
		for i, sItem := range sampleItems {
			if le(item.TransformedAddress, sItem.TransformedAddress) {
				...
				ch :=  aisc.NewChunk(item.ChunkAddress, item.ChunkData)
	...
	for item := range sampleItemChan {
		currentMaxAddr :=  aisc.EmptyAddress
		...
		if le(item.TransformedAddress, currentMaxAddr) || len(sampleItems) < SampleSize {
			...
			ch :=  aisc.NewChunk(item.ChunkAddress, item.ChunkData).WithStamp(stamp)
...
func transformedAddress(hasher *bmt.Hasher, chunk  aisc.Chunk, chType  aisc.ChunkType) ( aisc.Address, error) {
	switch chType {
	case  aisc.ChunkTypeContentAddressed:
		return transformedAddressCAC(hasher, chunk)
	case  aisc.ChunkTypeSingleOwner:
		return transformedAddressSOC(hasher, chunk)
	default:
		return  aisc.ZeroAddress, fmt.Errorf("chunk type [%v] is not valid", chType)
	}
}

func transformedAddressCAC(hasher *bmt.Hasher, chunk  aisc.Chunk) ( aisc.Address, error) {
	hasher.Reset()
	...
	return  aisc.NewAddress(taddr), nil
}

func transformedAddressSOC(hasher *bmt.Hasher, chunk  aisc.Chunk) ( aisc.Address, error) {
	...
	return  aisc.NewAddress(sHasher.Sum(nil)), nil
}
