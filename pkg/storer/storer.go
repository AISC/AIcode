...
import (
	...
	"github.com/aisc/pkg/storer/internal/events"
...
type PutterSession interface {
	storage.Putter
	// Done is used to close the session and optionally assign a  aisc.Address to
	// this session.
	Done( aisc.Address) error
...
type UploadStore interface {
	...
	BatchHint( aisc.Address) ([]byte, error)
}
...
type PinIterator interface {
	IteratePinCollection(root  aisc.Address, iterateFn func( aisc.Address) (bool, error)) error
}
...
type ReserveIterator interface {
	ReserveIterateChunks(cb func( aisc.Chunk) (bool, error)) error
}

// ReserveStore is a logical component of the storer that deals with reserve
// content. It will implement all the core functionality required for the protocols.
type ReserveStore interface {
	ReserveGet(ctx context.Context, addr  aisc.Address, batchID []byte) ( aisc.Chunk, error)
	ReserveHas(addr  aisc.Address, batchID []byte) (bool, error)
...
type RadiusChecker interface {
	IsWithinStorageRadius(addr  aisc.Address) bool
	StorageRadius() uint8
}
...
func New(ctx context.Context, dirPath string, opts *Options) (*DB, error) {
	...
	locker := func(addr  aisc.Address) func() {
...
func (noopRetrieval) RetrieveChunk(_ context.Context, _  aisc.Address, _  aisc.Address) ( aisc.Chunk, error) {
...
type putterSession struct {
	storage.Putter
	done    func( aisc.Address) error
	cleanup func() error
}

func (p *putterSession) Done(addr  aisc.Address) error { return p.done(addr) }
