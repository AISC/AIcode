...
import (
	...
	cs "github.com/aisc/pkg/storer/internal/chunkstore"
...
func TestNew(t *testing.T) {
	...
	t.Run("disk default options", func(t *testing.T) {
		...
		lstore := makeDiskStorer(t, dbTestOps( aisc.RandAddress(t), 0, nil, nil, time.Second))
		if lstore == nil {
			t.Fatalf("storer should be instantiated")
		}
	})
	...
	t.Run("migration on latest version", func(t *testing.T) {
		...
		t.Run("inmem", func(t *testing.T) {
			...
			lstore := makeInmemStorer(t, dbTestOps( aisc.RandAddress(t), 0, nil, nil, time.Second))
...
func dbTestOps(baseAddr  aisc.Address, reserveCapacity int, bs postage.Storer, radiusSetter topology.SetStorageRadiuser, reserveWakeUpTime time.Duration) *storer.Options {
