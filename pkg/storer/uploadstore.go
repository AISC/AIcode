...
// with a transaction.
func (db *DB) Report(ctx context.Context, chunk  aisc.Chunk, state storage.ChunkState) error {
...
func (db *DB) Upload(ctx context.Context, pin bool, tagID uint64) (PutterSession, error) {
	...
	return &putterSession{
		Putter: putterWithMetrics{
			storage.PutterFunc(func(ctx context.Context, chunk  aisc.Chunk) error {
				db.lock.Lock(uploadStoreKey)
				defer db.lock.Unlock(uploadStoreKey)
...
func (db *DB) BatchHint(address  aisc.Address) ([]byte, error) {
