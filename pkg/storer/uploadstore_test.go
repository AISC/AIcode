...
import (
	...
	chunktesting "github.com/aisc/pkg/storage/testing"
...
func testUploadStore(t *testing.T, newStorer func() (*storer.DB, error)) {
	...
	t.Run("get session info", func(t *testing.T) {
		...
		verify := func(t *testing.T, info storer.SessionInfo, id, split, seen uint64, addr  aisc.Address) {
			t.Helper()
			...
			if !info.Address.Equal(addr) {
				t.Fatalf("unexpected  aisc reference: want %s have %s", addr, info.Address)
			}
		}
		...
		t.Run("done", func(t *testing.T) {
			...
			verify(t, sessionInfo, tag.TagID, 0, 0,  aisc.ZeroAddress)
...
func TestListDeleteSessions(t *testing.T) {
	...
	t.Run("inmem", func(t *testing.T) {
		...
		testListDeleteSessions(t, func() (*storer.DB, error) {
			return storer.New(context.Background(), "", dbTestOps( aisc.RandAddress(t), 0, nil, nil, time.Second))
		})
	})
	...
	t.Run("disk", func(t *testing.T) {
		...
		testListDeleteSessions(t, diskStorer(t, dbTestOps( aisc.RandAddress(t), 0, nil, nil, time.Second)))
	})
}
...
func TestUploadStore(t *testing.T) {
	...
	t.Run("disk", func(t *testing.T) {
		...
		testUploadStore(t, diskStorer(t, dbTestOps( aisc.RandAddress(t), 0, nil, nil, time.Second)))
	})
}
...
func TestReporter(t *testing.T) {
	...
	t.Run("inmem", func(t *testing.T) {
		...
		testReporter(t, func() (*storer.DB, error) {
			...
			opts := dbTestOps( aisc.RandAddress(t), 0, nil, nil, time.Second)
