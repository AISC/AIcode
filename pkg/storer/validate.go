...
func Validate(ctx context.Context, basePath string, opts *Options) error {
	...
	sharky, err := sharky.New(&dirFS{basedir: path.Join(basePath, sharkyPath)},
		sharkyNoOfShards,  aisc.SocMaxChunkSize)
	if err != nil {
		return err
	}
	defer func() {
...
func validateWork(logger log.Logger, store storage.Store, readFn func(context.Context, sharky.Location, []byte) error) {
	...
	validChunk := func(item *chunkstore.RetrievalIndexItem, buf []byte) {
		...
		ch :=  aisc.NewChunk(item.Address, buf)
		if !cac.Valid(ch) {
			if soc.Valid(ch) {
				socCount++
				...
				h, err := cac.DoHash(buf[ aisc.SpanSize:], buf[: aisc.SpanSize])
				if err != nil {
					logger.Error(err, "cac hash")
					return
				}

				computedAddr :=  aisc.NewAddress(h)

				if !cac.Valid( aisc.NewChunk(computedAddr, buf)) {
	...
	for i := 0; i < 8; i++ {
		...
		go func() {
			defer wg.Done()
			buf := make([]byte,  aisc.SocMaxChunkSize)
...
func ValidatePinCollectionChunks(ctx context.Context, basePath, pin, location string, opts *Options) error {
	...
	sharky, err := sharky.New(fs, sharkyNoOfShards,  aisc.SocMaxChunkSize)
	if err != nil {
		return err
	}
	defer func() {
...
func (p *PinIntegrity) Check(ctx context.Context, logger log.Logger, pin string, out chan PinStat) {
	...
	var pins [] aisc.Address

	if pin != "" {
		addr, err :=  aisc.ParseHexAddress(pin)
		if err != nil {
	...
	for _, pin := range pins {
		...
		err := pinstore.IterateCollection(p.Store, pin, func(addr  aisc.Address) (bool, error) {
			n := time.Now()
