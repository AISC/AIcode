...
func Test_RandAddressAt(t *testing.T) {
	...
	base :=  aisc.MustParseHexAddress("ca1e9f3938cc1425c6061b96ad9eb93e134dfe8734ad490164ef20af9d1cf59c")
	b0 := base.Bytes()
	...
	for bitsInCommon := 0; bitsInCommon < 30; bitsInCommon++ {
		addr :=  aisc.RandAddressAt(t, base, bitsInCommon)
		assertNotZeroAddress(t, addr)
...
func Test_RandAddresses(t *testing.T) {
	...
	count := 20
	addrs :=  aisc.RandAddresses(t, count)
...
func assertNotZeroAddress(t *testing.T, addr  aisc.Address) {
	t.Helper()

	if got := len(addr.Bytes()); got !=  aisc.HashSize {
		t.Fatalf("expected %d, got %d",  aisc.HashSize, got)
	}
