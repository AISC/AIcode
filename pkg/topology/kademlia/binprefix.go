...
func generateCommonBinPrefixes(base  aisc.Address, suffixLength int) [][] aisc.Address {
	...
	binPrefixes := make([][] aisc.Address, int( aisc.MaxBins))

	// copy base address
	for i := range binPrefixes {
		binPrefixes[i] = make([] aisc.Address, bitCombinationsCount)
