...
type persistentCounters struct {
	PeerAddress        aisc.Address `json:"peerAddress"`
...
// peers specified be the  aisc.Address.
...
func (c *Collector) Record(addr  aisc.Address, rop ...RecordOp) {
...
// session.
func (c *Collector) Snapshot(t time.Time, addresses ... aisc.Address) map[string]*Snapshot {
...
func (c *Collector) IsUnreachable(addr  aisc.Address) bool {
	val, ok := c.counters.Load(addr.ByteString())
	if !ok {
		return true
	}
...
func (c *Collector) Filter(addr  aisc.Address, fop ...FilterOp) bool {
	val, ok := c.counters.Load(addr.ByteString())
	if !ok {
		return true
	}
...
func (c *Collector) Inspect(addr  aisc.Address) *Snapshot {
