...
func TestContainer(t *testing.T) {
	...
	t.Run("can add peers to container", func(t *testing.T) {
		...
		p1 :=  aisc.NewAddress([]byte("123"))
		p2 :=  aisc.NewAddress([]byte("456"))
		...
		i := 0
		peers := [] aisc.Address{p1, p2}
		if err = c.EachPeer(func(p  aisc.Address, _ uint8) (bool, bool, error) {
			if !p.Equal(peers[i]) {
	...
	t.Run("empty container after peer disconnect", func(t *testing.T) {
		...
		peer := p2p.Peer{Address:  aisc.NewAddress([]byte("123"))}
