...
type PSlice struct {
	peers     [][] aisc.Address // the slice of peers
	baseBytes []byte
	mu        sync.RWMutex
	maxBins   int
}
...
func New(maxBins int, base  aisc.Address) *PSlice {
	return &PSlice{
		peers:     make([][] aisc.Address, maxBins),
...
func (s *PSlice) Add(addrs ... aisc.Address) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	for i, count := range binChange {
		...
		if count > 0 && cap(peers) < len(peers)+count {
			newPeers := make([] aisc.Address, len(peers), len(peers)+count)
			copy(newPeers, peers)
			s.peers[i] = newPeers
		}
	}
...
func (s *PSlice) BinPeers(bin uint8) [] aisc.Address {
	...
	ret := make([] aisc.Address, len(s.peers[bin]))
...
func (s *PSlice) Exists(addr  aisc.Address) bool {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *PSlice) Remove(addr  aisc.Address) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	cpy := make([] aisc.Address, newLength)
...
func (s *PSlice) po(peer []byte) uint8 {
	po :=  aisc.Proximity(s.baseBytes, peer)
	if int(po) >= s.maxBins {
...
func (s *PSlice) index(addr  aisc.Address, po uint8) (bool, int) {
	idx :=  aisc.IndexOfAddress(s.peers[po], addr)
	if idx != -1 {
		return true, idx
	}
	return false, 0
