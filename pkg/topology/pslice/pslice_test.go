...
func TestShallowestEmpty(t *testing.T) {
	...
	var (
		...
		peers = make([][] aisc.Address, 16)
	)

	for i := 0; i < 16; i++ {
		for j := 0; j < 3; j++ {
			a :=  aisc.RandAddressAt(t, base, i)
...
func TestNoPanicOnEmptyRemove(t *testing.T) {
	...
	addr1 :=  aisc.RandAddressAt(t, base, 2)
	addr2 :=  aisc.RandAddressAt(t, base, 2)
...
func TestAddRemove(t *testing.T) {
	...
	var (
		...
		peers = make([] aisc.Address, 8)
	)

	// 2 peers per bin
	// indexes {0,1} {2,3} {4,5} {6,7}
	for i := 0; i < 8; i += 2 {
		peers[i] =  aisc.RandAddressAt(t, base, i/2)
		peers[i+1] =  aisc.RandAddressAt(t, base, i/2)
	}
	...
	chkNotExists(t, ps, append([] aisc.Address{peers[1]}, peers[4:]...)...)

	// this should not move the last cursor
	ps.Add(peers[7])
	chkLen(t, ps, 4)
	chkExists(t, ps, peers[0], peers[2], peers[3], peers[7])
	chkNotExists(t, ps, append([] aisc.Address{peers[1]}, peers[4:7]...)...)

	ps.Add(peers[5])
	chkLen(t, ps, 5)
	chkExists(t, ps, peers[0], peers[2], peers[3], peers[5], peers[7])
	chkNotExists(t, ps, [] aisc.Address{peers[1], peers[4], peers[6]}...)

	ps.Remove(peers[2])
	chkLen(t, ps, 4)
	chkExists(t, ps, peers[0], peers[3], peers[5], peers[7])
	chkNotExists(t, ps, [] aisc.Address{peers[1], peers[2], peers[4], peers[6]}...)
...
func TestIteratorError(t *testing.T) {
	...
	var (
		base =  aisc.RandAddress(t)
		ps   = pslice.New(4, base)
		a    =  aisc.RandAddressAt(t, base, 0)
		e    = errors.New("err1")
	)

	ps.Add(a)

	f := func(p  aisc.Address, _ uint8) (stop, jumpToNext bool, err error) {
...
func TestIterators(t *testing.T) {
	...
	peers := make([] aisc.Address, 4)
	for i := 0; i < 4; i++ {
		peers[i] =  aisc.RandAddressAt(t, base, i)
	}

	testIterator(t, ps, false, false, 0, [] aisc.Address{})
	testIteratorRev(t, ps, false, false, 0, [] aisc.Address{})

	for _, v := range peers {
		ps.Add(v)
	}

	testIterator(t, ps, false, false, 4, [] aisc.Address{peers[3], peers[2], peers[1], peers[0]})
	testIteratorRev(t, ps, false, false, 4, peers)

	ps.Remove(peers[2])
	testIterator(t, ps, false, false, 3, [] aisc.Address{peers[3], peers[1], peers[0]})
	testIteratorRev(t, ps, false, false, 3, [] aisc.Address{peers[0], peers[1], peers[3]})

	ps.Remove(peers[0])
	testIterator(t, ps, false, false, 2, [] aisc.Address{peers[3], peers[1]})
	testIteratorRev(t, ps, false, false, 2, [] aisc.Address{peers[1], peers[3]})

	ps.Remove(peers[3])
	testIterator(t, ps, false, false, 1, [] aisc.Address{peers[1]})
	testIteratorRev(t, ps, false, false, 1, [] aisc.Address{peers[1]})
...
func TestBinPeers(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.label, func(t *testing.T) {
			...
			binPeers := make([][] aisc.Address, len(tc.peersCount))

			// prepare slice
			ps := pslice.New(len(tc.peersCount), base)
			for bin, peersCount := range tc.peersCount {
				for i := 0; i < peersCount; i++ {
					peer :=  aisc.RandAddressAt(t, base, bin)
...
func isEqual(a, b [] aisc.Address) bool {
...
func TestIteratorsJumpStop(t *testing.T) {
	...
	testIterator(t, ps, true, false, 4, [] aisc.Address{peers[9], peers[6], peers[3], peers[0]})
	testIteratorRev(t, ps, true, false, 4, [] aisc.Address{peers[0], peers[3], peers[6], peers[9]})

	// // check that the stop functionality works correctly
	testIterator(t, ps, true, true, 1, [] aisc.Address{peers[9]})
	testIteratorRev(t, ps, true, true, 1, [] aisc.Address{peers[0]})

}

func testIteratorRev(t *testing.T, ps *pslice.PSlice, skipNext, stop bool, iterations int, peerseq [] aisc.Address) {
	t.Helper()
	i := 0
	f := func(p  aisc.Address, po uint8) (bool, bool, error) {
		if i == iterations {
...
func testIterator(t *testing.T, ps *pslice.PSlice, skipNext, stop bool, iterations int, peerseq [] aisc.Address) {
	t.Helper()
	i := 0
	f := func(p  aisc.Address, po uint8) (bool, bool, error) {
		if i == iterations {
...
func chkExists(t *testing.T, ps *pslice.PSlice, addrs ... aisc.Address) {
	t.Helper()
	for _, a := range addrs {
		if !ps.Exists(a) {
...
func chkNotExists(t *testing.T, ps *pslice.PSlice, addrs ... aisc.Address) {
	t.Helper()
	for _, a := range addrs {
		if ps.Exists(a) {
...
const (
	bins   = int( aisc.MaxBins)
	perBin = 1000
)

func BenchmarkAdd(b *testing.B) {
	base :=  aisc.RandAddress(b)
	ps := pslice.New(bins, base)

	addrs :=  aisc.RandAddresses(b, bins*perBin)
...
func BenchmarkEachBin(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		_ = ps.EachBin(func(a  aisc.Address, u uint8) (stop bool, jumpToNext bool, err error) {
...
func BenchmarkEachBinRev(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		_ = ps.EachBinRev(func(a  aisc.Address, u uint8) (stop bool, jumpToNext bool, err error) {
			return false, false, nil
		})
	}
}
