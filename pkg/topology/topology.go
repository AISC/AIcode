...
type Driver interface {
	...
	IsReachable() bool
	SetStorageRadiuser
	UpdatePeerHealth(addr  aisc.Address, h bool, t time.Duration)
}

type PeerAdder interface {
	// AddPeers is called when peers are added to the topology backlog
	AddPeers(addr ... aisc.Address)
}
...
type ClosestPeerer interface {
	...
	ClosestPeer(addr  aisc.Address, includeSelf bool, f Select, skipPeers ... aisc.Address) (peerAddr  aisc.Address, err error)
}
...
type EachPeerFunc func(addr  aisc.Address, bin uint8) (stop, jumpToNext bool, err error)

// PeerInfo is a view of peer information exposed to a user.
type PeerInfo struct {
	Address  aisc.Address       `json:"address"`
