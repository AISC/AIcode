...
import (
	...
	testingsoc "github.com/aisc/pkg/soc/testing"
...
const (
	...
	defaultMediaType = "aisc-manifest-mantaray"
)
...
// targeting  aisc.AddressIterFunc execution.
...
// Next matches the signature of  aisc.AddressIterFunc needed in
// Traverser.Traverse method and collects statistics about it's execution.
func (i *addressIterator) Next(addr  aisc.Address) error {
	i.mu.Lock()
	defer i.mu.Unlock()
...
func TestTraversalBytes(t *testing.T) {
	...
	for _, tc := range testCases {
		tc := tc
		chunkCount := int(math.Ceil(float64(tc.dataSize) /  aisc.ChunkSize))
...
func TestTraversalFiles(t *testing.T) {
	...
	for _, tc := range testCases {
		tc := tc
		chunkCount := int(math.Ceil(float64(tc.filesSize) /  aisc.ChunkSize))
		t.Run(fmt.Sprintf("%d-chunk-%d-bytes", chunkCount, tc.filesSize), func(t *testing.T) {
			...
			err = fManifest.Add(ctx, "/", manifest.NewEntry( aisc.ZeroAddress, rootMtdt))
			if err != nil {
				t.Fatal(err)
			}
...
func TestTraversalSOC(t *testing.T) {
	...
	s := testingsoc.GenerateMockSOC(t, generateSample( aisc.ChunkSize))
	sch := s.Chunk()
